...
bedrock.events.on('bedrock.init', () => {
  ...
  WITNESS_CACHE = new LruCache({max: 1000});
});
...
async function _getWitnesses({ledgerNode, blockHeight}) {
  ...
  // FIXME: remove passing latest block summary to block witnesses API and
...
function _getWitnessSelectionMethod({ledgerConfiguration}) {
  const {witnessSelectionMethod} = ledgerConfiguration;
  if(!witnessSelectionMethod) {
    throw new TypeError(
      '"ledgerConfiguration.witnessSelectionMethod" is required.');
  }
  return brLedgerNode.use(witnessSelectionMethod.type);
}
