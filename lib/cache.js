 ...
 * @return a Promise that resolves once the operation completes.
 */
api.consensus.setElectors = async ({electors, blockHeight, ledgerNodeId}) => {
 ...
 */
api.consensus.getElectors = async ({blockHeight, ledgerNodeId}) => {
  const key = _cacheKey.electors(ledgerNodeId);
  const json = await cache.client.get(key);
  if(!json) {
    return null;
  }
  const {electors, blockHeight: cachedBlockHeight} = JSON.parse(json);
...
api.events.addLocalMergeEvent = async ({event, meta, ledgerNodeId}) => {
  ...
  // full event goes into cache for gossip purposes
  ...
  try {
    const result = await cache.client.multi()
      ...
      .set(eventGossipKey, fullEvent, 'EX', 600)
...
api.events.setEventGossip = async (
  {event, eventHash, expire = 600, ledgerNodeId}) => {
  const eventKey = _cacheKey.eventGossip({eventHash, ledgerNodeId});
  return cache.client.set(eventKey, JSON.stringify({event}), 'EX', expire);
};
 ...
 * @return a Promise that resolves once the operation completes.
 */
api.voters.add = async ({voterId, ledgerNodeId}) => {
 ...
 */
api.voters.get = async ({ledgerNodeId}) => {
  const key = _cacheKey.voter(ledgerNodeId);
 ...
 * @return a Promise that resolves once the operation completes.
 */
api.voters.setLedgerNodeId = async ({voterId, ledgerNodeId}) => {
 ...
 */
api.voters.getLedgerNodeId = async ({voterId}) => {
  const key = _cacheKey.ledgerNode(voterId);
  return cache.client.get(key);
};
