...
function _getAncestorHashes({events, ledgerNodeId}) {
  ...
  // compute ordering for gossip first for two reasons: we reuse hash state
  // for events to optimize (it will be overwritten with the mixed hash once
  // `_sortIntoBlockOrder` is run) -- and current order will be randomized
  // when computing block order which would make gossip sorting take longer
  const gossipOrdering = _sortIntoGossipOrder({mergeEvents});
...
function _sortIntoGossipOrder({mergeEvents}) {
  /* Algorithm for gossip sort order:
  1. Sort merge events by `basisBlockHeight`. This ensures that blocks can
    be safely partitioned into arbitrary-sized chunks (with a minimum size of
    one merge event) for transfer without causing receiving clients to get
    stuck, unable to make the blocks they need to increase their
    `basisBlockHeight`.
  2. Sort again, topologically, but preserve `basisBlockHeight` order by
    topologically sorting each set of merge events that has the same
    `basisBlockHeight`. For each merge event, add its possible regular events
    in, sorted by event hash (no need to use a mixed hash here), before adding
    the merge event. This last step ensures that servers of gossip can easily
    partition blocks into merge events that the client is known to understand
    and to have the requisite ancestry to be able to validate and accept.
  */

  // 1. Sort by `basisBlockHeight`.
  mergeEvents.sort(_compareBasisBlockHeight);

  // 2. Sort same-basisBlockHeight chunks topologically. Add regular events,
  // sorted by hash, to produce the gossip ordering.
