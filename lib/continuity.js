...
api.findConsensus = ({
  ...
  // gather events that have achieved consensus; consider `y` events to have
  // achieved consensus (these are the same as `x` events anyway)
  const events = [];
...
function _getAncestorHashes({events, ledgerNodeId}) {
  ...
  // FIXME: Sorting has been historically performed here in `continuity.js`
  // instead of in `consensus.js` to reduce the amount of data that had to be
  // copied between processes when a workerpool was used. The workerpool
  // implementation has been removed, so this sorting could be moved outside of
  // this module and some complexity could be potentially reduced with respect
  // to processing `externalParents`.
 ...
 *   value is an array of objects with merge events `y` that have reached
 *   consensus.
 */
