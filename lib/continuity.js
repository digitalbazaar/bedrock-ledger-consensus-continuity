...
function _getAncestorHashes({events, ledgerNodeId}) {
  ...
  // compute base hash for mixin for sorting
  ...
  /* Algorithm for a total deterministic sort order for the events:
  1. Sort merge events by mixed hash (event hash + baseHash mixin).
  ...
  */
  ...
  mergeEvents.sort(compareEvents);
  ...
  //   merge event that merged them, in mixed-hash-sorted order. This order
  //   helps facilitate better gossip and processing of merge events as
  //   opposed to breadth-first sorting all regular events across merge events.
  next = mergeEvents;
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      if(event._c.externalParents) {
        event._c.externalParents.sort(compareEvents);
...
// FIXME: remove me
function _buildAncestorSet({event, ancestrySets}) {
  ...
  while(next.length > 0) {
    ...
    next = [];
    for(const event of current) {
      const allParents = event._c.parents.concat(
        event._c.externalParentHashes);
      for(const parent of allParents) {
        if(ancestors.has(parent)) {
          // already processed this lineage
          continue;
        }
        // add parent to ancestor set
        ancestors.add(parent);
        // copy parent's ancestor set if available
        const parentAncestorSet = ancestrySets.get(parent);
        if(parentAncestorSet) {
          for(const ancestor of parentAncestorSet) {
            ancestors.add(ancestor);
          }
          continue;
        }
        // iterate through parent's lineage
        next.push(parent);
      }
    }
  }
