...
async function _generateEd25519Key() {
  ...
  const publicKeyBuffer = Buffer.alloc(sodium.crypto_sign_PUBLICKEYBYTES);
  const privateKeyBuffer = Buffer.alloc(sodium.crypto_sign_SECRETKEYBYTES);
  sodium.crypto_sign_keypair(publicKeyBuffer, privateKeyBuffer);
  return {
