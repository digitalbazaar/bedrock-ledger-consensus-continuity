...
async function _getNeeded({ledgerNode, needed, peerId}) {
  ...
  let mergeBatch = _initMergeBatch();

  function _initMergeBatch() {
    return {
      events: [],
      eventCount: 0
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    const stream = _client.getEvents({eventHash: needed, peerId})
      ...
      .on('data', data => {
        ...
        mergeBatch.events.push(data.event);
        mergeBatch.eventCount++;

        // TODO allow for merge event itself
        // if(mergeBatch.eventCount > someMaxAllowableEvents + 1) {
        // if(mergeEvent.parentHash.length > someMaxAllowedValue) throw
        // if(regularEvent.operation.length > someMaxAllowedValue) throw
        ...
        q.add(async () => {
          ...
          if(mergeBatch.eventCount === neededCount) {
            const {eventHashes, uncommitted} = await _events.addBatch(
              {events: mergeBatch.events, ledgerNode, neededSet});
            // TODO: remove eventHashes from neededSet
            console.log('AFTER ADDBATCH', Date.now());
          }
          try {
            const record = await ledgerNode.consensus._events.add(
              {continuity2017: {peer: true},
                event: data.event, ledgerNode, neededSet});
