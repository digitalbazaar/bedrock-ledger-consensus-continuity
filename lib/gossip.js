...
function _tally(blockId, voters, callback) {
  /* Go through rounds and see if a majority won the round for the block. If
    a majority has voted for a particular manifest, finish tally and return
    the winner. If there is no majority but fewer than two-thirds have voted
    in a round, finish the tally because we need to wait for more votes to
    arrive. */
  // TODO: how to deal with double votes? can't simply say: "use the lowest
  // hash" or "ignore them" because not everyone will necessarily receive the
  // same votes!
  let done = false;
  let round = 0;
  let majority = Math.floor(voters.length / 2) + 1;
  ...
  async.until(() => done, callback => {
    api._storage.votes.tally(blockId, round++, (err, votes) => {
      ...
      } else if(votes[0].count > majority) {
        done = true;
        winner = votes[0].manifest;
      } else if(votes.length < twoThirds) {
        done = true;
      }
      callback();
    });
