...
api.contactElectors = ({
  ...
  async.auto({
    ...
    pull: ['notifications', (results, callback) => async.eachSeries([
      ...
      let done = false;
      async.until(() => done || condition(), callback => api.gossipWith(
        {callerId: creatorId, condition, ledgerNode, peerId}, (err, result) => {
...
api.gossipWith = ({ledgerNode, peerId}, callback) => {
  ...
  async.auto({
    ...
    events: ['history', (results, callback) => {
      ...
      async.timesLimit(history.length, 5, (i, callback) => {
        // i not included
    ...
    results.done = !results.history.truncated;
...
function _gossipMergeEvent(
  ...
  const eventHashes = mergeEvent.event.parentHash.filter(
    eventHash => !filter.includes(eventHash));
  const limit = gossipConfig.requestPool.maxSockets;
  async.mapLimit(eventHashes, limit, (eventHash, callback) =>
    _client.getEvent({eventHash, peerId}, (err, event) => {
      if(err) {
        return callback(err);
      }
      return callback(null, {eventHash, event});
    }), (err, result) => {
    if(err) {
      return callback(err);
    }
    // add the merge event
    result.push(mergeEvent);
    callback(null, result);
  });
}
