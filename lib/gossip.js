...
api.contactElectors = ({
  ...
  async.auto({
    ...
    poll: callback => api.pollPeers(
      {callerId: creatorId, contacted, ledgerNode, peers: toContact}, callback),
    ...
    push: ['poll', (results, callback) => async.eachLimit(
      toContact, limit, (elector, callback) => {
        const peerHeads = results.poll.heads[elector.id];
        if(!peerHeads) {
          // elector could not be contacted during polling, return early
          return callback();
        }
        api.pushGossip(
          {creatorId, ledgerNode, peerHeads, peerId: elector.id}, callback);
      }, callback)],
    pull: ['poll', (results, callback) => async.eachLimit(
      results.poll.top, limit, (elector, callback) => {
        ...
        api.gossipWith({isElector, ledgerNode, peerId: elector.id}, err => {
...
api.partitionHistory = (
  {creatorHeads, creatorId, eventTypeFilter}, callback) => {
  const _creatorHeads = bedrock.util.clone(creatorHeads);
  // remove the local creator from the heads
  ...
  async.auto({
    ...
    history: ['localHeads', 'creatorFilter', 'genesisMergeHash',
      (results, callback) => {
        ...
        const temp = {
          creatorId,
          before: bedrock.util.clone(_creatorHeads),
          creatorFilter: results.creatorFilter
        };

        // remove the creators in the filter from the list of heads
        results.creatorFilter.forEach(c => delete _creatorHeads[c]);

        temp.after = bedrock.util.clone(_creatorHeads);
...
api.pushGossip = ({creatorId, peerHeads, ledgerNode, peerId}, callback) => {
  async.auto({
    partitionHistory: callback => api.partitionHistory(
      {creatorHeads: peerHeads, creatorId}, callback),
