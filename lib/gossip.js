/*
 * Web Ledger Simple Gossip Protocol.
 *
...
function inspectLedger(ledgerId, session, callback) {
  ...
  //
  //   If you are a member of the population, then once you've spoken
  //   with every other member of the population once, you prepare your
  //   vote. Your vote must include every event you've heard about that
  //   hasn't yet achieved consensus. You include your vote in any future
  //   gossip -- and the winning vote must eventually get into the block.
  //   Note: If you are gossiping about an old time window for which you
  //   have already written a block, you include the vote stored in that
  //   block (which is just the events in order in that block) as your vote
  //   because it already won.
  //
  //   Failures:
  //
  //   1. a continuity member can't be contacted
  //   2. a continuity member isn't voting
  //   3. ???
  //
  //   Solution for #1.
  //   If a continuity member can't be contacted, proceed contacting
  //   the remaining members. If you are a continuity member, begin
  //   your vote after contacting all of the members you can get a
  //   hold of, provided that at least 51% of them can be reached.
  //   Otherwise, just keep trying to contact until
  //     configBlock.consensusTimeout/gossipTimeout is reached and a new
  //     set of continuity members is picked -- which is an indication that
  //     the population of continuity members should be included in the vote
  //     somewhere to be checked... needs more details worked out
  //
  //  Solution for #2.
  //  Collect all votes and if X member aren't voting and their votes couldn't
  //  sway the winner, ignore them. In other words, as soon as a winner can't
  //  be defeated, accept it and there's no need to keep communicating (is
  //  this true... we'll need a way to make sure votes are propagated when
  //  nodes are syncing to catch up on old data). Otherwise, it's no different
  //  from the solution for #1.

  // TODO: nodes say: "this is what i have for this time window"
