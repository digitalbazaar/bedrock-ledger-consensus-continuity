...
function _tmp(ledgerNode, session, callback) {
  ...
  async.auto({
    identity: callback => api._identity.get(ledgerNode, callback),
    ...
    gossip: ['voters', (results, callback) =>
      _gatherConsensus(ledgerNode, results.blockId, results.voters, callback)],
...
function _gatherConsensus(ledgerNode, blockId, voters, callback) {
  const manifest = {};
  const voteState = {};
  ...
  // determine if ledger node is a voter
  // if()
