    ...
    events: ['needed', (results, callback) => {
      ...
      _client.getEvents({eventHash: needed, peerId}, (err, stream) => {
        ...
        const q = async.queue((e, callback) => ledgerNode.consensus._events.add(
          {continuity2017: {peer: true}, event: e.event, ledgerNode},
          (err, result) => {
            callback(err, result);
          }), 1);
...
function _diff({eventHashes, ledgerNode}, callback) {
  ...
  console.log('NNNNNNNNN', eventHashes);
  async.auto({
    redis: callback => cache.client.multi()
      ...
      .exec((err, result) => {
        ...
        console.log('FFFFFFFFFF', notFound);
    ...
    mongo: ['redis', (results, callback) => {
      ...
      console.log('RRRRRRRRR', results.redis);
    ...
    console.log('9999999999999', eventHashes);
    console.log('SSSSSSSSSSSSS', needSet);
