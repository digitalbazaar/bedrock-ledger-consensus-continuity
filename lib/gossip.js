...
api.contactElectors = ({
  ...
  async.auto({
    ...
    sadd: ['poll', (results, callback) => {
      ...
      cache.client.sadd(cacheKey, activePeers, err => {
        if(err) {
          return callback();
        }
        cache.client.expire(
          cacheKey, Math.round(config['ledger-consensus-continuity']
            .worker.election.gossipInterval / 1000), () => callback());
      });
    }],
    ...
    srem: ['sadd', 'push', 'pull', (results, callback) => {
      ...
      cache.client.del(`ap-${creatorId}`, () => callback());
    }]
  }, callback);
};
