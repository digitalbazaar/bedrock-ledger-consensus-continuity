...
function _gossip(deciders, callback) {
  const manifest = {};
  const voteState = {};
  ...
  async.until(() => done, callback => {
    async.auto({
      ...
      collectVotes: callback => {
        // TODO: use client to contact a node and obtain all of its events
...
function officialGossip(peer, callback) {
  ...
  // * votes (indexed by manifest and round)
  //
