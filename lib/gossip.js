...
api.gossipWith = async ({callerId, ledgerNode, peer}) => {
  ...
  try {
    ...
    try {
      ...
      const startGetEventsTime = Date.now();
      ...
      timer.stop();
      const finishedGetEventsTime = Date.now();
      const size = await _getSizeInBytes(events);
      logger.verbose('Get needed finished...', {
        events: events.length,
        duration: finishedGetEventsTime - startGetEventsTime,
        size: `${size} B`
      });
    } catch(e) {
