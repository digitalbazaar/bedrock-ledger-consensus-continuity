...
api.gossipWith = ({ledgerNode, peerId}, callback) => {
  ...
  async.auto({
    ...
    partitionHistory: ['getEvents', (results, callback) =>
...
api.partitionHistory = (
  ...
  history: ['localHeads', 'creatorFilter', 'genesisMergeHash',
    (results, callback) => {
      ...
      // NOTE: for push gossip, the peer has informed us about the head it
      // has for this node (creatorId). We have been finding the first
      // descendant of that event and using that as the start of the search.
      // for push gossip, we want to send everything from the localBranch head
      // and stop at the head that the peer has provided.
      const treeHash = _creatorHeads[creatorId];
      // if this node knows about a creator that is not known to the peer
      // include all history for that creator up to the genesis merge
      // FIXME: is this right???
      ...
      // NOTE: treeHash is not adequate to restrict local node history
      ...
      async.auto({
        ...
        getHistory: ['restriction', (results, callback) => _events.getHistory({
          creatorFilter,
          ...
          startHash,
          // treeHash is used to calculate the firstDescendant
          treeHash
        }, callback)]
