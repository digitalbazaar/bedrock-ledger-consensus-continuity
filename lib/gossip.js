...
async function _getNeeded({needed, peerId}) {
  ...
  for(const eventHash of chunks) {
    try {
      ...
      const eventStream = await _client.getEvents({eventHash, peerId});
      logger.verbose('Line Stream created');
      const stream = eventStream.pipe(split2());
      eventStream.once('error', function(err) {
...
function _handleGossipError({error, reject}) {
  ...
  let t;
  return error.response.body.on('data', res => {
    logger.verbose('Received data in error...');
    if(!t) {
      return t = res;
    }
    ...
    try {
      ...
      logger.verbose('Successfully parsed JSON from error string...');
    } catch(e) {
      // error is not JSON
    }
    ...
    if(errorJson && errorJson.message && errorJson.type) {
      ...
      logger.verbose('Reject with errorJson...');
      return reject(new BedrockError(
        errorJson.message, errorJson.type, errorJson.details,
        errorJson.cause));
    }
    logger.verbose('Reject with BedrockError... network issue...');
    return reject(new BedrockError(
    ...
    logger.verbose('Caught uncaught error in stream....', {e});
    return reject(e);
  });
}
