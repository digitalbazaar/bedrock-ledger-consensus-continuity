 ...
 */
api.gossipWith = ({isElector, ledgerNode, peerId}, callback) => {
  ...
  async.auto({
    ...
    // NOTE: this function is not executed until later
    pushGossip: ['creator', 'peerHistory', (results, callback) => {
      callback(null, cb => {
        api.pushGossip({
...
api.pushGossip = ({creatorId, isElector, ledgerNode, peerId}, callback) => {
  const cacheKey = database.hash(creatorId + peerId);
  async.auto({
    cache: callback => {
      // FIXME: disabling cache
      return callback();
      // if this node is not an elector, override the cache
      if(!isElector) {
        logger.debug('Node is not an elector, proceeding to push gossip.', {
          destination: peerId,
          source: creatorId,
        });
        return callback();
      ...
      cache.client.get(cacheKey, (err, result) => {
        ...
        if(result) {
          // cache hit, return error to stop
          logger.debug('Peer has been contacted recently, abort.', {peerId});
          return callback(new BedrockError(
            'Peer has been contacted recently, abort.', 'AbortError',
            {peerId}));
        }
        // cache miss, proceed with gossip
        callback();
      });
    },
    peerHeads: ['cache', (results, callback) => _client.getHistory(
      {callerId: creatorId, headsOnly: true, peerId}, callback)],
    partitionHistory: ['peerHeads', (results, callback) => api.partitionHistory(
      {creatorHeads: results.peerHeads, creatorId}, callback)],
    sendEvents: ['partitionHistory', (results, callback) => {
      ...
      async.eachSeries(
        results.partitionHistory.history, (eventHash, callback) => {
          ...
          cache.client.set(
            cacheKey, true, 'EX',
            config['ledger-consensus-continuity'].gossip.cache.ttl);
          callback();
        });
    }]
  }, err => {
