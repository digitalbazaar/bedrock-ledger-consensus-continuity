    ...
    events: ['needed', (results, callback) => {
      ...
      _client.getEvents({eventHash: needed, peerId}, (err, stream) => {
        ...
        const q = async.queue((e, callback) => ledgerNode.consensus._events.add(
          {continuity2017: {peer: true}, event: e.event, ledgerNode, needed},
          (err, result) => {
            if(err) {
              return callback(err);
            }
            const {eventHash} = result.meta;
        ...
        q.drain = () => {
          ...
          // stream ended or errored, so invoke `_quit`
          _quit();
        };
        q.error = err => {
          stream.destroy();
        ...
        stream.on('end', () => {
          ...
          // if queue is idle then drain will not be called again and
          // `_quit` must be invoked here
          if(q.idle()) {
            _quit();
          }
        });
...
function _diff({eventHashes, ledgerNode}, callback) {
  ...
  async.auto({
    ...
    mongo: ['redis', (results, callback) => {
      ...
      ledgerNode.storage.events.difference(results.redis, callback);
    }]
  }, (err, results) => {
    if(err) {
      return callback(err);
