...
api.contactElectors = ({
  ...
  async.auto({
    ...
    pull: ['sadd', (results, callback) => async.eachLimit(
      toContact, limit, (peer, callback) => {
  ...
  async.auto({
    ...
    sendEvents: ['partitionHistory', (results, callback) => {
      ...
      if(eventCount > compressionThreshold) {
        ...
        // NOTE: events need to be processed in opposite order from regular push
        events.reverse();
        ...
        if(eventCount > maxEvents) {
          ...
          if(i === 0) {
            logger.debug('Push gossip event count exceeds maximum.', {
              compressionThreshold,
              eventCount,
              maxEvents,
            });
        ...
        return _pushCompressed({events, peerId}, callback);
      }
