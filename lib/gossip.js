  ...
  async.auto({
    ...
    push: ['poll', (results, callback) => async.eachLimit(
      results.poll.top, limit, (peer, callback) => {
        ...
        api.pushGossip({creatorId, peerHeads, peerId: peer.id}, callback);
      }, callback)],
...
api.pushGossip = ({creatorId, peerHeads, peerId}, callback) => {
  async.auto({
    ...
    sendEvents: ['partitionHistory', (results, callback) => {
      ...
      for(let i = history.length - 1; i >= 0; --i) {
        ...
        //   history[i].meta.continuity2017.creator === creatorId) {
        if(jsonld.hasValue(history[i].event, 'type', 'ContinuityMergeEvent')) {
      ...
      // mergeBundles.sort((a, b) => {
      //   return a.events[a.events.length - 1].event.parentHash
      //     .includes(b.mergeHash);
      // });
      mergeBundles.sort((a, b) => {
        return a.events[a.events.length - 1].meta.created -
          b.events[b.events.length - 1].meta.created;
      });
      // console.log('BBBBBBBB', JSON.stringify(mergeBundles, null, 2));
      // console.log('BBBBBBLLLLLLLLLLLLL', mergeBundles.length);
      async.eachOfSeries(mergeBundles, (bundle, bundleNum, callback) => {
        async.eachSeries(
          bundle.events, (eventRecord, callback) => {
            ...
            // algorithm results in a merge event being sent that produces an
            ...
            if(err && err.name === 'NetworkError' &&
              ...
              // console.log('XXXXXXXXXXXXXX', Date.now());
              console.log('XXXXXXXXXXX', JSON.stringify(mergeBundles, null, 2));
              throw new Error('boom');
              // console.log('HHHHHHHHHHH', creatorId.substr(-5), JSON.stringify(history, null, 2));
