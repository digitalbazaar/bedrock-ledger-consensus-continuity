  ...
  async.auto({
    ...
    events: ['history', (results, callback) => {
      ...
      async.timesLimit(history.length, limit, (i, callback) => {
        ...
        _gossipMergeEvent(
          {eventHashFilter, ledgerNode, mergeEvent, peerId}, callback);
      }, callback);
    }],
...
function _diff({eventHashes, ledgerNode}, callback) {
  ...
  async.auto({
    ...
    logger.debug('_diff, return', {
      resultsMongo: results.mongo
    });
...
function _gossipMergeEvent(
  ...
  const eventHashes = mergeEvent.event.parentHash.filter(
    h => !filter.includes(h));
  async.auto({
    difference: callback => _diff({eventHashes, ledgerNode}, callback),
    events: ['difference', (results, callback) => _client.getEvents(
      {eventHash: results.difference, peerId}, callback)]
  }, (err, results) => {
    if(err) {
      return callback(err);
    }
    results.events.push(mergeEvent);
    callback(null, results.events);
  });
}
