...
api.gossipWith = ({ledgerNode, peerId}, callback) => {
  ...
  async.auto({
    ...
    sendEvents: ['partitionHistory', (results, callback) => {
      // FIXME: limiting push gossip to 150 events to reduce duplicate errors
      // push gossip is regular events and merge events
      const limit = 100;
      logger.debug(
        'Push history event count', {
          limit,
          original: results.partitionHistory.history.length
        }
      );
      const historyPart = results.partitionHistory.history.slice(0, limit - 1);
      async.eachSeries(
        historyPart, (eventHash, callback) => async.auto({
