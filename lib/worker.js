...
async function _sync(session) {
  ...
  try {
    // FIXME: update agents to use promises
    await Promise.all([
      promisify(eventWriter.start.bind(eventWriter))(),
      promisify(gossipAgent.start.bind(gossipAgent))(),
      promisify(mergeAgent.start.bind(mergeAgent))(),
      promisify(consensusAgent.start.bind(consensusAgent))(),
      promisify(operationAgent.start.bind(operationAgent))()
    ]);
  } catch(e) {
    stopAgents();
    throw e;
