...
function _sync(session, callback) {
  ...
  logger.debug('consensus job running', {ledgerNodeId: ledgerNode.id});
  // logger.debug(
  ...
  async.parallel([
    ...
    callback => mergeAgent.start(callback),
    callback => consensusAgent.start(callback),
    callback => operationAgent.start(callback),
  ], err => {
