  ...
  async.until(condition, loop => {
    ...
    async.auto({
      ...
      merge: ['consensusRequired', (results, callback) => {
        ...
        _merge({ledgerNode, blockHeight: state.blockHeight}, callback);
      }],
      pushGossip: ['merge', (results, callback) => {
        logger.debug('Starting push gossip.');
        Promise.all(results.pullGossip)
          .then(() => {
            logger.debug('Push gossip complete.');
            callback();
          })
...
function _updateState({ledgerNode, state, voter}, callback) {
  async.auto({
    ...
    update: ['getElectors', (results, callback) => {
      ...
      state.contacted = {};
      const isElector = !!_.find(state.electors, e => e.id === voter.id);
      if(isElector) {
        // do not contact self
...
function _merge({ledgerNode, blockHeight}, callback) {
  async.auto({
    history: callback => api._events.getRecentHistory({ledgerNode}, callback),
...
function _contactElectors({
  ...
  async.eachLimit(random, limit, (elector, nextElector) => {
    ...
    _gossip.gossipWith(
      {ledgerNode, peerId: elector.id,
        previousBlockHash, blockHeight, elector}, (err, pushPromise) => {
  ...
  }, err => err ? callback(err) : callback(null, pushPromises));
}
