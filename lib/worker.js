    ...
    async.auto({
      ...
      consensus: ['newRegularEvents', (results, callback) => {
        ...
        if(condition()) {
          ...
          logger.debug('L324 insufficient time remaining to find consensus');
          console.log('not enough time left, quitting1');
          return expire(loop);
        }
...
function _findConsensus({ledgerNode, electors}, callback) {
  async.auto({
    ...
    merge: ['history', (results, callback) => {
      logger.debug('L348 running mergeBranches');
...
function _hasNewRegularEvents({ledgerNode}, callback) {
  logger.debug('L363 Start getHashes');
  ...
  ledgerNode.storage.events.getHashes({
    ...
    logger.debug('L374 getHashes Result', {err, hashes});
    if(err) {
      return callback(err);
    }
    if(hashes.length === 0) {
