...
function _runElection(
  ...
  async.until(condition, loop => {
    ...
    async.eachLimit(random, limit, (elector, nextElector) => {
      ...
      // create callback that will execute `loop` on next gossip interval
      let nextInterval = gossipInterval;
      if(gossipInterval >= session.timeRemaining()) {
        expired = true;
        nextInterval = 0;
      }
      const next = err => setTimeout(loop.bind(null, err), nextInterval);
