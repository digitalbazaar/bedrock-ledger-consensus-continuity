...
function _sync(session, callback) {
  ...
  async.auto({
    ...
    stopWriter: ['extend', (results, callback) => ew.stop(callback)]
  }, err => {
    ...
    async.auto({
      ...
      consensus: ['finishBlock', (results, callback) => _findConsensus(
        {ledgerNode, voter, state, condition}, (err, consensus) => {
          ...
          if(contacted === state.electors.length && state.pulledNotifications &&
            mergeAttempted) {
            if(!consensusRequired) {
...
function _updateState({ledgerNode, state, voter}, callback) {
  async.auto({
    ...
    update: ['getElectors', (results, callback) => {
      ...
      state.pulledNotifications = false;
    ...
    keys: callback => {
      ...
      async.map(
        signatureCreators, (publicKeyId, callback) =>
          api._storage.keys.getPublicKey(
