    ...
    events: ['difference', (results, callback) => async.eachSeries(
      results.difference, (eventHash, callback) => {
        async.auto({
          ...
          signature: ['mergeEvent', (results, callback) => signature.verify(
            {doc: results.mergeEvent, ledgerNodeId: ledgerNode.id}, callback)],
          // retrieve all parent events in parallel
          difference: ['signature', (results, callback) =>
            ledgerNode.storage.events.difference(
              results.mergeEvent.parentHash, callback)],
          ...
          store: ['parentEvents', (results, callback) => {
            async.each(results.parentEvents, (e, callback) => {
              if(e.type === 'WebLedgerEvent') {
                // add the event
                return ledgerNode.events.add(
                  e, {continuity2017: {peer: true}}, callback);
              }
              callback();
            }, callback);
          }],
          ...
          storeMerge: ['store', (results, callback) => ledgerNode.events.add(
            results.mergeEvent, {continuity2017: {peer: true}}, callback)]
        }, (err, results) => {
          if(err) {
            console.log('EEEEEEE', JSON.stringify(err.toObject(), null, 2));
            return callback(err);
          }
          callback();
        });
...
function _gossipWith({ledgerNode, peerId}, callback) {
  ...
  async.auto({
    ...
    history: ['treeHash', (results, callback) => api._client.getHistory(
      {peerId, treeHash: results.treeHash}, callback)],
    events: ['history', (results, callback) => _gossipEvents(
      {eventHashes: results.history, ledgerNode, peerId}, callback)]
  }, err => callback(err));
}
