...
function _extendBlockchain({session, ledgerNode, voter}, callback) {
  ...
  async.until(condition, loop => {
    ...
    async.auto({
      ...
      gossip: ['consensusRequired', (results, callback) => _contactElectors(
        {ledgerNode, voter, condition, ...state}, callback)],
      merge: ['gossip', (results, callback) => {
        if(condition()) {
        ...
        mergeAttempted = true;
        _merge({ledgerNode, ...state}, callback);
      }]
    }, loop);
  }, callback);
}
  ...
  async.auto({
    ...
    consensus: ['initState', 'history', (results, callback) => {
      ...
      api._election.findConsensus(
        {ledgerNode, history: results.history, ...state},
        (err, result) => {
