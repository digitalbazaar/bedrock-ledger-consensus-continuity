    ...
    async.auto({
      ...
      consensus: ['newRegularEvents', (results, callback) => {
        ...
        _findConsensus({ledgerNode, electors}, (err, consensus) => {
          logger.debug('_findConsensus complete.', {consensus});
          if(err) {
            logger.error('Error in _findConsensus', {error: err});
            return callback(err);
          }
          if(consensus) {
            logger.debug('Found consensus.', {consensus});
...
function _findConsensus({ledgerNode, electors}, callback) {
  async.auto({
    ...
    merge: ['history', (results, callback) => {
      logger.debug('L337 running mergeBranches');
    ...
    history2: ['merge', (results, callback) => {
      logger.debug('Starting getRecentHistory.');
      api._events.getRecentHistory({ledgerNode}, (err, result) => {
        logger.debug('getRecentHistory complete');
        callback(err, result);
      });
    }],
    consensus: ['history2', (results, callback) => {
      ...
      api._election.findConsensus(
        {electors, history: results.history2, ledgerNode}, (err, result) => {
          logger.debug('election.findConsensus complete.', {result});
          callback(err, result);
        });
    }]
...
function _gossipWith({ledgerNode, peerId}, callback) {
  ...
  async.auto({
    ...
    peerHistory: ['creator', 'treeHash', (results, callback) =>
      api._client.getHistory({
        callerId: results.creator.id, peerId, treeHash: results.treeHash
      }, (err, result) => {
        if(err) {
          logger.error('PEER HISTORY FAILED', {error: err});
        }
        logger.debug('_gossipWith peerHistory', {peerHistory: result});
        callback(err, result);
      })],
    getEvents: ['peerHistory', (results, callback) => _gossipEvents({
      eventHashes: results.peerHistory.history, ledgerNode, peerId
    }, callback)],
    localPeerHistory: ['treeHash', (results, callback) =>
      events.aggregateHistory({
        eventTypeFilter: 'ContinuityMergeEvent',
        ledgerNode,
        startHash: results.treeHash
      }, callback)],
    // FIXME: necessary to wait for getEvents?
    localHistory: [
      'getEvents', 'localPeerHistory', 'peerHistory', (results, callback) =>
        events.getHistory({
          eventHashFilter: [
            ...results.peerHistory.history, ...results.localPeerHistory
          ],
          ledgerNode,
          treeHash: results.peerHistory.callerHead
        }, (err, result) => {
          logger.debug('gossipWith localHistory', {result});
