...
api.scheduleWork = (session) => {
  ...
  const maxTime = bedrock.config['ledger-consensus-continuity'].worker.session.maxTime;
...
function _runElection(
  ...
  const condition = () => (electionResult.events && electionResult.rollCall) ||
    session.isExpired();
  const limit = bedrock.config['ledger-consensus-continuity'].gossip.concurrentPeers;
