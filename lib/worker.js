...
function _gossipWith({ledgerNode, peerId}, callback) {
  ...
  async.auto({
    ...
    localHistory: ['treeHash', (results, callback) => events.getHistory(
      {ledgerNode, treeHash: results.treeHash}, (err, result) => {
        // TODO: complete push gossip
        console.log('EVENTS FOR PUSH', err, result);
        callback();
      })]
  }, err => callback(err));
}
