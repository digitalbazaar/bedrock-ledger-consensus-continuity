...
function _writeBlock(
  ...
  async.auto({
    ...
    keys: ['votes', (results, callback) => {
      let signatureCreators = results.votes.map(v => v.signature.creator);
      signatureCreators = _.uniq(signatureCreators);
      async.map(
...
// TODO: rename, replacing old `_writeBlock`
// consensusResult = {event: [event hashes], consensusProof: [event hashes]}
function _writeBlock2(ledgerNode, blockHeight, consensusResult, callback) {
  async.auto({
    config: callback =>
      ledgerNode.storage.events.getLatestConfig((err, result) => {
        if(err) {
    ...
    events: callback =>
      // TODO: optimize getting events
      async.map(consensusResult.event, (eventHash, callback) =>
        ledgerNode.events.get(eventHash, callback), callback),
    consensusProof: callback =>
      // TODO: optimize getting events
      async.map(consensusResult.consensusProof, (eventHash, callback) =>
        ledgerNode.events.get(eventHash, callback), callback),
    updateEvents: callback =>
      async.each(consensusResult.event, (eventHash, callback) =>
        ledgerNode.storage.events.update(eventHash, [{
          op: 'unset',
          changes: {
            meta: {
    ...
    keys: ['events', 'consensusProof', (results, callback) => {
      const signatureCreators = _.uniq(
        results.events
          .map(r => r.event.signature.creator)
          .concat(results.consensusProof.map(r => r.event.signature.creator)));
      async.map(
        signatureCreators, (keyId, callback) => api._storage.keys.getPublicKey(
          ledgerNode.id, {id: keyId}, callback), (err, result) => {
          if(err) {
    ...
    createBlock: ['config', 'keys', 'previousBlock', 'updateEvents',
      (results, callback) => {
        const block = {
          ...
          consensusMethod: 'Continuity2017',
          type: 'WebLedgerEventBlock',
          event: results.events.map(r => r.event),
          consensusProof: results.consensusProof.map(r => r.event),
          previousBlock: results.previousBlock.eventBlock.block.id,
          previousBlockHash: results.previousBlock.eventBlock.meta.blockHash,
          publicKey: results.keys
        };
