  ...
  async.auto({
    ...
    writeBlock: ['updateState', (results, callback) => {
      ...
      _writeBlock({
        ledgerNode,
        blockHeight: results.nextBlock.blockHeight,
        consensusResult: results.election.consensusResult
      }, callback);
    }],
...
function _runElectionNEW({
  ...
  async.until(condition, loop => {
    ...
    _contactElectorsNEW(
      ...
      // FIXME: this check needs to check for *regular* events only, it should
      //   NOT include `ContinuityMergeEvent` as these alone do not mean there
      //   are more events that need to reach consensus, to clarify,
      //   we should not create a new merge event if there are no regular
      //   events that need to reach consensus (i.e. there's no actual state
      //   machine changes that need recording in the blockchain)... we also
      //   don't need to try and find consensus

      // see if there are any non-merge events to put in a block
      ledgerNode.storage.events.getHashes({
        // FIXME: also ensure type is not `ContinuityMergeEvent`
        consensus: false,
        limit: 1
      }, (err, hashes) => {
        if(err) {
        ...
        // TODO: run events.getRecentHistory

        // TODO: check to see if there are any events that have no children
        //  that events.mergeBranches can merge together, expiring early if not

        // TODO: run events.mergeBranches

        // TODO: if successful, add new merge event to recent history and
        //   update each parent by doing ._children.push(newMergeEvent),
        //   update treeParent by doing treeParent._treeChild = newMergeEvent,
        //   update newMergeEvent with _children and _parent arrays
        //   (i.e. emulate what a whole extra call to `getRecentHistory`
        //   would do without having to call it again just for the one
        //   new event)

        // TODO: run _findConsensus, setting electionResult.consensus and
        //   then expire
...
function _contactElectorsNEW(
  {ledgerNode, voter, previousBlockHash, blockHeight, electors,
  peers, condition, state}, callback) {
  ...
  let toContact = _.difference(electors, contacted).filter(
    e => e.id !== voter.id);
  const max = Math.min(
    ...
    createBlock: ['config', 'keys', 'previousBlock', 'updateEvents',
      (results, callback) => {
        ...
        api._hasher(block, (err, blockHash) => {
          ...
          block.event = consensusResult.event;
