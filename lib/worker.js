...
function _sync(ledgerNode, session, callback) {
  ...
  async.auto({
    ...
    getElectors: ['nextBlockId', (results, callback) => {
      console.log('LEDGERNODE_ID', ledgerNode.id);
      console.log('NEXTBLOCKID', results.nextBlockId);
      api._voters.getBlockElectors(ledgerNode, results.nextBlockId, callback);
    }],
...
function _getNextBlockId(ledgerNode, callback) {
  // FIXME: add `blockNumber` or `blockHeight` property to blocks and then
  // use it from previous block
  // TODO: get latest consensus block from ledgerNode storage
  // TODO: evaluate alternate approach of storing id number and building new
  // id:
  //   ledgerNode.storage.events.getLatestConfig(cb)
  //   config.event.input[0].ledger + /blocks/ + id
  ledgerNode.storage.blocks.getLatest((err, block) => {
    if(err) {
      return callback(err);
    }
    const last = block.eventBlock.block.id;
    const nextParts = last.split('/');
    nextParts[nextParts.length - 1] =
      (parseInt(nextParts[nextParts.length - 1], 10) + 1).toString();
...
function _electNextManifest(
  session, ledgerNode, voter, blockId, electors, callback) {
 ...
 * @param callback(err, manifest) called once the operation completes.
 */
function _tally(ledgerNodeId, blockId, voters, callback) {
  ...
  async.until(() => done, callback => {
    api._storage.votes.tally(ledgerNodeId, blockId, round++, (err, votes) => {
      if(err) {
        return callback(err);
      }
...
function _vote(ledgerNode, voter, blockId, callback) {
  ...
  api._storage.votes.getLast(ledgerNode.id, blockId, voter.id, (err, vote) => {
    if(err) {
      return callback(err);
    }
    if(vote) {
      // previous vote has occurred, so do instant run-off and vote again
      return _instantRunoffVote(ledgerNode, vote, voter, callback);
    }
    _firstRoundVote(ledgerNode, voter, blockId, callback);
  });
}

function _firstRoundVote(ledgerNode, voter, blockId, callback) {
  // TODO: no previous vote, then
  //   create a manifest out of all non-deleted, non-consensus event hashes
  // TODO: note - could be zero current events... what then?
 ...
 */
function _createCurrentManifest(ledgerNode, blockId, callback) {
...
function _instantRunoffVote(ledgerNode, vote, voter, callback) {
