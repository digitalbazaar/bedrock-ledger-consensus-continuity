...
function _sync(session, callback) {
  ...
  async.auto({
    ...
    if(results.election.initialConsensus >=
      api._election.twoThirdsMajority(results.getElectors.length)) {

      // FIXME: this can happen when the twoThirdsMajority of nodes have
      // reached consensus on a *new* block since last _sync session.  Perhaps
      // inspecting the timestamps on the block could fine tune this

      // this node is catching up, run sync again immediately
      logger.debug('Ledger node is catching up.', {
        ledgerNodeId: ledgerNode.id,
        blockHeight: results.nextBlock.blockHeight
      });
      return _sync(session, callback);
    }
    callback(err);
  });
}
        ...
        gossip: callback => _gossipWith(
          ...
          (err, consensusPhase) => {
            ...
            if(loopCount === 1 && consensusPhase === 'consensus') {
              electionResult.initialConsensus++;
            }
            callback();
          }),
