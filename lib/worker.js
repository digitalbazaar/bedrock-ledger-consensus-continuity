...
function _writeBlock(
  ...
  async.auto({
    ...
    rollCallManifest: api._storage.manifests.get(
      ledgerNode.id, rollCallManifestHash, callback),
    events: ['eventManifest', (results, callback) => {
      // TODO: get events from manifest
      callback();
    }],
    votes: ['rollCallManifest', (results, callback) => {
      // TODO: get votes from manifest
      callback();
    }],
    validateEvents: ['events', (results, callback) => {
      // TODO: validate all events, serially in order, those that fail
      // must be removed from the list of events to include in the block
      callback();
    }],
    // TODO: what options go in `getLatest`?
    previousBlock: callback => ledgerNode.storage.blocks.getLatest(
      {}, callback),
    previousBlockHash: ['previousBlock', (results, callback) =>
      hasher(results.previousBlock, callback)],
    block: ['events', 'votes', (results, callback) => {
      const block = {
        '@context': 'https://w3id.org/webledger/v1',
        id: results.config.ledger + '/blocks/' + blockHeight,
        type: 'WebLedgerEventBlock',
        event: [results.validateEvents],
        // TODO: add votes under `electionResults`
        //electionResults: votes,
        previousBlock: results.previousBlock.id,
        previousBlockHash: results.previousBlockHash
      };
      hasher(block, (err, hash) => {
        if(err) {
          return callback(err);
        }
        callback(null, {
...
// TODO: duplicated from `index.js` ... avoid this
