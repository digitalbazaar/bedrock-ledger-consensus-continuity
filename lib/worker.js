    ...
    async.eachLimit(random, limit, (elector, nextElector) => {
      ...
      async.auto({
        ...
        if(results.eventsVotes) {
          electionResult.events = results.eventsVotes.manifestHash;
        }
        if(results.rollCallVotes) {
          electionResult.rollCall = results.rollCallVotes.manifestHash;
      ...
      if(!isElector) {
        return setTimeout(
          loop,
          config['ledger-consensus-continuity']
            .worker.election.gossipInterval);
      }
