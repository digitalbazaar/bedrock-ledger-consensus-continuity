...
function _writeBlock(
  ...
  async.auto({
    ...
    votes: ['rollCallManifest', (results, callback) => {
      ...
      api._votes.get(ledgerNode.id, blockHeight, 'RollCall', (err, records) => {
        if(err) {
          return callback(err);
        }
    ...
    validateEvents: ['events', (results, callback) => {
      console.log('RRRRR', results.events);
    ...
    previousBlockHash: ['previousBlock', (results, callback) =>
      api._hasher(results.previousBlock, callback)],
    block: ['events', 'votes', 'previousBlock', 'previousBlockHash',
      (results, callback) => {
        console.log('BBBBBB', results.previousBlock);
        const block = {
