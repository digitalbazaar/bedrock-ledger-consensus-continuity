  ...
  async.until(condition, loop => {
    ...
    async.each(electors, (elector, nextElector) => {
      ...
      if(!manifests.events) {
        ...
        return _voteForEvents(ledgerNode, voter, blockHeight, loop);
      }
      ...
      if(!manifests.rollCall) {
        ...
        //   vote; vote for one.
        return _voteForRollCall(ledgerNode, voter, blockHeight, loop);
      }
 ...
 * @param callback(err, manifest) called once the operation completes.
 */
function _tally(ledgerNodeId, blockHeight, voters, electionType, callback) {
 ...
 * @param callback(err) called once the operation completes.
 */
function _voteForEvents(ledgerNode, voter, blockHeight, callback) {
  ...
  api._storage.votes.getLast(
    ...
    _castFirstRoundEventVote(ledgerNode, voter, blockHeight, callback);
  });
}
 ...
 * @param callback(err) called once the operation completes.
 */
function _castFirstRoundEventVote(ledgerNode, voter, blockHeight, callback) {
  async.auto({
...
function _castInstantRunoffEventVote(ledgerNode, vote, voter, callback) {
 ...
 * @param callback(err) called once the operation completes.
 */
function _voteForRollCall(ledgerNode, voter, blockHeight, callback) {
  // get last vote cast by voter for the block
  api._storage.votes.getLast(
    ...
    if(vote) {
      // previous vote has occurred, so do instant run-off and vote again
      return _castInstantRunoffRollCallVote(ledgerNode, vote, voter, callback);
    }
...
function _castInstantRunoffRollCallVote(ledgerNode, vote, voter, callback) {
  // TODO: implement me
