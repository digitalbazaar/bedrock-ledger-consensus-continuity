  ...
  async.auto({
    ...
    history2: ['merge', (results, callback) => {
      ...
      api._events.getRecentHistory({ledgerNode}, (err, result) => {
        logger.debug('getRecentHistory complete', {result});
        callback(err, result);
      });
    }],
...
function _gossipWith({ledgerNode, peerId}, callback) {
  ...
  async.auto({
    ...
    // TODO: necessary to wait for getEvents?
    localHistory: ['getEvents', 'peerHistory', (results, callback) =>
      events.getHistory({
        creatorFilter: peerId,
        eventHashFilter: results.peerHistory.history,
        ledgerNode,
        treeHash: results.peerHistory.callerHead
      }, (err, result) => {
        console.log('!!!!!!!!!!LOCALHISTORY', result);
    ...
    sendEvents: ['localHistory', (results, callback) => async.eachSeries(
      results.localHistory, (eventHash, callback) => async.auto({
        ...
        send: ['get', (results, callback) => {
          console.log('EEEEEEEEEEE', results.get.event);
