  ...
  async.auto({
    ...
    creatorHeads: ['allCreators', (results, callback) => {
      ...
      async.each(results.allCreators, (creatorId, callback) =>
        api._events._getLocalBranchHead(
    ...
    peerHistory: ['creatorHeads', (results, callback) =>
      api._client.getHistory({
        creatorHeads: results.creatorHeads, peerId
      }, (err, result) => {
        if(err) {
    ...
    localHistory: [
      'creator', 'getEvents', 'genesisMergeHash',
      'peerHistory', (results, callback) => {
        ...
        events.getHistory({
          ...
          // ],
          eventHashFilter: _.values(peerCreatorHeads),
          ledgerNode,
          treeHash
        }, (err, result) => {
