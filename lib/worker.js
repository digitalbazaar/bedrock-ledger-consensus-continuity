...
function _sync(ledgerNode, session, callback) {
  ...
  async.auto({
    ...
    // TODO: get additional peers from `ledgerNode.peerLedgerAgents` and
    // pass them to `_runElection` after `electors` as an additional set of
    // peer to gossip with... these may be necessary to bootstrap a ledger,
    // also note that a helper function should be used to get the voter IDs
    // from those Web Ledger Agents or from a local cache of them
