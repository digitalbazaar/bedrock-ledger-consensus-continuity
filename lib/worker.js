...
function _getNextBlockInfo(ledgerNode, callback) {
  ...
  ledgerNode.storage.blocks.getLatest({expandEvents: false}, (err, block) => {
    if(err) {
      return callback(err);
    }
...
function _contactElectors(
  ...
  async.eachLimit(random, limit, (elector, nextElector) => {
    ...
    async.auto({
      ...
      console.error("Execution time (hr): %ds %dms", hrend[0], hrend[1]/1000000);
      nextElector();
    });
  }, callback);
}
          ...
          async.each(toSend, (e, callback) => async.auto({
            ...
            sendEvent: ['getEvent', (results, callback) =>
              api._client.sendEvent({
                event: results.getEvent.event,
                eventHash: results.getEvent.meta.eventHash,
                peerId: peer.id}, err => {
                if(err) {
                  logger.verbose('Non-critical error in _gossipWith.', err);
                }
