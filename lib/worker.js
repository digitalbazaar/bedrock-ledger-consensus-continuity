...
function _runElection({
  ...
  async.until(condition, loop => {
    ...
    // loop:
    // 1. if there are regular events, find consensus
    // 2. run gossip
    async.auto({
      newRegularEvents: callback =>
      ...
      consensus: ['newRegularEvents', (results, callback) => {
        ...
        if(!newEventsFound) {
          if(gossipped) {
            // already gossipped and still nothing to merge so expire
            // FIXME: uncomment... tests currently depend on a full gossip
            //   round with all electors -- fix tests
            //return expire(loop);
          }
          // nothing to find consensus on
          return callback();
        }
        if(condition()) {
        ...
        _findConsensus({ledgerNode, electors}, callback);
      }],
      gossip: ['consensus', (results, callback) => {
        electionResult.consensus = results.consensus;
        if(condition()) {
          // insufficient time to gossip
          return expire(loop);
        }
        gossipped = true;
        _contactElectors(
          {ledgerNode, voter, previousBlockHash, blockHeight, electors,
            peers, condition, electionResult}, callback);
      }]
    }, callback);
...
function _hasNewRegularEvents({ledgerNode}, callback) {
  // FIXME: this is not right either, what is need here?
  const eventsCollection = ledgerNode.storage.events.collection;
  return async.auto({
    creator: callback => voters.get(ledgerNode.id, (err, result) =>
      err ? callback(err) : callback(null, result.id)),
    head: ['creator', (results, callback) => api._events._getLocalBranchHead(
      {creator: results.creator, eventsCollection}, callback)],
    event: ['head', (results, callback) => eventsCollection.findOne({
      'event.type': {$ne: 'ContinuityMergeEvent'},
      'event.treeHash': results.head,
      'meta.consensusDate': {$exists: false}
    }, {eventHash: 1}, (err, result) => callback(err, !!result))]
  }, (err, results) => err ? callback(err) : callback(null, results.event));

  // FIXME: existing implementation does not filter merge events, but also
  // it returns events that have already been merged
  // always return true for now (always returning false does not work)
...
function _contactElectors({
  ...
  const toContact = electors.filter(e => e.id !== voter.id);
  const max = Math.min(
    ...
    createBlock: ['config', 'keys', 'previousBlock', 'updateEvents',
      (results, callback) => {
        ...
        api._hasher(block, (err, blockHash) => {
          ...
          block.consensusProof = consensusResult.consensusProof.map(
            r => r.eventHash);
          callback(null, {
            block: block,
            meta: {blockHash}
