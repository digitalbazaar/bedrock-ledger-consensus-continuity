  ...
  async.until(condition, loop => {
    ...
    _contactElectors(
      ...
      const expire = () => {
        ...
        expired = true;
        // detect if node is catching up or consensus is in progress by
        // seeing if enough nodes reported consensus for the current
        // block height
        const inConsensus = Object.keys(state.contacted)
          ...
          .reduce((accumulator, consensusPhase) =>
            accumulator + (consensusPhase === 'consensus' ? 1 : 0), 0);
        if(inConsensus >= api._election.twoThirdsMajority(electors.length)) {
          // node is catching up/consensus is in progress, either way we
          // should schedule to run again immediately and not wait for
          // `gossipInterval` to expire
          logger.debug(
      ...
      ledgerNode.storage.events.getHashes({
        ...
        if(hashes.length === 0) {
          // nothing to vote on, clear contacted electors
          state.contacted = {};
          return expire();
        }
