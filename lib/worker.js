  ...
  async.until(condition, loop => {
    ...
    async.auto({
      ...
      consensus: ['newRegularEvents', (results, callback) => {
        ...
        /*console.log('\n\nRUNNING FIND CONSENSUS FOR BLOCK', blockHeight, {
          ledgerNode: ledgerNode.id
        });*/
        // NOTE: ***DO NOT LOG CONSENSUS TO DEBUG OR CONSOLE, IT CAN BE HUGE***
        _findConsensus({ledgerNode, blockHeight, electors}, (err, consensus) => {
...
function _findConsensus({ledgerNode, blockHeight, electors}, callback) {
  async.auto({
    ...
    //   updates are fixed still outstanding as of 2017-12-19
    ...
    consensus: ['history2', (results, callback) => {
      ...
      api._election.findConsensus(
        {electors, history: results.history2, blockHeight, ledgerNode}, (err, result) => {
