...
function _sync(ledgerNode, session, callback) {
  ...
  async.auto({
    ...
    nextManifest: ['getElectors', (results, callback) => {
...
function _electNextManifest(
  ...
  async.until(() => winner || session.isExpired(), loop => {
    ...
    async.each(electors, (elector, callback) => {
      ...
      // TODO: also, if we already have a winner, don't bother voting
      if(err || winner || !isElector) {
        return loop(err);
      }
      // 4. Now that all other electors have been contacted, if not done
      //   already, since we are an elector, vote.
      _vote(ledgerNode, voter, blockHeight, loop);
    });
  }, err => err ? callback(err) : callback(err, winner));
}
...
function _gossipWith(ledgerNode, voter, blockHeight, peer, callback) {
  ...
  _getBlockStatus(blockHeight, peer, (err, status) => {
    ...
    // TODO: we need another step in here where an election is over, but
    // we need to pick the exact voting results that will go into the
    // consensus block; the results of that "second election" don't have to
    // be logged, but the results of the first one does, so we can have
    // that voter information in the ledger itself so that the elector
    // selection algorithm has access to it (and so that elector nodes
    // can indicate address changes in their votes?... if we want to support
    // that in version 1 of continuity)

    // 2.b. Certify votes if in 'election' or 'consensus' phase.
    // TODO: throw error if status.phase !== 'election' or 'consensus'
...
/**
 * Tallies the vote for the passed block height. If there is a winning manifest
 ...
 * @param voters the voting population for the block height.
 * @param callback(err, manifest) called once the operation completes.
 */
function _tally(ledgerNodeId, blockHeight, voters, callback) {
  ...
  async.until(() => done, callback => {
    api._storage.votes.tally(
      ledgerNodeId, blockHeight, round++, (err, votes) => {
      if(err) {
        return callback(err);
      }
 ...
 * @param callback(err) called once the operation completes.
 */
function _vote(ledgerNode, voter, blockHeight, callback) {
  // get last vote cast by voter for the block
  api._storage.votes.getLast(
    ledgerNode.id, blockHeight, voter.id, (err, vote) => {
    if(err) {
      return callback(err);
    }
    if(vote) {
      // previous vote has occurred, so do instant run-off and vote again
      return _castInstantRunoffVote(ledgerNode, vote, voter, callback);
    }
    _castFirstRound(ledgerNode, voter, blockHeight, callback);
  });
}
 ...
 * @param callback(err) called once the operation completes.
 */
function _castFirstRound(ledgerNode, voter, blockHeight, callback) {
  async.auto({
    createManifest: callback => _createCurrentManifest(
    ...
    storeVote: ['signVote', (results, callback) => {
      api._storage.votes.add(ledgerNode.id, results.signVote, err => {
 ...
 */
function _createCurrentManifest(ledgerNode, blockHeight, callback) {
...
function _castInstantRunoffVote(ledgerNode, vote, voter, callback) {
