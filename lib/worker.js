  ...
  async.until(condition, loop => {
    ...
    _contactElectors(
      ...
      const expire = () => {
        ...
        expired = true;
        // detect if a super majority of electors were not in the gossip phase
        // for the current block height
        const inConsensus = Object.keys(state.contacted)
          .map(electorIdHash => state.contacted[electorIdHash].consensusPhase)
          .reduce((accumulator, consensusPhase) =>
            accumulator + (consensusPhase !== 'gossip' ? 1 : 0), 0);
        if(inConsensus >= api._election.twoThirdsMajority(electors.length)) {
          // possible explanations for a super majority not in the gossip
          // phase include some combination of the below:
          // 1. this node is behind and needs to catch up
          // 2. consensus phases have begun
          // 3. nodes could not be contacted
          // ... for any of these cases we schedule to run again immediately
          // and not wait for `gossipInterval` to expire
          logger.debug(
            'Ledger node is catching up or consensus is in progress; ' +
      ...
      ledgerNode.storage.events.getHashes({
        ...
        if(hashes.length === 0) {
          // nothing to vote on, clear contacted electors so we will gossip
