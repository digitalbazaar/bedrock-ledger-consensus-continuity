...
function _gossipWith(ledgerNode, voter, blockHeight, peer, callback) {
  ...
  // TODO: just call `api._client.getBlockStatus(peer, ...`
  //   and eliminate helper function here
  api._client.getBlockStatus(blockHeight, peer, (err, status) => {
    if(err) {
      return callback(err);
    }
    console.log('BBBBBBSSSSSSSSS', status);
    // 2.a. Gossip with peer if in 'gossip' phase.
    if(status.phase === 'gossip') {
      // TODO: status has a list of event hashes in it...

      // TODO: in parallel:
      // 1. determine all event hashes that the peer does
      //   not know that we know (check our database against the hash list)
      // 2. determine all event hashes that we don't know

      // TODO: then, in parallel:
      // 1. Send all events that the peer doesn't know via POST /events
      // 2. Get all events that we don't know via GET /events
