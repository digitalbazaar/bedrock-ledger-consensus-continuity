  ...
  async.auto({
    ...
    incompleteBlock: callback =>
      ledgerNode.storage.blocks.getSummaryByHeight({
        blockHeight: state.blockHeight, consensus: false, eventHash: true
      }, (err, summary) => {
    ...
    consensusResult: ['incompleteBlock', (results, callback) => {
      ...
      consensusResult.eventHash = results.incompleteBlock.block.eventHash;
      // TODO: support getting consensus proof hashes from storage in summary
      // ...currently they are always returned
      consensusResult.consensusProofHash =
        results.incompleteBlock.block.consensusProofHash;
...
function _postWriteBlock(
  ...
  async.auto({
    ...
    updateEvents: ['updateKey', (results, callback) =>
      async.each(consensusResult.eventHash, (eventHash, callback) => {
        const now = Date.now();
    ...
    cache: ['updateEvents', (results, callback) => {
      ...
      let mergeEventKeys;
      if(consensusResult.mergeEventHash) {
        // exact merge event hashes are known (not recovering from an
        // incomplete block)
        mergeEventKeys = consensusResult.mergeEventHash
          .map(eventHash => _cacheKey.event({eventHash, ledgerNodeId}));
      } else {
        // recovering from an incomplete block, so do not have a breakdown
        // of merge events that excludes regular events, but this will not
        // trigger an error with redis (it will not error on deleting keys
        // that do not exist)
        mergeEventKeys = consensusResult.eventHash.concat(
          consensusResult.consensusProofHash);
      }
      cache.client.multi()
