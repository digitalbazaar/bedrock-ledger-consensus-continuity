...
// TODO: document
// consensusResult = {eventHash, ordering, mergeEventHash, witnesses, forkerSet}
api.write = async ({worker, consensusResult}) => {
  ...
  // use `consensusResult.forkerSet` to update *all* merge events for *all*
  // entries in the `forkerSet` to have
  // `meta.continuity2017.forkDetectedBlockHeight=blockHeight`
  const {forkerSet} = consensusResult;
  if(forkerSet.size > 0) {
    await _markNewForkers({ledgerNode, forkers: [...forkerSet], blockHeight});
  }

  // update events with block information up to 100 at a time
  // FIXME: this should be converted to a bulk update operation and batches
  // of 1000 should be sent with the rationale that that update size stays
  // under the 16 MiB query limit
  const {ordering} = consensusResult;
  let now = Date.now();
  const eventUpdates = [];
  for(const {eventHash, blockOrder, gossipOrder} of ordering) {
    eventUpdates.push({
      eventHash,
      patch: [{
        op: 'set',
  ...
  await ledgerNode.storage.events.updateMany({events: eventUpdates});
...
async function _markNewForkers({ledgerNode, forkers, blockHeight} = {}) {
  // FIXME: make this a covered query
  const {collection} = ledgerNode.storage.events;
  const result = await collection.updateMany({
    'meta.continuity2017.creator': {$in: forkers},
    'meta.continuity2017.type': 'm'
  }, {
    $set: {'meta.continuity2017.forkDetectedBlockHeight': blockHeight}
  });
  return result;
}
