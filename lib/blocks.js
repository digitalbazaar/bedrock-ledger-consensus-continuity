...
api.write = ({ledgerNode, state, consensusResult}, callback) => {
  async.auto({
    ...
    createBlock: ['config', 'keys', (results, callback) => {
      ...
      // FIXME: eventHash and consensusProofHash are xxHashes, should be
...
// TODO: proofs should be stored and retrieved in the same fasion has events
...
function _postWriteBlock(
  ...
  async.auto({
    ...
    cache: ['updateEvents', (results, callback) => {
      ...
      logger.debug('Removing outstanding event keys.', {mergeEventKeys});
      cache.client.multi()
        .srem(outstandingMergeKey, mergeEventKeys)
        .del(mergeEventKeys)
        .exec((err, result) => {
          logger.debug('Key removal results', {error1: err, result});
          callback(err, result);
        });
    }],
