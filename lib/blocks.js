...
// TODO: document
// consensusResult = {eventHash, ordering, mergeEventHash, witnesses}
api.write = async ({worker, consensusResult}) => {
  // FIXME: handle fork detection:
  // FIXME: use a counter for tracking forks; just need to always bump it
  // beyond the max counter for a given peer every time we find a new fork
  // until that fork has been "detected" be reaching consensus in a block;
  // if we two different fork numbers for the same peer ever reach consensus
  // (including the `null` fork number) -- we know the fork was detected
  // 1. during peer event validation, get children of tree hash parents for
  //   every merge event (at the same time as looking up the out of batch
  //   parents)... should usually be zero results, but if any records are
  //   returned, we know it's a fork, but before doing anything else, check
  //   the tree parent to see if it has `forkedAtBlockHeight` already set,
  //   and, if so, if the `basisBlockHeight` for the merge event >= to the
  //   `forkedAtBlockHeight` value, throw an exception, the merge event is
  //   invalid; otherwise, copy its `forkedAtBlockHeight` value, no need to set
  //   a fork ID; if that isn't set, get the latest fork ID by doing a database
  //   query using an index on fork ID for creator and sort by fork ID, inc
  //   by 1 and return that value; set that as the `meta.forkId` on the event
  // 2. when writing a block, if you get a merge event in that has
  //   `meta.forkId` but not `meta.forkedAtBlockHeight` set, then do a database
  //   query with a limit of 1 for any merge event by the same creator with
  //   a different fork ID with consensus=true; if found, then the fork was
  //   detected for the first time; set `meta.forkedAtBlockHeight=blockHeight`
  //   on the the merge event and all other merge events by that creator
  ...
  await Promise.all(ordering.map(({eventHash, blockOrder, gossipOrder}) =>
...
api.writeGenesis = async ({block, ledgerNode}) => {
  const meta = {
