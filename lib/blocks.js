...
// FIXME: remove this; repairing the cache should not be necessary
// TODO: document
// ensure block is committed to cache
api.repairCache = async ({blockHeight, ledgerNode}) => {
  const ledgerNodeId = ledgerNode.id;
  ...
  const cache = await _cache.blocks.commitBlock({ledgerNodeId});
...
api.write = async ({worker, consensusResult}) => {
  ...
  // FIXME: remove redis caching of block
  const ledgerNodeId = ledgerNode.id;
  await _cache.blocks.commitBlock({ledgerNodeId});
  ...
  // update worker consensus state now that a new block has been written -- if
  // no configuration change has occurred -- otherwise, presume that the
  // configuration change will cause the work session to be terminated
  if(!hasEffectiveConfigurationEvent) {
    await worker._updateConsensusState({blockRecord, consensusResult});
  }

  return {blockHeight, hasEffectiveConfigurationEvent};
};
