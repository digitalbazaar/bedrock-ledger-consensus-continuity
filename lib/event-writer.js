    ...
    this.client.blpop(_cacheKey.eventQueue(ledgerNodeId), 3, (err, pResult) => {
      ...
      async.auto({
        ...
        range: callback => this.client.lrange(
          _cacheKey.eventQueue(ledgerNodeId), 0, 1000, callback),
        ...
        prepare: ['get', (results, callback) => {
          console.log('GGGGGGGGGGGGGGGGGGGG', results.get);
          if(!results.get) {
            return callback();
          }
          ...
          for(let i = 0; i < results.get.length; ++i) {
            ...
            // self contained head tracking here? treeHash in eventHashes?
          ...
          // headGeneration map is Map [eventHash, generation]
          callback(null, {eventHashes, events, headTreeMap});
        }],
        // mutates results.events
        generation: ['prepare', (results, callback) => {
          if(results.prepare.headTreeMap.size === 0) {
            return callback();
          }
          console.log('%%%%', JSON.stringify(results.prepare.events, null, 2));
          // eventHash, treeHash
          ...
          _events.getHeads(
            {eventHashes, ledgerNode: this.ledgerNode}, (err, result) => {
              ...
              console.log('************** GETHEADS', result);
              // if a head for and eventHash was not returned, it means
              // the ancestor has not been seen before and must be in this batch
                ...
                for(let i = 0; i < events.length; ++i) {
                  ...
                  if(events[i].eventHash === treeHash) {
                    ...
                    if(generation === undefined) {
                      // this is the first event for this creator, intialize
                      generation = 0;
                    }
                    ...
                    console.log('THIS COLLECTION', this.ledgerNode.storage.events.collection.s.name);
                    console.log('PARENT', events[i].meta);
                    console.log('CHILD', child.meta);
                  }
                }
              }
