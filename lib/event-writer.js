...
module.exports = class EventWriter {
  ...
  start() {
    ...
    this.client.blpop(eventQueueKey, 1, (err, pResult) => {
      if(err) {
      ...
      async.auto({
        ...
        range: callback => this.client.lrange(eventQueueKey, 0, 1000, callback),
        ...
        get: ['range', (results, callback) => {
          ...
          const rangeCount = results.range ? results.range.length : 0;
        ...
        store: ['generation', (results, callback) => {
          ...
          this.storage.events.collection.insertMany(
            results.prepare.events, {ordered: true}, (err, result) => {
              if(err) {
                if(database.isDuplicateError(err) || (err.writeErrors &&
                  ...
                  console.log('IIIIIIIII', result.nInserted);
                  console.log('LLLLLLLLLLL', results.prepare.events.length);
                  return callback();
                }
                return callback(err);
              }
