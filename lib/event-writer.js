...
module.exports = class EventWriter {
  ...
  start() {
    ...
    this.client.blpop(_cacheKey.eventQueue(ledgerNodeId), 3, (err, pResult) => {
      if(err) {
      ...
      async.auto({
        ...
        prepare: ['get', (results, callback) => {
          ...
          for(let i = 0; i < results.get.length; ++i) {
            eventHashes.push(results.get[i].meta.eventHash);
            _.defaults(results.get[i].meta, {created: now, updated: now});
            events.push({
              event: results.get[i].event,
              eventHash: results.get[i].meta.eventHash,
              meta: results.get[i].meta
            });
            ...
            // designed to update creatorId to the last generation in the list
            headMap[creatorId] = {
              eventHash: meta.eventHash,
              generation: meta.continuity2017.generation
            };
            if(headAdvance[creatorId]) {
              headAdvance[creatorId]++;
            } else {
              headAdvance[creatorId] = 1;
            }
          }
          callback(null, {eventHashes, events, headAdvance, headMap});
        }],
        store: ['prepare', (results, callback) => {
          if(!results.get) {
            return callback();
          }
        ...
        cache: ['store', (results, callback) => {
          ...
          const headUpdate = [];
          const creators = Object.keys(results.prepare.headMap);
          creators.forEach(creatorId => {
            ...
            headUpdate.push(
              headKey, results.prepare.headMap[creatorId].eventHash,
              headGenerationKey, results.prepare.headMap[creatorId].generation
            );
          });
          ...
          multi.srem(eventQueueSetKey, results.prepare.eventHashes);
          if(headUpdate.length !== 0) {
            multi.mset(headUpdate);
          }
          const {headAdvance} = results.prepare;
          Object.keys(headAdvance).forEach(creatorId => {
            const headAdvanceKey = _cacheKey.headAdvance(
              {creatorId, ledgerNodeId});
            multi.decrby(headAdvanceKey, headAdvance[creatorId]);
          });
          multi.exec(callback);
        }],
      }, err => {
