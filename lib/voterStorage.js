 ...
 *
 * TODO: maybe remove `ledgerAgentId` and use `voter-id` in URL... and
 *   maybe include nextBlockId and phase
 *
...
api.get = (options, callback) => {
  ...
  if(!(options.ledgerNodeId || options.voterId)) {
    throw new Error('options.ledgerNodeId or options.voterId must be set.');
  }
  ...
  if(options.ledgerNodeId) {
    query.ledgerNodeId = options.ledgerNodeId;
  } else  {
    query.voterId = options.voterId;
  }
  ...
  collection.findOne(query, {voter: 1}, (err, record) => {
    ...
    if(!record) {
      const details = {};
      if(options.ledgerNodeId) {
        details.ledgerNodeId = options.ledgerNodeId;
      } else {
        details.voterId = options.voterId;
      }
      return callback(new BedrockError(
