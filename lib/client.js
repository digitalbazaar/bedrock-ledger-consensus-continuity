...
const {wrapKeyPair} = require('./signature');
...
async function _signRequest({keyId, url, json}) {
  ...
  const plainText = createSignatureString({
    includeHeaders,
    headers,
    requestOptions: {created, url, method: 'POST', headers, keyId}
  });
  const data = new TextEncoder().encode(plainText);
  const keyPair = await wrapKeyPair({peerId: keyId});
...
exports.notifyPeer = async ({localPeerId, remotePeer}) => {
  ...
  try {
    const headers = await _signRequest({keyId: localPeerId, url, json});
