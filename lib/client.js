...
api.getHistory = (treeHash, peer, callback) => {
  const url = peer + '/gossip';
  request.post(
    {url, strictSSL, json: {treeHash}, timeout}, (err, res) => {
      if(err) {
        return callback(new BedrockError(
          'Could not get history.', 'NetworkError', {peer, treeHash},
          err));
      }
      if(res.statusCode !== 200) {
        return callback(new BedrockError(
...
api.getManifest = (manifestHash, peer, callback) => {
  ...
  async.auto({
    get: callback => request.get(
      {url, strictSSL, json: true, timeout}, (err, res) => {
        if(err) {
          return callback(new BedrockError(
...
api.sendEvent = (options, callback) => {
  const {event, eventHash, peerId} = options;
