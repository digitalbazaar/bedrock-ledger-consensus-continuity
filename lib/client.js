...
api.getManifest = (manifestHash, peer, callback) => {
  ...
  request.get({url, strictSSL, json: true}, (err, res) => {
    if(err) {
      return callback(new BedrockError(
        err.messsage, err.name, {manifestHash, peer}, err));
    }
    if(res.statusCode !== 200) {
      return callback(new BedrockError(
        res.body || 'Could not get manifest.',
        'NetworkError', {
          httpStatusCode: res.statusCode,
          manifestHash,
          peer
...
api.sendEvent = (event, peer, callback) => {
  ...
  request.post({url, strictSSL, json: event}, (err, res) => {
    if(err) {
      return callback(new BedrockError(
        err.messsage, err.name, {event, peer}, err));
    }
    if(res.statusCode !== 201) {
      return callback(new BedrockError(
        res.body || 'Could not send event.',
        'NetworkError', {
          httpStatusCode: res.statusCode,
          event,
          peer
...
api.getEvent = (eventHash, peer, callback) => {
  ...
  request.get({url, strictSSL, json: true}, (err, res) => {
    if(err) {
      return callback(new BedrockError(
        err.messsage, err.name, {eventHash, peer}, err));
    }
    if(res.statusCode !== 200) {
      return callback(new BedrockError(
        res.body || 'Could not get event.',
        'NetworkError', {
          httpStatusCode: res.statusCode,
          eventHash,
          peer
