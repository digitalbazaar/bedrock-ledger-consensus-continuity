...
exports.getHistory = async ({
  blockHeight, localPeerId, peerHeads, remotePeerId, eventIndex
} = {}) => {
  if(!localPeerId) {
  ...
  const data = {
    ...
    // the local peer knows less than the full block, it can send `eventIndex`
    blockHeight,
    // the peerId sent to the peer node is the peerId of the local node
    peerId: localPeerId,
    peerHeads: heads
  };
  // if `eventIndex` is specified, the local peer doesn't know the full
