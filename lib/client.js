...
const EVENT_VALIDATION_URL_CACHE = new LRU({max: 100, maxAge: 60 * 1000});
bedrock.events.on('bedrock.ready', async () => {
  const {'ledger-consensus-continuity': {
    gossip: {eventValidation}
  }} = config;

  const {httpsAgentOpts} = eventValidation;

  if(httpsAgentOpts) {
    EVENT_VALIDATION_HTTPS_AGENT = new https.Agent(httpsAgentOpts);
  } else {
