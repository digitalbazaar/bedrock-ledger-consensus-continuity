...
api.getCompressedHistory = ({callerId, creatorHeads, peerId}, callback) => {
  ...
  // FIXME: allocate a properly sized buffer
  // const eventsGz = Buffer.alloc(1000000);
  // const eventStream = new stream.Duplex();
  const buffers = [];
  // eventStream.end(eventsGz);
  const r = request.post({url, strictSSL, json, timeout});
  r.on('error', callback);
  r.on('response', res => {
    if(res.statusCode !== 200) {
      return callback(new BedrockError(
        'Error returned by server.', 'NetworkError', {
    ...
    r.on('end', err => {
      callback(err, Buffer.concat(buffers));
    });
  });
};
...
api.sendCompressed = ({gzEvents, peerId}, callback) => {
  const url = `${peerId}/events-compressed`;
  const r = request.post({strictSSL, url}, (err, res) => {
    if(err) {
      return callback(err);
    }
    if(res.statusCode !== 204) {
      return callback(new BedrockError(
        'Could not send compressed events.', 'NetworkError',
        {httpStatusCode: res.statusCode, peerId}));
    }
    callback();
  });
  const formData = r.form();
  formData.append('events.gz', gzEvents, {
