...
const {getKeyPair} = require('./localPeers');
...
const _signRequest = async ({peerId, url, json}) => {
  ...
  const plainText = createSignatureString({
    includeHeaders,
    requestOptions: {url, method: 'POST', keyId: peerId}
  });
  const keyPair = await getKeyPair({peerId});
...
exports.notifyPeer = async ({localPeerId, remotePeer}) => {
  ...
  const headers = await _signRequest({peerId: localPeerId, url, json});
  try {
