/*
 * Client for Continuity2017 gossip protocol.
 *
...
2. Add client.js
  ...
  - get events
3. Add deciders.js
  - get decider population for a block ID
  - include URLs for each node
...
function _tmp(ledgerNode, session, callback) {
  ...
  async.auto({
    deciders: callback => _getDeciders(callback),
    gossip: ['deciders', (results, callback) => {
      _gossip(results.deciders, callback);
    }]
  }, (err, results) => {
  });
...
function _getDeciders(callback) {

}

// gather consensus for a single block
function _gatherConsensus(deciders, callback) {
  const manifest = {};
  const voteState = {};
  ...
  // TODO: determine if this node is a decider
  ...
  //   Go through rounds and see if a majority won the round for the block
  //   Must be 67 votes per round before moving to the next round
  //   If a majority is found, quit loop early, we have consensus
  ...
  //   only writing the first 51 votes will not work when there is network
...
//   the phase. If any manifest has received 51 votes, end the phase
...
// Note: Once a majority vote is in, it is not so simple to add those
//   votes to a consensus block. Different nodes may receive different
//   parties (e.g. one has votes from 51 members, another has 52 -- but
...
//   the block, but at least a clear majority vote will be included in the
//   blockchain and it would have agreement across nodes.
...
//   be counted as a majority. We need to agree on
//   the "roll call".
...
//     support and include the list of 51 most likely
...
//   there is a manifest with 51 votes and those 51
//   include (NOT GOING TO WORK: FLP)


  // TODO: update code below based on algorithms above, it is old
  let done = false;
  async.until(() => done, callback => {
    async.auto({
      // TODO: write temporary manifest to database
      manifest: callback => _updateManifest(manifest, callback),
      pickDecider: callback => {
        // TODO: pick a random decider to gossip with
        callback();
      },
      collaborate: callback => {
        // TODO: run officialGossip(decider, callback)

        // TODO: track current vote state
        // TODO: if decider reports a vote, record it and don't talk
        //   to them again until the next "round"
        // if none left, done = true;
        // TODO:
      },
      estimateView: callback => {
        // TODO: estimate view to adopt

        // TODO: if majority voted for a view, decide it, we're done

        // TODO: otherwise if this node is a decider, calculate new vote
        //   and write that to meta
      }
    }, callback);
  });
}
...
function officialGossip(peer, callback) {
  ...
  // deleted. A manifest that has received a majority vote can be dereferenced
  // by looking at the events in the block it is associated with. The event
  // hashes for these events can be hashed (in lexicographical order) to
  // produce the manifest hash.
  //
  ...
  // majority for the current round, then we have consensus.
