...
api.sendEvent = ({callerId, event, eventHash, mergeHash, peerId}, callback) => {
  ...
  request.post({
    json: {callerId, event, eventHash, mergeHash}, strictSSL, timeout, url,
    time: true
  }, (err, res) => {
    if(err) {
      return callback(new BedrockError(
...
api.getEvent = ({eventHash, peerId}, callback) => {
  ...
  request.get({url, strictSSL, json: true, time: true, timeout}, (err, res) => {
    if(err) {
      return callback(new BedrockError(
        'Could not get event.', 'NetworkError', {eventHash, peerId}, err));
    }
    logger.debug('client.getEvent', {
      elapsedTime: res.elapsedTime
    });
    if(res.statusCode !== 200) {
      return callback(new BedrockError(
