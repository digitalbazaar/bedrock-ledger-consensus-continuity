...
bedrock.events.on('bedrock.ready', async () => {
  const {'ledger-consensus-continuity': {
    gossip: {eventValidation, operationValidation}
  }} = config;

  if(eventValidation.httpsAgentOpts) {
    EVENT_VALIDATION_HTTPS_AGENT = new https.Agent(
  ...
  if(operationValidation.httpsAgentOpts) {
    OPERATION_VALIDATION_HTTPS_AGENT = new https.Agent(
...
exports.validateEvent = async ({event, ledgerNodeId, localPeerId, session}) => {
  ...
  const data = {event, ledgerNodeId, url, session};
  const response = await httpClient.post(url, {
...
exports.validateOperation = async ({
  operation, ledgerNodeId, basisBlockHeight, localPeerId, url, session
}) => {
  const {'ledger-consensus-continuity': {
    gossip: {operationValidation}
  }} = config;
  ...
  const data = {operation, ledgerNodeId, basisBlockHeight, session};
  const response = await httpClient.post(url, {
