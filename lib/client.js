...
exports.validateEvent = async ({event, ledgerNodeId, creatorId, session}) => {
  const {'ledger-consensus-continuity': {
    client: {timeout},
    gossip: {eventsValidation}
  }} = config;

  const baseUrl = eventsValidation.baseUrl || creatorId;
  const url = baseUrl + '/events-validation';

  let httpsAgent;
  if(eventsValidation.httpsAgentOpts) {
    httpsAgent = new https.Agent(eventsValidation.httpsAgentOpts);
  } else {
    ({httpsAgent} = brHttpsAgent);
  }
...
exports.getHistory = async ({
  basisBlockHeight, blockHeight, localPeerId, peerHeadsMap,
  remotePeerId, blockEventCount, localEventNumber
} = {}) => {
  if(!localPeerId) {
  ...
  const data = {
    ...
    basisBlockHeight,
    // `blockHeight` sent is the *known* `blockHeight` to the local peer; if
    // the local peer knows less than the full block, it can send
    // `blockEventCount`
    blockHeight,
  ...
  // if `blockEventCount` is specified, the local peer doesn't know the full
  // block for `blockHeight`
  if(blockEventCount !== undefined) {
    data.blockEventCount = blockEventCount;
  }
  // if `localEventNumber` is specified, include it for skipping non-consensus
  // events that have already been received
...
exports.notifyPeer = async ({localPeerId, remotePeerId}) => {
  const url = `${remotePeerId}/notify`;
  const {'ledger-consensus-continuity': {client: {timeout}}} = config;
  const {httpsAgent} = brHttpsAgent;
  try {
    await axios({
