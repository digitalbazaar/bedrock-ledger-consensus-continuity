...
api.findConsensus = ({
  ...
  if(recoveryElectors.length > 0 &&
    ...
    throw new Error(
      '"recoveryGenerationThreshold" must be given when recovery electors ' +
      'are specified.');
  }
 ...
 */
api.supermajority = electorCount => (electorCount <= 3) ? electorCount :
 ...
 */
api.maximumFailures = electorCount => (api.supermajority(electorCount) - 1) / 3;
...
function _findMergeEventProof({
  ...
  recoveryGenerationThreshold, logger = {debug: () => {}, verbose: () => {}}
}) {
  ...
  // if recovery electors are specified but we have not received sufficient
  // generations of events from them, then we can safely skip all recovery
  // calculations, proceeding as if recovery electors were NOT specified
  if(recoveryElectors.length > 0) {
    // if all recovery electors haven't participated yet or if their tail
    // event does not have a high enough generation, then we can't possibly
    // make a recovery decision, so clear recovery electors to proceed
    // as if they were not set (safe to do, if consensus is reached then
    // it would only be reached when recovery mode was not entered anyway)
    for(const elector of recoveryElectors) {
      const electorTails = tails[elector.id];
      if(!electorTails || electorTails.every(
        r => r._generation < recoveryGenerationThreshold)) {
        recoveryElectors = [];
        break;
      }
    }
  ...
  if(Object.keys(yByElector).length < supermajority) {
    // can only return early if `recoveryElectors.length === 0`
    if(recoveryElectors.length === 0) {
    ...
    // `2f+1` ... thus we proceed by only checking for recovery mode trigger
    yByElector = {};
  }
...
function _computeRDescendants(
  ...
  for(const elector of recoverySet) {
    ...
    for(const r of rByElector[elector]) {
...
function _computeYDescendants({ledgerNodeId, event, electorSet, yByElector}) {
...
function _updateRecoveryInfo({
  ledgerNodeId, electors, descendants, recoverySet, recovery,
  logger = {debug: () => {}, verbose: () => {}}
}) {
  ...
  */
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      if(event._treeParent._votes || event._votes) {
        if(!event._votes) {
        ...
        if(!event._yDescendants) {
          const diff = _computeYDescendants(
            {ledgerNodeId, event, electorSet, yByElector});
      ...
      // info
      if(event._treeParent._recovery || event._recovery) {
        if(!event._recovery) {
          // shallow copy tree parent's recovery info
          // FIXME: do we need to copy or can we reuse?
          const {noProgressCounter, electorsSeen} = event._treeParent._recovery;
          event._recovery = {
            noProgressCounter,
          ...
          _updateRecoveryInfo(
            {ledgerNodeId, electors, descendants: diff, recoverySet,
              recovery: event._recovery, logger});
        }
...
function _tally({
  ...
  return _computeSupport(
    {ledgerNodeId, event, blockHeight, electors, recoveryElectors, logger});
}
...
function _computeRecovery({
  ...
  // if endorsement has been reached, that means that a consensus decision has
  // not been, so enter recovery mode
  if(!event._recovery.byzantine && event._recovery._endorsement === event) {
    // enter recovery mode
  ...
  // threshold
  if(!event._recovery._proposal &&
    event._recovery.noProgressCounter >= recoveryGenerationThreshold) {
    // recovery mode proposed! ... this elector cannot ever reject this
    // proposal, though, the proposal will not be activated until all
    // recovery electors also propose entering recovery mode -- and detect
    // that all other recovery electors have done so
    event._recovery._proposal = true;
  }
  ...
  const nonByzantineRecoveryElectors = [];
  for(const trackingEvent of trackingEvents) {
    const creator = _getCreator(trackingEvent);
    if(trackingEvent._votes && trackingEvent._votes[creator] === false) {
      // recovery elector is byzantine, don't count it
      byzantine++;
    } else if(trackingEvent._recovery.proposal) {
      proposals++;
      nonByzantineRecoveryElectors.push({id: creator});
    }
  }

  const requiredProposals = recoveryElectors.length - byzantine;
  if(proposals === requiredProposals) {
    // here we must calculate an endorsement event for determining when
    // it's safe to enter recovery mode, i.e. just because we know that
    // recovery mode should be entered, it does not mean that the other
    // recovery electors know yet (because they may not have our event),
    // so we wait until an endorsement of this event so we know they know
    const ancestryMap = _buildAncestryMap(event);
    const endorsement = _findEndorsementMergeEvent({
      ledgerNodeId, x: event, electors: nonByzantineRecoveryElectors,
      requiredProposals, descendants: {}, ancestryMap
    });
    if(endorsement) {
      if(endorsement.length > 1) {
        // more than one endorsement means byzantine recovery elector,
        // disqualify it
        event._recovery.byzantine = true;
        /*console.log(
          'detected byzantine recovery elector', _getCreator(event),
          ' while finding recovery mode endorsement for', event._generation);*/
      } else {
        event._recovery.endorsement = endorsement[0];
      }
      /*console.log(
        'created recovery mode endorsement for', event._generation);*/
    }
  }
...
function _computeSupport({
  ...
  Rejecting proposals/Deciding:
    ...
    3. If not rejected, see if >= 2f+1 endorsed proposals for the same thing
       ...
       a decision can only be made if the event was created by a recovery
       elector AND that elector does not have a pending
       "recovery mode proposal". If this caveat applies but is not met, simply
       continue on and hope that the next event will satisfy the requirements.
  ...
  */
  if(proposal) {
    /*console.log('EXISTING PRECOMMIT FOR',
      proposal._supporting.map(r=>r._generation));*/
    ...
    if(total >= (f + 1)) {
      //console.log('switching due to f+1 proposals at', event._generation);
      nextChoice = tally[0];
    }
    ...
    */
    ...
    if(total >= (f + 1)) {
      //console.log(
      //  'switching due to f+1 endorsed proposals at', event._generation);
      nextChoice = tally[0];
    }
  ...
  */
  if(proposal) {
    ...
    if(!reject) {
      ...
      /*if(reject) {
        console.log(
          'rejecting proposal due to f+1 endorsed support at',
          event._generation);
      }*/
    }

    if(reject) {
      /*console.log('rejecting proposal for',
        proposal._supporting.map(r=>r._generation));*/
      proposal = null;
    }
  }
  ...
  if(proposal) {
    ...
    */
    //console.log(
    //  'endorsed proposal count', nextChoice.endorsedProposalCount);
    if(nextChoice.endorsedProposalCount >= supermajority) {
      // can only decide if no recovery electors are specified or if they
      // are and *this* event is a recovery elector created event that has
      // no recovery proposal
      if(true || recoveryElectors.length === 0 ||
  ...
  */
  if(hasSupermajority && !proposal) {
    /*console.log('previous proposal not found, creating new one at',
      event._generation);*/
    ...
    if(!event._endorser) {
      ...
      if(endorsement) {
        ...
        /*console.log(
          'created endorse point for proposal', event._generation,
          'at', event._endorser._generation);*/
      }
    }
  }
