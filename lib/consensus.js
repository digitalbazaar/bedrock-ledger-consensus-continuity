...
function _computeSupport({
  ...
  Compute support:
    1. If >= `2f+1` support for the same thing, support it.
  ...
  */
  ...
  for(const elector in event._votes) {
    ...
    // update counts
    tallyResult.count++;
    if(tallyResult.count >= supermajority) {
      supermajoritySupport = tallyResult;
    }
    if(_hasEndorsedSwitch(e)) {
      tallyResult.endorsedSwitchCount++;
    }
    if(e._proposal) {
  ...
  */
  let nextChoice;
  if(supermajoritySupport) {
    nextChoice = supermajoritySupport;
  } else {
  ...
  */
  ...
  if(switched) {
    ...
    if(!lastSwitch._switchEndorsed &&
      ...
      // update count
      nextChoice.endorsedSwitchCount++;
    }
  }
...
function _getPriorityPeers({
  ...
  For each elector, determine if it needs an endorsement for a switch,
  decision proposal, or recovery proposal. If so, track whether any other
  elector can endorse and see if there are enough to reach a supermajority.
  ...
  */
  ...
  for(const [elector, head] of heads) {
    ...
    // proposal or for a switch... use this to test whether or not merging
    // each other head could progress consensus
    let proposalEndorsers = null;
    let supportEndorsers = null;
    if(head._lastSwitch && !_hasEndorsedSwitch(head)) {
      supportEndorsers = new Set(head._lastSwitch[endorsersKey]);
    }
    ...
    for(const [otherElector, otherHead] of heads) {
      ...
      // if `otherHead` has new endorsed switch or decision/recovery proposal,
      // then prioritize to merge it in
      if((!_hasEndorsedSwitch(mra) && _hasEndorsedSwitch(otherHead)) ||
        (!_hasEndorsedProposal(mra) && _hasEndorsedProposal(otherHead)) ||
      ...
      // merge it in
      if((!mra && otherHead._supporting) ||
        (mra && !_compareSupportSet(mra._supporting, otherHead._supporting))) {
        priorityPeers.add(elector);
        break;
      }

      // if `otherHead` has a switch that `elector` can endorse, then
      // prioritize `elector`
      if(otherHead._lastSwitch &&
        otherHead._lastSwitch[endorsersKey] &&
        !_hasEndorsedSwitch(otherHead) &&
        !otherHead._lastSwitch[endorsersKey].has(elector)) {
        priorityPeers.add(elector);
        break;
      }
      ...
      // if `elector` needs a switch endorsement and `otherElector` hasn't
      // provided it, track it to check for threshold later
      if(supportEndorsers && !supportEndorsers.has(otherElector) &&
        _hasElectorAncestor({
          event: otherHead,
          elector,
          ancestor: head._lastSwitch
        })) {
        supportEndorsers.add(otherElector);
      }
    ...
    // if support/proposal endorsers would reach threshold, then prioritize
    // `elector`
    const threshold = (recoverySet.size > 0 ?
      recoverySupermajority : supermajority);
    if((supportEndorsers && supportEndorsers.size >= supermajority) ||
      (proposalEndorsers && proposalEndorsers.size >= threshold)) {
      priorityPeers.add(elector);
    }
  }
  ...
  if(recoverySet.size > 0) {
    ...
    if(count <= r) {
      for(const recoveryElector of recoverySet) {
        priorityPeers.add(recoveryElector);
      }
    }
  }

  // Note: In non-recovery mode, the above algorithm must guarantee there will
  // always be > `f` electors in `priorityPeers` or else it is possible that
  // all prioritized peers will be failed nodes.
