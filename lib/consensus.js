  ...
  for(const x of allXs) {
    ...
    while(next.length > 0) {
      ...
      for(const event of current) {
        ...
        event.event.parentHash.forEach(hash => parentHashes.add(hash));
        if(event._parents) {
...
function _tally({
  ...
  // TODO: may need `f+1` based switches to only use endorsed precommits and
  //   support to prevent byzantine forks (may even need endorsements to
  //   do precommit unioning)

  If no precommit:
    1. if >= f+1 precommits for same set, support it (larger set wins tie)
      (this means, when tallying, keep track of precommit counts as well)
    2. if not, union support
  If have a precommit:
    1. choose support by unioning precommits
    2. check for a rejection...
      2.1. if support is not for the precommit, reject
      2.2. if >= f+1 support something other than the precommit and
           each of those sets is a larger set, reject
    3. if not rejected, see if endorse point has happened/passed
         if >= 2f+1 precommits for the same thing we support, decide.
         otherwise propagate endorse point forward
  If no existing precommit (or rejected it)
    1. if >= 2f+1 nodes support our next choice, precommit it
  */
  ...
  _.values(event._votes).forEach(e => {
    ...
    const endorsedPreCommit = e._preCommit && e._preCommit._endorsed;
    if(tallyResult) {
      ...
      tally.push({
        set: e._supporting,
        count: 1,
        preCommitCount: endorsedPreCommit ? 1 : 0
      });
    }
  });
  ...
  Note: Only union precommits when our tree parent has one and the other
    precommits are endorsed.
  ...
  */
  ...
  If no next choice has been set:
    If >= f+1 have precommitted for a set, choose the largest such set.
    And continue...
  */
  if(!nextChoice && tally.length > 0) {
    // sort tallies by descending precommit count and support set size
    tally.sort((a, b) => {
      const diff = b.preCommitCount - a.preCommitCount;
      if(diff !== 0) {
        return diff;
      }
      return b.set.length - a.set.length;
    });
    if(tally[0].preCommitCount >= (f + 1)) {
      nextChoice = tally[0];
    }
  }

  /*
  If next choice has still not been set:
    Set next choice to the union of all support (not precommits).
    And continue...
  */
  if(!nextChoice) {
    ...
    if(!nextChoice) {
      // create new choice
      nextChoice = {set: union, count: 0, preCommitCount: 0};
    }
  }
  ...
  */
  if(preCommit) {
    ...
    if(!reject) {
      // if `f+1` nodes support sets larger than existing precommit, reject
      let total = 0;
      for(const tallyResult of tally) {
        if(tallyResult.set.length > nextChoice.set.length) {
          total += tallyResult.count;
        }
      }
      reject = total >= (f + 1);
    }
    ...
    if(reject) {
      /*console.log('rejecting precommit for',
        preCommit._supporting.map(r=>r._generation));*/
      if(preCommit._endorsePoint) {
        preCommit._endorsePoint = null;
      }
      preCommit._endorsed = false;
      preCommit = null;
    }
  }

  if(preCommit && preCommit._endorsePoint === event) {
    // endorse point reached
    preCommit._endorsed = true;
  }

  if(preCommit && preCommit._endorsed) {
    /*
    Local support matches AND the precommit has been endorsed. So, if a
    supermajority of precommits support the same set, then consensus
    has been reached.
    Otherwise, continue on and hope the next event will decide...
    */
    if(nextChoice.preCommitCount >= supermajority) {
      // consensus reached
  ...
  */
  if(hasSupermajority && !preCommit) {
    ...
    // compute endorse point for the preCommit
    ...
    if(endorsePoint) {
      ...
      event._endorsePoint = endorsePoint[0].mergeEvent;
      /*console.log('marking endorse point event',
        event._endorsePoint._generation, 'for precommit', event._generation);*/
    } else {
      // FIXME: remove me
      /*console.log('no endorse point event yet for precommit',
        _getCreator(event), event._generation);*/
    }
  }
