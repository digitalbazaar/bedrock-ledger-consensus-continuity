...
api.findConsensus = ({
  ...
  const proof = _findMergeEventProof({
    ledgerNodeId, history, tails, blockHeight, electors, state,
    mode, logger
  });
...
function _findConsensusMergeEventProof({
  ...
  //let startTime = Date.now();
  //logger.verbose('Start sync _findConsensusMergeEventProof: _runExperiments');
  //console.log('Start sync _findConsensusMergeEventProof: _runExperiments');
  ...
  /*console.log('End sync _findConsensusMergeEventProof: _runExperiments', {
    duration: Date.now() - startTime
  });*/
  /*logger.verbose('End sync _findConsensusMergeEventProof: _runExperiments', {
    duration: Date.now() - startTime
  });*/
...
function _experiment({
  ...
  */
  ...
  /*logger.verbose('Continuity2017 _experiment finding support seen...',
    {ledgerNode: ledgerNodeId, eventHash: event.eventHash});*/
  // tally support seen
  /*const startTime = Date.now();
  logger.debug('Start sync _experiment tally support');*/
  const tally = new Map();
  const fPlusOne = f + 1;
  ...
  /*logger.debug('End sync _experiment tally support', {
    duration: Date.now() - startTime
  });*/
...
function _cacheSupport({state, event}) {
  ...
  if(event._c.support === false) {
    state.supportCache.set(memo.s, false);
    return;
  }
  const support = [];
