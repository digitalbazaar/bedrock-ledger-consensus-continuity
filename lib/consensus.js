    ...
    for(const event of current) {
      ...
      if(d) {
        ...
        if(electorSet.has(creator)) {
          ...
          // Y's generation
          const creatorYs = new Set(yByElector[creator]);
          let creatorY = event;
          while(creatorY) {
          ...
          // FIXME: ensure this check does not prevent detecting byzantine nodes
          if(creatorY && event._generation >= creatorY._generation) {
...
function _useMostRecentVotingEvent({
  ...
  if(elector in votes) {
    ...
    if(existing === false || votingEvent === false) {
      ...
      // FIXME: this check may not be sufficient to detect byzantine nodes...
      //   could it be that the generations are skewed so that when we update
      //   to use the most recent voting event we stay ahead of where the
      //   the generations clash, thereby preventing detection? if so, we need
      //   to do a better scan of the ancestry to make sure there is no fork
    ...
    for(const event of current) {
      if(!event._supporting) {
        ...
        for(const elector of electorSet) {
          // FIXME: if `event` has any voting event that has a tree ancestor
