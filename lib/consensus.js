  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      if(!event._treeParent) {
        ...
        if(isWitness) {
          mostRecentWitnessAncestors.set(creator, event._treeParent);
        }
      }
      if(!event._index) {
        event._index = {};
  ...
  while(next.length > 0) {
    ...
    for(const {treeDescendant, endorsers} of current) {
      ...
      for(const [witness, ancestorEvent] of
        treeDescendant._index.mostRecentWitnessAncestors) {
        ...
        const xAncestor =
          ancestorEvent._index.mostRecentWitnessAncestors.get(xCreator);
...
function _computeMostRecentVotes({event}) {
  event._votes = new Map(event._index.mostRecentWitnessAncestors);
  // event is a `y`, always use itself when voting
  if(event._y) {
    event._votes.set(_getCreator(event), event);
  }
}
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      if((event._treeParent && event._treeParent._votes) || event._y) {
        ...
        let defer = false;
        for(const e of event._votes.values()) {
...
function _computeSupport({
  ...
  */
  ...
  if(event._votes.get(creator) === false) {
  ...
  for(const e of event._votes.values()) {
    ...
    let tallyResult = tally.get(e._supporting);
    if(!tallyResult) {
  ...
  console.log('------------------');
  for(const [k, v] of event._votes) {
    if(v._supporting) {
      console.log('|');
      console.log('|-elector:', k.substr(-5));
      console.log('  generation:', v._generation);
      v._supporting.forEach(r => {
        console.log(
  ...
  */
  ...
  if(fPlusOneSupport.length > 0) {
    ...
    const union = new Set();
    for(const votingEvent of event._votes.values()) {
      if(votingEvent) {
  ...
  /*
    ...
    next choice count.
  */
  const previousVoteEvent = event._votes.get(creator);
  ...
  */
  ...
  if(proposal) {
    ...
    const reject = proposal._supporting !== nextChoice.set;
    if(reject) {
  ...
  */
  ...
  event._votes.set(creator, event);
  ...
  for(const [elector, head] of heads) {
    ...
    for(const [otherElector, otherHead] of heads) {
      ...
      const mra = head._index && head._index.mostRecentWitnessAncestors &&
        head._index.mostRecentWitnessAncestors.get(otherElector);
      if(!mra) {
...
function _hasElectorAncestor({event, elector, ancestor}) {
  const mra = event._parents.length > 0 &&
    event._index.mostRecentWitnessAncestors.get(elector);
