 ...
 */
api.findConsensus = ({
  ledgerNodeId, history, blockHeight, electors, state = null,
  logger = noopLogger
}) => {
  ...
  const tails = _getWitnessBranches({history, witnesses: electors, state});
  ...
  const proof = _findMergeEventProof({
    ledgerNodeId, history, tails, blockHeight, electors, state, logger
  });
  ...
  if(state) {
    ...
    // to be reclaimed; do not preserve after `_findMergeEventProof`
    state.eventMap = null;
  }
  ...
  const events = [];
  for(const p of proof.consensus) {
    events.push(p.x);
  }
...
function _getAncestorHashes({events, ledgerNodeId}) {
          ...
          if(!hashes.has(parentHash) && !parentHashes.has(parentHash) &&
            ...
            descendants.push({
              ...
              _c: {
                ...
                // sort regular events into the same generation as the
                // merge event parents of the event
  ...
  order.sort(function compareEvents(a, b) {
    let diff = 0;

    if(_getCreator(a) === _getCreator(b)) {
      /* Note: We do not need to consider the fork case here; if a byz node
      forks and has multiple events assigned to the same generation, then
      they will still be sorted topologically correctly, and then, by
      generation whereby they can become mixed across forks potentially causing
      some operations to become ineffectual, discouraging this bad behavior).
      For generational ties, they will still be deterministically broken using
      hashes. */
...
function _findMergeEventProof({
  ledgerNodeId, history, tails, blockHeight, electors, state = null,
  logger = noopLogger
}) {
  //let startTime = Date.now();
  ...
  const candidates = _findConsensusCandidates({
    ledgerNodeId, tails, blockHeight, witnesses: electors, logger
  });
  ...
  const ys = _findConsensusMergeEvents({
    ledgerNodeId, yByWitness, blockHeight, witnesses: electors, state, logger
  });
...
function _findConsensusCandidates({
  ledgerNodeId, tails, blockHeight, witnesses, logger = noopLogger
}) {
  ...
  const witnessesWithTails = Object.keys(Object.fromEntries(tails));
...
function _getPriorityPeers({
  ledgerNodeId, history, tails, blockHeight, witnesses, logger
}) {
  /* Algorithm:
