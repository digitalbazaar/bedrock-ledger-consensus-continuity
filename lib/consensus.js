...
function _computeSupport({
  ...
  */
  ...
  2. A proposal is endorsed at its endorsement event if the endorsement event
  ...
  */
  ...
  if(supermajoritySupport) {
    ...
    for(const elector in event._votes) {
      ...
      if(votingEvent) {
        // if `votingEvent` is initial Y, it supports itself
        if(!votingEvent._supporting) {
          union.add(votingEvent);
          continue;
        }
        // `votingEvent` has some computed support
