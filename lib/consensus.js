...
function _findMergeEventProofCandidates({
  ...
  _computeMostRecentWitnessAncestors({ledgerNodeId, xByElector});
...
function _computeMostRecentWitnessAncestors({ledgerNodeId, xByElector}) {
  ...
  const xElectorSet = new Set(Object.keys(xByElector));
  let next = [].concat(...[...Object.values(xByElector)]);
  while(next.length > 0) {
    const current = next;
    next = [];
    for(const event of current) {
      ...
      const isWitness = xElectorSet.has(creator);
      ...
      for(const parent of event._parents) {
        ...
        if(xElectorSet.has(parentCreator)) {
...
function _findEndorsementMergeEvent({
  ...
  if(supermajority === 1) {
    ...
    return [{mergeEvent: x, descendants: _copyMapOfSets(descendants)}];
  }
  ...
  let next = x._treeChildren.map(treeDescendant => ({
    ...
    descendants: x._treeChildren.length === 1 ?
      descendants : _copyMapOfSets(descendants)
  }));
  while(next.length > 0) {
    const current = next;
    next = [];
...
// FIXME: use to replace old version
function _findEndorsementMergeEvent2({
  ledgerNodeId, x, electors, supermajority,
  descendants = new Map(), ancestryMap = _buildAncestryMap(x),
  type
}) {
  ...
  if(supermajority === 1) {
    ...
    if(type === 'elector') {
      ...
      x._recoveryEndorsers = _endorsers;
    }
    return [{mergeEvent: x, descendants: _copyMapOfSets(descendants)}];
  }
  ...
  // FIXME: if recovery mode is not removed, fix this up
 ...
 * @param diff a Set to populate with new descendants found.
 */
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      for(const parent of event._parents) {
        ...
        descendants.set(parent, new Set([event]));
        // if(diff) {
        //   diff.add(event);
        // }
        // next.push(parent);

        // if parent index is not available, it must be recursed
        if(true || !parent._index) {
          // only happens with mode = `firstWithConsensusProof`
          if(diff) {
            diff.add(event);
          }
          next.push(parent);
        ...
        // if(diff) {
        //   diff.add(event);
        //   for(const creator in parent._index.mostRecentAncestors) {
        //     //console.log('adding',
        //     //  parent._index.mostRecentAncestors[creator].eventHash);
        //     const ancestorEvent = parent._index.mostRecentAncestors[creator];
        //     if(ancestorEvent) {
        //       diff.add(ancestorEvent);
        //     }
        //   }
        // }
        //
        // // use parent's already calculated information
        // //console.log('adding', event.eventHash);
        // for(const elector in parent._index.electorPedigree) {
        //   const map = parent._index.electorPedigree[elector];
        //   for(const [k, v] of map) {
        //     const existing = descendants.get(k);
        //     if(!existing) {
        //       descendants.set(k, new Set(v));
        //       for(const d of v) {
        //         if(diff) {
        //           diff.add(d);
        //         }
        //       }
        //     } else {
        //       // FIXME: this has to be here to make the `diff` work, why?...
        //       for(const d of v) {
        //         existing.add(d);
        //         if(diff) {
        //           diff.add(d);
...
// shallow copy a map of sets (sets will be shallow copied)
function _copyMapOfSets(mapOfSets) {
  const copy = new Map();
  for(const [k, v] of mapOfSets) {
    copy[k] = new Set(v);
  }
  return copy;
}
...
function _computeMostRecentAncestors({
  ...
  for(const ancestorEvent of diff) {
    if(ancestorEvent === event) {
      // skip self, if included in `diff`
      continue;
    }
...
function _computeInfluentialElectorPedigree({
  ...
  if(parentElectorPedigree) {
    // FIXME: reuse of this is very important, much faster
    if(/*false &&*/event._treeParent._treeChildren.length === 1) {
      ...
      for(const key in parentElectorPedigree) {
        event._index.electorPedigree[key] =
          _copyMapOfSets(parentElectorPedigree[key]);
      }
    }
  } else {
  ...
  for(const elector of influentialElectorSet) {
    ...
    for(const ancestorEvent of mergeEvents) {
      ...
      // // add most recent ancestors from each parent to diff
      // for(const parent of event._parents) {
      //   if(!(parent._index && parent._index.mostRecentAncestors)) {
      //     continue;
      //   }
      //   //   for(const creator in parent._index.mostRecentAncestors) {
      //   //     //console.log('adding',
      //   //     //  parent._index.mostRecentAncestors[creator].eventHash);
      //   //     diff.add(parent._index.mostRecentAncestors[creator]);
      //   //   }
      //
      // }
      _findDescendantsInPath({
...
function _tallyBranches({
  ...
  */
  ...
  // compute most recent ancestors
  // FIXME: do we want to do this
  /*let next = tailsArray.slice();
  while(next.length > 0) {
    const current = next;
    next = [];
    for(const event of current) {
      // defer if any ancestors hasn't computed their most recent ancestors yet
      let defer = false;
      for(const parent of event._parents) {
        if(!(parent._index && parent._index.mostRecentAncestors)) {
          next.push(parent);
      ...
      if(!(event._index && event._index.mostRecentAncestors)) {
        ...
        // add tree children, if any, for influential electors
        if(event._treeChildren &&
          influentialElectorSet.has(_getCreator(event))) {
          next.push(...event._treeChildren);
        }
      }
    }
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      if((event._treeParent && event._treeParent._recovery) ||
        ...
        const recoveryEvents = Object.values(
          event._index.mostRecentWitnessAncestors)
...
function _computeEndorsement({
  ...
  const endorsement = _findEndorsementMergeEvent2({
