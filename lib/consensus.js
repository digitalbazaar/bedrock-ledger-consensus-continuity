...
api.findConsensus = ({
  ...
  // Object.keys(tails).forEach(k => console.log({
  //   electorId: k,
  //   tail: tails[k][0],
  //   ledgerNodeId,
  //   event: tails[k][0].event
  // }));
  // console.log({tails, ledgerNodeId});
...
function _findMergeEventProof({
  ...
  // console.log(candidates.xByElector);
...
function _computeInfluentialElectorPedigree({
  ...
  if(parentElectorPedigree) {
    if(event._treeParent._treeChildren.length === 1) {
      ...
      for(const key in event._index.electorPedigree) {
  ...
  for(const elector of influentialElectorSet) {
    ...
    for(const ancestorEvent of mergeEvents) {
      ...
      if(!event._index.electorPedigree[elector] &&
        ancestorEvent.eventHash in descendants) {
        // no descendants map for this elector yet, add the map and add
        // ancestor event to diff
        event._index.electorPedigree[elector] = descendants;
