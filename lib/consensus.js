...
function _updateRecoveryInfo({
  ledgerNodeId, electors, electorSet, descendants, recoverySet, event,
  logger = {debug: () => {}, verbose: () => {}}
}) {
  ...
  if(recovery.proposal) {
    ...
    console.log('EXISTING RECOVERY PROPOSAL FOUND FOR', _getCreator(event));
    return;
  }
...
function _computeRecovery({
  ...
  // threshold
  console.log('COMPUTE RECOVERY FOR', _getCreator(event),
    'PROPOSAL', event._recovery.proposal, 'NO PROGRESS COUNTER',
    event._recovery.noProgressCounter,
    'THRESHOLD', recoveryGenerationThreshold);
