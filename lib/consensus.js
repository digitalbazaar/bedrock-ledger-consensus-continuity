...
function _tally({
  ...
  If no precommit:
    1. if >= f+1 precommits for same set and are endorsed, support it
      (larger set wins tie)
    2. if not, union support
  If have a precommit:
    1. choose support by unioning *endorsed* precommits
    ...
    3. if not rejected, see if endorse point has happened/passed
         if >= 2f+1 precommits (endorsement not required) for the same thing
         we support, decide.
  If no existing precommit (or rejected it)
    1. if >= 2f+1 nodes support our next choice (endorsement not required),
       precommit it
  */
  ...
  _.values(event._votes).forEach(e => {
    ...
    const endorsed = _hasEndorsedSupport(e);
  ...
  /* Find the union of all ancestral current (and endorsed) precommit event
  support sets, as long as we have our own precommit:
    Then set next choice to endorsed precommit union.
    And continue...
  ...
  */
  ...
  If no next choice has been set:
    If >= f+1 have precommitted for a set and been endorsed, choose the
    largest such set.
    And continue...
  */
  if(!nextChoice && tally.length > 0) {
    // sort tallies by descending endorsed precommit count and support set size
    tally.sort((a, b) => {
      const diff = b.endorsedPreCommitCount - a.endorsedPreCommitCount;
      if(diff !== 0) {
        return diff;
      }
      return b.set.length - a.set.length;
    });
    if(tally[0].endorsedPreCommitCount >= (f + 1)) {
      nextChoice = tally[0];
    }
  }
  ...
  If next choice has still not been set:
    Set next choice to the union of all support (precommit and endorsement
    not required).
    And continue...
  */
  if(!nextChoice) {
  ...
  */
  ...
  if(previousChoice !== nextChoice) {
    // set first time support
    event._firstSupport = event;
    ...
    if(endorsePoint) {
      ...
      event._endorser = endorsePoint[0].mergeEvent;
      event._endorser._endorses = event;
      /*console.log(
  ...
  if(event._endorses) {
    ...
    event._endorses._endorsed = true;
  }
  ...
  */
  ...
  if(preCommit && preCommit._endorsed) {
    ...
    supermajority of precommits support the same set (endorsement of the
    other precommits is not required), then consensus has been reached.
    Otherwise, continue on and hope the next event will decide...
    */
    //console.log('endorsed precommit reached at', event._generation);
    //console.log('precommit count', nextChoice.preCommitCount);
  ...
  */
  ...
  // propagate node's first time support
  if(!event._firstSupport) {
    event._firstSupport = event._treeParent._firstSupport;
  }
...
function _hasEndorsedSupport(event) {
  const firstSupport = event._firstSupport;
  if(firstSupport && firstSupport._endorsed) {
    return event._generation >= firstSupport._endorser._generation;
  }
  return false;
}
