...
function _tally({
  ...
  */
  if(existingPreCommit) {
    ...
    if(!_compareSupportSet(existingPreCommit._supporting, nextChoice.set)) {
      ...
      precommit. So, if it has supermajority support *and* that supermajority
      comes from endorsing nodes, then consensus has been reached.
      Otherwise, existing precommit is rejected and continue...
      */
      if(hasSupermajority) {
        // must ensure that endorsing nodes comprise a supermajority, so go
        // through each vote, check if it has a vote from the confirm point
        // event's creator that has a generation that is >= the precommit's,
        // that descends from it (protect against forking), and that is
        // supporting the precommit
        let total = 0;
        for(const elector in event._votes) {
          const votingEvent = event._votes[elector];
          const ancestor = votingEvent._votes[creator];
          if(ancestor &&
            ancestor._generation >= existingPreCommit._generation) {
            // if voting event's ancestor descends from (or is) the precommit
            // and the voting event supports the precommit, increment total
            if(_descendsFrom(ancestor, existingPreCommit) &&
              _compareSupportSet(
                existingPreCommit._supporting, votingEvent._supporting)) {
              total++;
            }
          }
        }
  ...
  */
  if(hasSupermajority && !existingPreCommit) {
    ...
    if(confirmPoint) {
      // `confirmPoint` will be an array as byzantine nodes may fork, but
      ...
      event._confirmPoint = confirmPoint[0].mergeEvent;
