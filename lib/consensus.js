...
api.findConsensus = ({
  ...
  console.log('TTTTTTTTTTTTTTTTTTTTTTTtt TAILS', tails);
...
function _findMergeEventProof({
  ...
  if(recoveryElectors.length > 0) {
    ...
    for(const elector of recoveryElectors) {
      ...
      if(electorTails) {
        console.log('HEAD_GENERATION THRESHOLD',
          electorTails[0]._headGeneration, recoveryGenerationThreshold);
      }
    }
    const threshold = recoveryElectors.length - recoveryDecisionThreshold + 1;
    canEnterRecoveryMode = (count >= threshold);
    console.log('RECOVERY MODE POSSIBLE', canEnterRecoveryMode);
  }
...
function _updateRecoveryInfo({
  ledgerNodeId, electors, electorSet, descendants, recoverySet, recovery,
  logger = {debug: () => {}, verbose: () => {}}
}) {
  ...
  for(const trackingEvent of descendants) {
    ...
    console.log('===============ELECTOR', elector, 'GENERATION',
      trackingEvent._generation);
  ...
  if(newElectorsSeen.size >= threshold) {
    console.log('PROGRESS MADE ALL AT ONCE, THRESHOLD NEWLY_SEEN',
      threshold, newElectorsSeen.size);
  ...
  for(const elector of newElectorsSeen) {
    ...
    if(recovery.electorsSeen.size > beforeSize &&
      ...
      console.log('PROGRESS MADE, THRESHOLD ADDED', threshold,
        recovery.electorsSeen.size - beforeSize);
      recovery.noProgressCounter = 0;
      recovery.electorsSeen.clear();
      return;
    }
  ...
  console.log('NEW ELECTORS SEEN', recovery.electorsSeen.size - originalSize);
    ...
    for(const event of current) {
      ...
      if((event._treeParent && event._treeParent._recovery) ||
        ...
        if(!event._recovery) {
          ...
          // FIXME: do we need to copy or can we reuse?
          const {noProgressCounter, electorsSeen, decision, skip} =
          ...
          if(!event._recovery.skip) {
            ...
            _updateRecoveryInfo({
              ledgerNodeId, electors, electorSet, descendants: diff,
              recoverySet, recovery: event._recovery, logger});
          }
        }
...
function _computeRecovery({
  ...
  // threshold
  console.log('COMPUTE RECOVERY, PROPOSAL', event._recovery.proposal, 'NO PROGRESS COUNTER',
    event._recovery.noProgressCounter, 'THRESHOLD', recoveryGenerationThreshold);
  ...
  console.log('************** PROPOSALS', proposals, 'REQUIRED_PROPOSALS',
    requiredProposals);
...
function _computeSupport({
  ...
  */
  if(proposal) {
    /*console.log('EXISTING PROPOSAL FOR',
  ...
  if(proposal) {
    ...
    */
    ...
    if(nextChoice.endorsedProposalCount >= supermajority) {
      console.log('*************ENDORSED PROPOSAL COUNT', nextChoice.endorsedCount);
      ...
      if(recoveryElectors.length === 0) {
        ...
        const recoverySet = new Set(recoveryElectors.map(e => e.id));
        // console.log('RECOVERYSET', recoverySet);
        ...
        // console.log(`${Date.now()} DECISION COUNT`, count);
