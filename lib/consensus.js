    ...
    for(const event of current) {
      if(!event._supporting) {
        ...
        if(!event._yDescendants) {
          ...
          // get unique diff and exclude self (do not use own event as
          // voting event, we must calculate support first -- only initial
          // y's will use themselves and they are already set in `event_votes`
