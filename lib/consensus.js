...
function _computeInfluentialElectorPedigree({
  ...
  for(const elector of influentialElectorSet) {
    ...
    for(const ancestorEvent of mergeEvents) {
      ...
      if(ancestorEvent.eventHash in descendants) {
        // ensure that the `ancestorEvent` itself is added to the diff; this
        // is suboptimal when used for computing most recent ancestors, but
        // does not create corruption; however, it's currently the only way to
        // ensure `ancestorEvent` will be included
