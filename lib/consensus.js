    ...
    for(const event of current) {
      ...
      if(event._treeParent._recovery || event._recovery) {
        if(!event._recovery) {
          ...
          // FIXME: do we need to copy or can we reuse?
          const {noProgressCounter, electorsSeen, decision} =
...
function _computeRecovery({
  ...
  for(const trackingEvent of trackingEvents) {
    ...
    if(!(trackingEvent._votes && trackingEvent._votes[creator] === false) &&
      trackingEvent._recovery.proposal) {
      proposals++;
    }
  }
  ...
  if(proposal) {
    ...
    */
    ...
    if(nextChoice.endorsedProposalCount >= supermajority) {
      ...
      if(recoveryElectors.length === 0) {
      ...
      } else if(event._recovery && !event._recovery.proposal) {
        ...
        let count = 0;
        const {tracking} = event._recovery;
        for(const elector in tracking) {
          const {proposal, decision} = tracking[elector]._recovery;
          const isByzantine = event._votes[elector] === false;
          if(!isByzantine && !proposal && decision) {
            count++;
          }
        }
