...
function _findMergeEventProof({
  ...
  const {xByElector, rByElector} = candidates;
 ...
 *   `rByElectors`; in `xByElectors`, each elector maps to the first event(s)
 *   on that elector's branch. In `yByElectors`, each elector maps to merge
 *   event(s) Y on its branch that prove a merge event X on its branch has been
 *   endorsed by a super majority of electors; in `rByElectors` each recovery
 *   elector maps to the next event for which recovery information needs to be
 *   computed.
 */
function _findMergeEventProofCandidates({
  ...
  for(const elector of electorsWithTails) {
    ...
    // include each merge event in initial descendants map; it is used to halt
    ...
    for(const mergeEvent of mergeEvents) {
      ...
      // FIXME: remove me, once determined it is unnecessary and adjust comment
      // above loop and initial descendants map
...
// FIXME: remove me if not needed any more
...
function _getCreator(event) {
  ...
  if(!creator) {
    // FIXME: never should happen as no regular events are present in the
    // history; remove this code after confirmation and don't use `_.get`
    // above anymore
