  ...
  if(!history.linked) {
    ...
    for(const e of history.events) {
      for(const parentHash of e.event.parentHash) {
        if(!history.eventMap[parentHash]) {
          continue;
        }
...
function _findMergeEventProof({
  ...
  const ys = _findConsensusMergeEventProof(
    {ledgerNodeId, xByElector: candidates.xByElector,
      yByElector: yCandidatesByElector, blockHeight, electors, logger});
...
function _findMergeEventProofCandidates({
  ...
  // FIXME: need to allow multiple events for each elector, in the event that
  ...
  for(const elector of electorsWithTails) {
    ...
    // FIXME: must not skip electors with multiple tails detected (byzantine),
    //   as not every valid node will see the fork and we must calculate
    //   their support properly
    ...
    const result = _findEndorsementMergeEvent(
      {ledgerNodeId, x: electorTails[0], electors, supermajority, descendants});*/
    if(result) {
...
function _findEndorsementMergeEvent(
  ...
  while(treeDescendant) {
    ...
    // FIXME: this function needs to be modified to allow forked endorsement
    //   events to be returned... i.e. when computing `y` events, we must
    //   return all possible `y` events (byzantine nodes may fork) as some
    //   nodes may only see one side of the fork... and we need to properly
    //   calculate their support from *their* perspective, not our own...
    //   alternatively, `y`s could be detected as we calculate support (at
    //   every merge event), but this would likely be slower than our current
    //   method of making sure we don't bother computing support until we
    //   have a sufficient number of `y`s, etc.
...
function _updateToMostRecentVotes({
  ledgerNodeId, y, yByElector, descendants, electorSet, votes,
  logger = {debug: () => {}, verbose: () => {}}
}) {
  let next = [y];
  while(next.length > 0) {
  ...
  */
  ...
  while(next.length > 0) {
    ...
    for(let event of current) {
      if(!event._supporting) {
        ...
        for(const elector of electorSet) {
          ...
          _updateToMostRecentVotes(
            {ledgerNodeId, y, yByElector, descendants, electorSet,
      ...
      const result = _tally(
        {ledgerNodeId, event, yByElector, blockHeight, electors});
...
function _tally({
  ...
  /*logger.verbose('Continuity2017 _tally tally',
    {ledgerNode: ledgerNodeId, tally});*/
  // TODO: remove me
  //tally.sort((a, b) => b.count - a.count);
  /*console.log('VOTE TALLY', ledgerNodeId, tally.map(t => ({
    count: t.count,
  ...
  */
  ...
  If no next choice has been set:
    Then set next choice to the union of all support (not precommits).
    And continue...
  */
  if(!nextChoice) {
  ...
  */
  if(hasSupermajority && !existingPreCommit) {
    ...
    const confirmPoint = _findEndorsementMergeEvent(
