...
function _computeMostRecentVotes({event, yElectorSet, yByElector}) {
  ...
  for(const yElector of yElectorSet) {
    const ancestorEvent = event._index.mostRecentWitnessAncestors.get(yElector);
...
function _computeSupport({
  ...
  */
  ...
  for(const elector in event._votes) {
    ...
    let tallyResult = _findSetInTally(tally, e._supporting);
    if(!tallyResult) {
  ...
  */
  ...
  if(fPlusOneSupport.size > 0) {
    ...
    if(fPlusOneSupport.size === 1) {
      ...
      nextChoice = _findSetInTally(tally, union);
      if(!nextChoice) {
        // create new choice
        nextChoice = {
          set: union,
...
function _findSetInTally(tally, set) {
  if(!set) {
    return false;
  }
  for(const tallyResult of tally) {
    if(_compareSupportSet(tallyResult.set, set)) {
      return tallyResult;
    }
  }
  return false;
...
function _compareSupportSet(set1, set2) {
  ...
  // can compare events directly as hashes are assumed unique
  return _.differenceWith(set1, set2).length === 0;
}
