  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      for(const parent of event._parents) {
        ...
        //});
        const d = descendants.get(parent.eventHash);
        if(d) {
          if(!d.includes(event)) {
            d.push(event);
            if(diff) {
        ...
        descendants.set(parent.eventHash, [event]);
        if(diff) {
          diff.push(event);
        }
        next.push(parent);
...
function _flattenDescendants({ledgerNodeId, x, descendants}) {
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      //});
      const d = descendants.get(event.eventHash);
      if(d) {
...
function _computeInfluentialElectorPedigree({
  ...
  for(const elector of influentialElectorSet) {
    ...
    for(const ancestorEvent of mergeEvents) {
      ...
      const addedToDiffPreviously = descendants.has(ancestorEvent.eventHash);
      _findDescendantsInPath({
        ledgerNodeId,
        x: ancestorEvent,
        y: event,
      ...
      if(!addedToDiffPreviously && descendants.has(ancestorEvent.eventHash)) {
