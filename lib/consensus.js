...
api.find = async ({
  ...
  const filterByHashSet = h => hashSet.has(h);
  const blockOrdering = result.eventHashes.blockOrdering.filter(
    filterByHashSet);
  const gossipOrdering = result.eventHashes.gossipOrdering.filter(
    filterByHashSet);
  return {
    consensus: true,
    consensusProofHash: result.consensusProofHashes,
    creators: result.creators,
    eventHash: blockOrdering,
    gossipOrdering,
