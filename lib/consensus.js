 ...
 */
api.findConsensus = ({
  ledgerNodeId, history, blockHeight, electors, recoveryElectors = [],
...
function _tallyBranches({
  ...
  */
  // TODO: RECOVERY MODE: rename `endorsePoint` => `endorsement`
  const recoverySet = new Set(recoveryElectors.map(e => e.id));
...
function _computeRecovery({
  ...
  if(proposals === requiredProposals) {
    ...
    const endorsement = _findEndorsementMergeEvent(
      {ledgerNodeId, x: event, recoveryElectors, requiredProposals,
