...
function _tally({
  ...
  If precommit:
    1. If >= f+1 precommits for a smaller set, support it.
    2. Otherwise, union *endorsed* precommits.
  If no precommit:
    1. If >= f+1 endorsed precommits for the same set, support it.
       (larger set wins tie).
       TODO: optimize for total of f+1 rather than f+1 for the same set
  ...
  */
  if(preCommit) {
    ...
    if(!nextChoice) {
      // TODO: optimize to find largest endorsed precommitted faster
    ...
    */
    if(tally.length > 0) {
      // sort tallies by descending endorsed precommit count and support set
      // size
      tally.sort((a, b) => {
      ...
      // TODO: uncomment to optimize
      // TODO: determine if need to remove self from count
      /*const totalEndorsedCount = tally.reduce((aggregator, tallyResult) => {
        return aggregator + tallyResult.endorsedPreCommitCount;
      }, 0);
      if(totalEndorsedCount >= (f + 1)) {
        nextChoice = tally[0];
      }*/
      // TODO: use above and remove to optimize
