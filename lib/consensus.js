/*
 * Web Ledger Continuity2017 Consensus Method.
 *
...
api._test = callback => {
  const session = brLedger.consensus.createSession('consensus', 1000);
...
// TODO: register job that will run `runConsensusJob` on interval

// TODO: expose consensus job function for testing

// TODO: run every X interval
function runConsensusJob(job, callback) {
...
function _sync(ledgerNode, session, callback) {
  ...
  async.auto({
    voter: callback => api._storage.voters.get(
      {ledgerNodeId: ledgerNode.id}, callback),
...
function _gossipWith(ledgerNode, voter, blockId, peer, callback) {
  ...
  _getBlockStatus(blockId, peer, (err, status) => {
    ...
    if(status.phase === 'gossip') {
      ...
      // {
      //   events: [<all events known that `peer` doesn't know>]
      // }
