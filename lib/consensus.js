...
function _tally({
  ...
  If precommit:
    1. If >= f+1 *other* precommits (endorsed or not), support the largest.
    2. Otherwise, union *endorsed* precommits.
       Note: The endorsement requirement here in `2` may not actually be
          necessary because it is not used in the proofs, but double check
          that before considering an optimization here.
          TODO: ^potential optimization w/o impacting outcome of consensus,
          would just speed things up in some cases if correct.
  If no precommit:
  ...
  */
  if(preCommit) {
    ...
    // determine if there are f+1 precommits *other* than `preCommit`
    const total = tally
      .filter(tallyResult => tallyResult.preCommitCount > 0)
      .reduce((aggregator, tallyResult) => {
        // TODO: optimize by filtering tally result out prior to calling reduce
        if(_compareSupportSet(preCommit._supporting, tallyResult.set)) {
          return aggregator;
        }
