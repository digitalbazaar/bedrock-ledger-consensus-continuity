...
function _computeRecovery({
  ...
  2. Iterate over most recent ancestor events from all electors and
     tally decision proposals, recovery proposals, endorsed recovery proposals,
     and "met no progress threshold"s from most recent ancestors, using `event`
  ...
  3. If event's branch has an existing recovery proposal decide if it should
     ...
     recovery proposal and endorsed recovery proposal counts if necessary.
  4. If a recovery proposal endorse point has been reached; mark any matching
     ...
     count if necessary.
  5. If the event's branch has no recovery proposal, no decision proposal,
     the "met no progress threshold" tally is >= `2r+1`, and there
     are < `f+1` decision proposals, then create a recovery proposal on the
     event, adjust the recovery proposal count, and compute its future endorse
     point.
  6. If >= `2r+1` recovery proposals and >= `r+1` are endorsed, enter recovery
     mode.
  */
  ...
  */
  ...
  5. If the event's branch has no recovery proposal, no decision proposal,
     the "met no progress threshold" tally is >= `2r+1`, and there
     are < `f+1` decision proposals, then create a recovery proposal on the
     event, adjust recovery proposal count, and compute its future endorse
     point.
  */
  ...
  */
  ...
  if(recoveryProposals >= recoverySupermajority &&
    endorsedRecoveryProposals >= (r + 1)) {
    // enter recovery mode
...
function _computeSupport({
  ...
  Rejecting proposals/Deciding:
    ...
    2. If >= `2f+1` proposals and >= `f+1` are endorsed, decide.
  ...
  */
  ...
  /*
  If >= `2f+1` proposals and >= `f+1` are endorsed, decide; consensus has
  been reached. Otherwise, continue on and hope the next event will decide...
  */
  if(nextChoice.proposalCount >= supermajority &&
    nextChoice.endorsedProposalCount >= (f + 1)) {
