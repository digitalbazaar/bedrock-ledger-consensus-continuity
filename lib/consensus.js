...
function _getPriorityPeers({
  ...
  */
  ...
  for(const elector in tails) {
    const electorTails = tails[elector];
    if(electorTails.length > 0) {
      // byzantine node, do not tally
      continue;
    }
    const [tail] = electorTails;
    if(tail._head.length > 0) {
      // byzantine node, do not tally
      continue;
    }
    ...
    // got nothing! ... only to be used when there's no other differentiator
  ...
  if(endorsedProposals > 0) {
    ...
    return nonByzantinePeers;
  }

  // filter via recovery set if size > 0
