...
function _computeSupport({
  ...
  console.log('COMPUTESUPPORTELECTORS', electors);
  console.log('COMPUTESUPPORTRECOVERYELECTORS', recoveryElectors);
  /* Choosing support:
  ...
  if(proposal) {
    ...
    */
    ...
    if(nextChoice.endorsedProposalCount >= supermajority) {
      ...
      console.log('$$$$$$$$$$RECOVERYELECTORS.LENGTH', recoveryElectors.length);
      ...
      if(recoveryElectors.length === 0) {
        ...
        console.log('EVENT._VOTES', Object.keys(event._votes));
        for(const elector in event._votes) {
          ...
          if(votingEvent && recoverySet.has(elector)) {
            console.log('votingEvent._recovery', votingEvent._recovery);
        ...
        console.log(`${Date.now()} DECISION COUNT / THRESHOLD`, count, recoveryDecisionThreshold);
