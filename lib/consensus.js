...
function _getAncestorHashes({events, ledgerNodeId}) {
  ...
  order.sort((a, b) => {
    ...
    if(_getCreator(a) === _getCreator(b)) {
      // TODO: check if there is no longer a need for `|| 0`
      diff = (a._c.generation || 0) - (b._c.generation || 0);
    }
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      for(const parent of event._c.parents) {
        ...
        for(const [elector, ancestorEvent] of parentMRWAs) {
          _useMostRecentAncestorEvent({
      ...
      if(event._c.treeChildren.length > 0 && isWitness) {
        next.push(...event._c.treeChildren);
      }
    }
  }
...
function _useMostRecentAncestorEvent({
  ...
  const existing = index.get(elector);
  if(existing === undefined) {
    ...
    index.set(elector, ancestorEvent);
    return;
  }
  ...
  index.set(elector, younger);
}

function _computeMostRecentVotes({event}) {
  event._c.votes = new Map(event._c.mostRecentWitnessAncestors);
  // event is a `y`, always use itself when voting
  if(event._c.y) {
    event._c.votes.set(_getCreator(event), event);
  }
}
  ...
  */
  ...
  while(next.length > 0) {
    ...
    for(const event of current) {
      ...
      // it is a `y` by virtue of having a `_c.y` property), then determine its
      // voting ancestry
      if((event._c.treeParent && event._c.treeParent._c.votes) || event._c.y) {
        if(!event._c.votes) {
          _computeMostRecentVotes({event});
        }
        // if some voting ancestors haven't computed support yet, defer
        let defer = false;
        for(const e of event._c.votes.values()) {
          if(e && e !== event && e._c.supporting === null) {
            next.push(event);
            defer = true;
            break;
          }
      ...
      if(event._c.votes) {
        ...
        const result = _computeSupport(
          {ledgerNodeId, event, blockHeight, electors, supportSets, logger});
...
function _computeSupport({
  ledgerNodeId, event, blockHeight, electors, supportSets, logger = noopLogger
}) {
  // support already chosen for event
  if(event._c.supporting !== null) {
    return null;
  }
  ...
  */
  ...
  if(event._c.votes.get(creator) === false) {
    event._c.supporting = false;
    return null;
  }
  ...
  for(const e of event._c.votes.values()) {
    ...
    let tallyResult = tally.get(e._c.supporting);
    if(!tallyResult) {
  ...
  console.log('votes received at generation', event._c.generation);
  console.log('by experimenter', creator.substr(-5));
  console.log('------------------');
  for(const [k, v] of event._c.votes) {
    if(v._c.supporting) {
      console.log('|');
      console.log('|-elector:', k.substr(-5));
      console.log('  generation:', v._c.generation);
      v._c.supporting.forEach(r => {
        console.log(
  ...
  */
  ...
  if(fPlusOneSupport.length > 0) {
    ...
    const union = new Set();
    for(const votingEvent of event._c.votes.values()) {
      if(votingEvent) {
        // if `votingEvent` is itself, include it in union as initial `y`
        if(votingEvent === event) {
          union.add(votingEvent);
          continue;
        }
        // add computed support from `votingEvent`
        for(const e of votingEvent._c.supporting) {
          union.add(e);
        }
      }
    }
  ...
  /*
    ...
    next choice count.
  */
  const previousVoteEvent = event._c.votes.get(creator);
  const switched = !(previousVoteEvent && previousVoteEvent._c.supporting &&
    nextChoice.set === previousVoteEvent._c.supporting);
  if(switched) {
  ...
  */
  ...
  if(proposal) {
    ...
    const reject = proposal._c.supporting !== nextChoice.set;
    if(reject) {
  ...
  */
  ...
  // support next choice
  event._c.supporting = nextChoice.set;
  event._c.votes.set(creator, event);
...
function _computeProposalEndorsement({
  ...
  if(event._c.proposalEndorsement) {
    // already computed
    return;
  }
...
function _getPriorityPeers({
  ...
  */
  ...
  for(const {id: elector} of electors) {
    ...
    nonByzantinePeers.push(elector);
    heads.set(elector, tail._c.head[0]);
  }
  ...
  for(const [elector, head] of heads) {
    ...
    for(const [otherElector, otherHead] of heads) {
      ...
      const mra = head._c.mostRecentWitnessAncestors &&
        head._c.mostRecentWitnessAncestors.get(otherElector);
      if(!mra) {
      ...
      // merge it in
      if((!mra._c.supporting && otherHead._c.supporting) ||
        (mra._c.supporting && otherHead._c.supporting &&
        mra._c.supporting !== otherHead._c.supporting)) {
        priorityPeers.add(elector);
        break;
      }
      ...
      if(otherHead._c.proposal && otherHead._c.proposal._c.endorsers &&
        ...
        !otherHead._c.proposal._c.endorsers.has(elector)) {
        priorityPeers.add(elector);
        break;
      }
...
function _hasElectorAncestor({event, elector, ancestor}) {
  const mra = event._c.parents.length > 0 &&
    event._c.mostRecentWitnessAncestors.get(elector);
