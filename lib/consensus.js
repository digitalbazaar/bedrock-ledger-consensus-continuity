...
function _getPriorityPeers({
  ...
  If `recoveryElectors.length > 0` then prioritize recovery electors in the
  case that any proposals exist. Otherwise, prioritize decision electors.

  1. If any endorsed decision proposals exist, then prioritize (recovery)
     electors without them.
  2. If any decision proposals exist, then prioritize (recovery) electors
     without them.
  3. If any endorsed recovery proposals exist, then prioritize (recovery)
     electors without them.
  4. If any recovery proposals exist, then prioritize (recovery) electors
     without them.
  5. Otherwise, no special priority.

  TODO: add a step before 5 to prioritize any electors that haven't been heard
  from in a while.
  */
  const recoverySet = new Set(recoveryElectors.map(e => e.id));

  // tally proposal information at head of every non-byzantine elector branch
  let endorsedProposals = 0;
  let proposals = 0;
  let endorsedRecoveryProposals = 0;
  let recoveryProposals = 0;
  const hasNoEndorsedProposal = [];
  const hasNoProposal = [];
  const hasNoEndorsedRecoveryProposal = [];
  const hasNoRecoveryProposal = [];
  ...
  for(const {id: elector} of electors) {
    ...
    if(!electorTails) {
      ...
      hasNoEndorsedProposal.push(elector);
      hasNoProposal.push(elector);
      if(recoverySet.has(elector)) {
        hasNoEndorsedRecoveryProposal.push(elector);
        hasNoRecoveryProposal.push(elector);
      }
      continue;
    }
    ...
    const {_head: [head]} = tail;

    // if has endorsed proposal then won't be prioritized, but tally it
    if(_hasEndorsedProposal(head)) {
      endorsedProposals++;
      continue;
    }
    hasNoEndorsedProposal.push(elector);

    // if has proposal then will only be prioritized if another peer has
    // an endorsed proposal, but tally it
    if(head._proposal) {
      proposals++;
      continue;
    }
    ...
    // not a recovery elector, continue...
    if(!recoverySet.has(elector)) {
      continue;
    }

    // if has endorsed recovery proposal then will only be prioritized if
    // another peer has a decision proposal, but tally it
    if(_hasEndorsedRecoveryProposal(head)) {
      endorsedRecoveryProposals++;
      continue;
    }
    hasNoEndorsedRecoveryProposal.push(elector);

    // if has recovery proposal then will only be prioritized if
    // another peer has a decision/endorsed recovery proposal, but tally it
    if(head._recovery && head._recovery.proposal) {
      recoveryProposals++;
    } else {
      hasNoRecoveryProposal.push(elector);
    }
  ...
  let priorityPeers = nonByzantinePeers;
  if(endorsedProposals > 0) {
    if(hasNoEndorsedProposal.length > 0) {
      priorityPeers = hasNoEndorsedProposal;
    }
  } else if(proposals > 0) {
    if(hasNoProposal.length > 0) {
      priorityPeers = hasNoProposal;
    }
  } else if(endorsedRecoveryProposals > 0) {
    if(hasNoEndorsedRecoveryProposal.length > 0) {
      priorityPeers = hasNoEndorsedRecoveryProposal;
    }
  } else if(recoveryProposals > 0) {
    if(hasNoRecoveryProposal.length > 0) {
      priorityPeers = hasNoRecoveryProposal;
    }
  }

  // filter via recovery set if size > 0 and any proposals exist because only
  // recovery electors participate in endorsement
  if(recoverySet.size > 0 && (endorsedProposals > 0 || proposals > 0 ||
    endorsedRecoveryProposals > 0 || recoveryProposals > 0)) {
    priorityPeers = priorityPeers.filter(e => recoverySet.has(e));
  }
