...
function _computeRecovery({
  ...
  if(event._recovery.proposal) {
    for(const creator in tracking) {
      // note: according to rules, does not matter if creator has forked,
      // always use a decision from another recovery elector to reject a
      ...
      if(tracking[creator]._recovery.decision) {
  ...
  tracking[_getCreator(event)] = event;

  // TODO: may have to do endorsement of proposals here

  // check the recovery tracking ancestry for this event for how many
  // other recovery electors have proposed entering recovery mode
  let proposals = 0;
  for(const creator in tracking) {
    // note: according to rules, does not matter if creator has forked, always
    // count proposal
    if(tracking[creator]._recovery.proposal) {
      proposals++;
    }
  }

  if(proposals.length === recoveryElectors.length) {
    // once all recovey electors have made proposals, they cannot switch
    // to a decision, so we can safely enter recovery mode
    // TODO: we may need to get endorsements of these proposals to prevent
    // concurrent switching from proposal => decision via influence by
    // another recovery elector
    ...
    */
    if(nextChoice.endorsedProposalCount >= supermajority) {
      ...
      if(recoveryElectors.length === 0) {
        ...
        // if the decision was from another recovery elector)
        ...
        // then finalize decision; iterate through recovery electors checking
        // to see if any of them have seen a sufficient number of decisions
        const recoverySet = new Set(recoveryElectors.map(e => e.id));
        for(const elector of recoverySet) {
          const votingEvent = event._votes[elector];
          if(!votingEvent) {
            // can't see this recovery elector, skip it
            continue;
          }
          // check this recovery elector's recovery info for enough seen
          // decisions
          const {tracking} = votingEvent._recovery;
          let count = 0;
          for(const creator in tracking) {
            // note: rules indicate that recovery elector forks don't matter
            // when counting decisions
            if(tracking[creator]._recovery.decision) {
              count++;
            }
          }
          if(count >= recoveryDecisionThreshold) {
            consensusReached = true;
            break;
          }
        }
