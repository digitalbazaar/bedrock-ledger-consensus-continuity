...
function _findMergeEventProof({
  ...
  const ys = _findConsensusMergeEventProof(
    {ledgerNodeId, xByElector, yByElector, blockHeight, electors, logger});
...
function _findMergeEventProofCandidates({
  ...
  for(const elector of electorsWithTails) {
    ...
    // include in a block should an X be selected
  ...
  // TODO: remove use only `yByElector` (replace `xByElector` with it)
  ...
  for(const elector in xByElector) {
    for(const x of xByElector[elector]) {
      ...
      const results = [{mergeEvent: x, descendants: new Map()}];
      const mergeEvents = [];
...
function _tallyBranches({
  ...
  */
  ...
  // TODO: clean up now superfluous variables
  const influentialElectorSet = yElectorSet;
...
function _computeEndorsement({
  ...
  const sharedEndorsementKey = '_electorEndorsement';
