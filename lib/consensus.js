...
function _computeRecovery({
  ...
  if(event._recovery.proposal) {
    for(const creator in tracking) {
      ...
      const trackingEvent = tracking[creator];
      if(trackingEvent && trackingEvent._recovery.decision) {
  ...
  // go through the recovery tracking ancestry for this event (including
  ...
  for(const creator in tracking) {
    if(!tracking[creator]) {
      // skip, no results to compute for a byzantine creator
      continue;
    }
    const result = results[creator] = {};
    result.byzantines = new Set();
    result.proposals = 0;
    const {tracking: peerTracking} = tracking[creator]._recovery;
    for(const c in peerTracking) {
      const trackingEvent = peerTracking[c];
      if(!trackingEvent) {
        result.byzantines.push(c);
    ...
    if(nextChoice.endorsedProposalCount >= supermajority) {
      ...
      if(recoveryElectors.length === 0) {
        ...
        for(const elector of recoverySet) {
          ...
          let count = 0;
          for(const creator in tracking) {
            // count decisions seen by non-byzantine recovery electors
            const trackingEvent = tracking[creator];
            if(trackingEvent && trackingEvent._recovery.decision) {
              count++;
            }
