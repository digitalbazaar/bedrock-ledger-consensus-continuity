...
function _getElectorBranches({history, electors}) {
  ...
  const witnessTails = {};
  const witnessSet = new Set(electors.map(e => e.id));
  ...
  for(const witness in witnessTails) {
    let generation = 1;
    const tail = witnessTails[witness];
    let next = tail;
    let head = null;
    while(next.length > 0) {
      const current = next;
...
function _findMergeEventProof({
  ...
  const ys = _findConsensusMergeEventProof(
    {ledgerNodeId, yByElector, blockHeight, witnesses: electors, logger});
 ...
 * @param logger the logger to use.
 *
 * @return `null` or a map containing `yByElectors`, whereby each witness maps
...
function _findMergeEventProofCandidates({
  ...
  const witnessesWithTails = Object.keys(tails);
  ...
  /*console.log('Continuity2017 witnesses with tails for ledger node ' +
    ledgerNodeId + ' with required supermajority ' + supermajority,
    {ledgerNode: ledgerNodeId, witnessesWithTails});*/
  if(witnessesWithTails.length < supermajority) {
  ...
  for(const witness of witnessesWithTails) {
    ...
    // support properly
    const witnessTails = tails[witness];
  ...
  while(next.length > 0) {
    ...
    for(const {treeDescendant, endorsers} of current) {
      ...
      for(const [witness, ancestorEvent] of
        ...
        // console.log('witnesses', witnessSet);
...
function _findConsensusMergeEventProof({
  ledgerNodeId, yByElector, blockHeight, witnesses,
  logger = noopLogger
}) {
  /*logger.verbose(
  ...
  const consensus = _tallyBranches({
    ledgerNodeId, yByElector, blockHeight, witnesses, logger
  });
...
function _tallyBranches({
  ledgerNodeId, yByElector, blockHeight, witnesses, logger = noopLogger
}) {
  /* Algorithm:
  ...
  */
  ...
  const yWitnessSet = new Set(Object.keys(yByElector));
  const influentialTails = new Map();
  for(const witness of yWitnessSet) {
    influentialTails.set(witness, yByElector[witness]);
  }
  const tailsArray = [].concat(...[...influentialTails.values()]);
