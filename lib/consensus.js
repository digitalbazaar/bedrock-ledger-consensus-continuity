...
function _findMergeEventProof({
  ...
  if(recoveryElectors.length > 0) {
    ...
    // TODO: RECOVERY_MODE: if recovery mode can't be entered, then we should
    // be able to optimize away computing some of the recovery mode info,
    // but we still need to be able to check decisions to make sure the
    // `recoveryDecisionThreshold` is met
  }
  ...
  // determine if a decision can even be made yet...
