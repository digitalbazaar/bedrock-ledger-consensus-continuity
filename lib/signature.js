 ...
 * @param {Array<string>} [options.includeHeaders = coveredContent] - Which
 ...
 * @param {Array<string>} [options.expectedHeaders = coveredContent] - Which
 *   headers are expected to be in the request.
 *
 * @returns {Promise<object>} The result of verification and the keyId.
 */
exports.verifyRequest = async ({req, expectedHeaders = coveredContent}) => {
  try {
    ...
    const key = new Ed25519VerificationKey2018(_getPublicKey(keyId));
...
function _getPublicKey(publicKeyId) {
  const publicKeyBase58 = _localPeers.getPublicKeyFromId(
    {peerId: publicKeyId});
  return {
    id: publicKeyId,
