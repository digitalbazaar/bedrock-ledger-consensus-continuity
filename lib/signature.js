...
api.verify = ({doc, ledgerNodeId}, callback) => {
  ...
  async.auto({
    ...
    finalize: ['storeKey', (results, callback) => {
      const keyResultError = _.get(results, 'verify.keyResults.error', 'none');
    ...
    async.auto({
      ...
      getKey: ['localStorage', (results, callback) => {
        if(results.localStorage) {
          logger.debug('Key from Storage', {result: results.localStorage});
        ...
        jsigs.getPublicKey(publicKeyId, options, (err, result) => {
          ...
          logger.debug('Key from Web', {result});
          newKey = result;
