...
api.verify = callbackify(async ({doc, ledgerNodeId}) => {
  ...
  // FIXME: remove when simplifying public keys
  // new key to store
  if(newKey) {
    ...
    storage.keys.addPublicKey(ledgerNodeId, newKey).catch(() => {});
  }
  ...
  if(key.publicKeyBase58) {
    ...
    const b58Fingerprint = _getPublicKeyFromId(keyOwner.id);
    if(key.publicKeyBase58 !== b58Fingerprint) {
      throw new BedrockError(
