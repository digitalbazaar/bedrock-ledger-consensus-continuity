...
api.get = async (
  ...
  if(!(privateKey || publicKey)) {
    const voterId = await _cache.peers.get({ledgerNodeId});
    if(voterId !== null) {
      // cache hit
      return {id: voterId};
    }
  ...
  // update cache
  await _cache.peers.add({voterId: voter.id, ledgerNodeId});
 ...
 * reached.
 *
 * @param localPeerId the ID of the node to get the peers for.
...
// FIXME: remove this, it is unused
api.getPeerIds = async ({localPeerId, ledgerNode, limit}) => {
  ...
  const query = {
    ...
    // break... why?
    // exclude localPeerId
    //'meta.continuity2017.creator': {$ne: localPeerId},
  };
 ...
 */
api.storage.add = async voter => {
  logger.verbose('adding voter', {voterId: voter.id});
