...
async function _isPeerCommitmentRequired({
  ...
  const [wasWitness, hasCommitmentOrHead] = await Promise.all([
    _hasBeenWitness({ledgerNode, peerId}),
...
async function _hasBeenWitness({ledgerNode, peerId, explain = false} = {}) {
...
// FIXME: uncomment to check for commitments on non-witnesses before merging
/*
async function _getCommitmentsToPeer({
