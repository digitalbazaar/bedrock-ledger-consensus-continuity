  ...
  if(!cachedRecord) {
    try {
      const record = await api.optionallyAdd(
        {ledgerNode, remotePeer, notifier: true});
      if(record) {
        // update cache
        records.push(record);
...
api.optionallyAdd = async ({
  ledgerNode, remotePeer, notifier = false
} = {}) => {
  ...
  try {
    ...
    if(notifier) {
      // FIXME: check if the peer has never been a witness and if
      // no one has ever committed to them; if so, set a flag in the peer
      // information so they can be committed to later (also consider that we
      // don't want to flip that flag *back* if it was cleared previously) --
      // and once committed to and the local merge event written to the
      // database, update the peer to remove that flag to prevent multiple
      // commitments from happening as they are unnecessary
      // FIXME: maybe we don't want to add a "new" peer if `notifier=false`
      // ... so we always want to run the above checks for new peers, but
      // don't choose to add the peer if it's not via a notification and it
      // is a new peer
    }
