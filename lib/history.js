...
api._findNewForkers = async function({
  ...
  // for all forker numbers that are not `0`, we must check to see if a
  ...
  const $or = [];
  for(const [creator, localForkNumber] of forkNumberMap) {
    if(localForkNumber === 0) {
      // not a fork
      continue;
    }
    ...
    $or.push({
      ...
      'meta.continuity2017.localForkNumber': {$ne: localForkNumber}
    });
  }
...
api._getConsensusHistory = async function({
  ...
  result.cursor = {
    ...
    blockEventCount: cursorRecord.meta.continuity2017.gossipOrder,
...
api._getKnownPeerHeads = async ({ledgerNode, peerHeads} = {}) => {
  ...
  for(const {meta} of records) {
    const {
      continuity2017: {creator, generation, localForkNumber}
    } = meta;
    const head = {creator, generation, localForkNumber};
...
function _filterByPeerHeads({records, remotePeerId, peerHeads} = {}) {
  ...
  and `localForkNumber` information to determine if we can short-circuit or
  not. For heads of forkers, this may be inefficient, resulting in hashes being
  sent to the remote peer that it actually has, which is not a protocol
    ...
    for(const vertex of current) {
      ...
      if(heads) {
        for(const head of heads) {
          ...
          if(localForkNumber === head.localForkNumber &&
