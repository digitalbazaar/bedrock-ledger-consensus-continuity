...
api.getHeads = async ({ledgerNode, creatorIds, latest = false}) => {
  // FIXME: if `creatorIds` is not specified, as it would not be if this were
  // to be used in gossip, can we make use of `peerChildlessHashes`? We'd need
  // the creator IDs for those; consider this when updating gossip to use this
  // function to account for expansive growth in non-witnesses over time
  const heads = new Map();
...
api.getHead = async ({
  ...
  if(!head) {
    ...
    if(records.length > 0) {
      ...
      head = {eventHash, generation, basisBlockHeight};
...
api.getGenesisHead = async ({ledgerNode}) => {
  ...
  // generation and basisBlockHeight for genesis are always zero
  head = {eventHash: meta.eventHash, generation: 0, basisBlockHeight: 0};
