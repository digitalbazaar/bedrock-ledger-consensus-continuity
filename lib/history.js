...
api._getNonConsensusHistory = async function({
  ...
  // FIXME: rename `inspectPeerHeads`; it is now to look up the given event
  // hashes to get creator, generation, and fork info, it's not validation
  // anymore

  // inspect the `peerHeads` from the remote peer to determine which are valid
  // and to get the local `forkNumber` associated with head, if any; we cannot
  // assume `peerHeads` are valid as they could come from an untrusted peer
  // FIXME: determine if we want to throw if the peer sends its own heads
  ...
  // filter by remote peer ID and any given peer heads
  const {peerId: remotePeerId} = remoteInfo;
  const filtered = _filterByPeerHeads(
    {records, remotePeerId, peerHeads: validHeads});
...
api._getNonConsensusHeads = async ({
  ledgerNode, limit = 100, explain = false
} = {}) => {
  const {collection} = ledgerNode.storage.events;
  const cursor = collection.aggregate([
    ...
    // sort by generation
    // Note: We include `consensus` and `type` in the sort because doing so
    // causes the index to be used, it does not affect the outcome
    {
      $sort: {
        'meta.consensus': 1,
        'meta.continuity2017.type': 1,
    ...
    // group by creator to make them distinct and return
    ...
    // limit number of distinct heads
    {
      $limit: limit
    },
    // map `_id` to `creator`
...
api._inspectPeerHeads = async ({ledgerNode, peerHeads} = {}) => {
  ...
  for(const {meta} of records) {
    ...
    if(head.generation === generation && head.creator === creator) {
      validHeads.push({...head, forkNumber});
    }
  }
  return {validHeads};
};
