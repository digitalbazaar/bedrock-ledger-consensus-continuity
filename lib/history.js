...
api.getNonConsensusPeerHeads = async ({
...
// gets the peer heads for a given block from the perspective of a remote peer
api.getPeerHeadsForBlock = async ({
  ledgerNode, blockHeight, remotePeerId, limit = 1000, explain = false
} = {}) => {
  // FIXME: consider moving to `bedrock-ledger-consensus-continuity-storage`
  // FIXME: make sure this is a covered query
  const {collection} = ledgerNode.storage.events;
  const cursor = collection.aggregate([
    // find merge events in the given `blockHeight`
    {
      $match: {
...
// remoteInfo: {peerId, peerHeads, blockHeight}
api.partition2 = async ({ledgerNode, remoteInfo} = {}) => {
  ...
  // FIXME: remove me
  console.log('peer', remoteInfo.peerId, 'blockHeight', remoteInfo.blockHeight);

  // inspect `peerHeads` to determine which are valid and get next block info
  const [{validHeads}, nextBlockInfo] = await Promise.all([
    api.inspectPeerHeads({ledgerNode, peerHeads: remoteInfo.peerHeads}),
    _blocks.getNextBlockInfo({ledgerNode})
  ]);

  // search, one block at a time, for events starting one block after the
  // remote peer's declared known `blockHeight` and going until local peer's
  // next block height (thus including any non-consensus events); stop once
  // some events are found or all local blocks and non-consensus events have
  // been checked
  let startHeads;
  let history;
  let minBlockHeight = Math.min(
    remoteInfo.blockHeight + 1, nextBlockInfo.blockHeight);
  const {peerId: remotePeerId} = remoteInfo;
  do {
    // get starting heads for search; use non-consensus heads if min block
    // height is the local next block height, otherwise use heads from the
    // min block height
    if(minBlockHeight === nextBlockInfo.blockHeight) {
      startHeads = await api.getNonConsensusPeerHeads(
        {ledgerNode, remotePeerId});
    } else {
      startHeads = await api.getPeerHeadsForBlock(
        {ledgerNode, blockHeight: minBlockHeight, remotePeerId});
    }
    ...
    // long fork line to begin with -- if we have deterministic sort and the
    // client sends the `eventHash` right before the truncation, then we could
    // continue -- if given the same `peerHeads` ... and even if not. A
    // deterministic sort should always sort that `eventHash` to the top and
    // everything before it can be dropped. So the solution here may be to
    // treat unseen peer heads as if they are forks (drop them) and for
    // seen heads that *are* forks, drop them; but this isn't enough, we would
    // need to always drop heads for any peer that has previously forked
    // because our graph lookup isn't smart enough to pick up on when we
    // need to stop and when we don't because we only give it `generation`
    // and `creator`, not `eventHash` or any kind of "ancestry" information.
    // ... This necessarily means forking creates inefficiencies in the gossip
    // protocol, which is quite undesirable. We'd need to include fork
    // information in our stopping conditions ... so any event that has
    // {$lte: generation}, AND that has a fork ID that is in the given fork ID
    // list ... that would guarantee that it was an ancestor. That should work
    // once we have fork ID implemented.
    ...
    known `blockHeight` if the remote peer is not following protocol. The query
    here ensures we always stop searching at the declared `blockHeight` and
    do not rely on the `peerHeads` to be after it to stop the search, thus
    preventing remote peers that aren't following protocol from causing us
    to find more events than are in a single block.

    /* Note: This code will limit the maximum number of events examined to
    those that are in a single block. However, without a hard block size
    boundary, there could be more events in a single block than we would want
    to examine in a single call. If significantly large blocks become a
    problem, a future change will be required to reduce processing time to
    something more manageable. */
    history = await api._aggregateHistory({
      ledgerNode, minBlockHeight, startHeads, endHeads: validHeads
    });

    /* Note: An alternative approach to returning once some events are found
    would be to keep looping forward in history until we've collected a minimum
    number of events. However, that creates an asymmetry that an attacker could
    exploit whereby they only need to issue one request to cause us to perform
    many graph lookups that actually return documents. Instead, they must
    perform multiple requests which enables us to more easily detect and
    block bad actors at the request level before we run this code. If this
    asymmetry can be avoided by another implementation in the future, we can
    change this behavior to be more efficient without having to make any
    protocol level changes. */
    if(history.length > 0) {
      break;
    }

    // no events found in the current block, try the next one; note that this
    // should only happen when the events required to reach consensus in a
    // certain block don't occur until a few blocks later, which should be
    // very rare and not under the control of an attacker; also, under these
    // conditions, queries should be relatively fast since no results are
    // being found
    minBlockHeight++;
  } while(minBlockHeight <= nextBlockInfo.blockHeight);

  // sort history
  const sortedHistory = _sortHistory(history);

  // FIXME: the most history that could possibly be returned at once is
  // a single block -- do we need to consider additional truncation here?

  // FIXME: truncate history to permissible number of event hashes to
  // be returned
  // FIXME: if we're going to allow continuations via truncated event hash,
  // then we'll have to make the sort deterministic (see code that sorts
  // hashes for blocks, kahn's algorithm alone is not deterministic)
  // FIXME: can't truncate events consistently if non-consensus peer heads
  // were used as those may change from request to request; how to handle?

  // FIXME: when truncating the history, we need to include the heads from
  // the history so we can return them to allow for a continuation; this
  // means computing those heads from the returned history... which means
  // returning generation and creator information along with the history
  // so the heads can be found; can't use `startHeads` when truncating

  timer.stop();

  return {
    peerHeads: startHeads,
...
api._aggregateHistory = async function({
  ...
  // based on:
  // 1. `minBlockHeight` - Stop at any merge event that reached consensus in
  //   the block associated with `minBlockHeight`.
  ...
  const restrictSearchWithMatch = {
    $nor: [{
      'meta.blockHeight': {$exists: true, $lte: minBlockHeight},
      'meta.continuity2017.type': 'm'
    }]
  };
