...
api._getEventHistory = async function({
  ...
  const records = await api._getSortedEventSummaries({
    ledgerNode,
    minBlockHeight,
  ...
  // FIXME: remove me
  /*console.log({
    lp: (await _peers.get({ledgerNodeId: ledgerNode.id})).id.substr(-4),
  ...
  if(truncated) {
    ...
    result.cursor = {
      ...
      basisBlockHeight: Math.max(basisBlockHeight, requiredBlockHeight),
      localEventNumber,
      hasMore: true
    };
  } else {
...
api._findNewReplayers = async function({
  ledgerNode, replayNumberMap, explain = false
} = {}) {
...
// FIXME: move to bedrock-ledger-consensus-continuity-storage
api._getSortedEventSummaries = async function({
...
async function _getAcceptableHistory({
  ...
  for(acceptableCount = 0; acceptableCount < maxRecords; ++acceptableCount) {
    ...
    if(requiredBlockHeight !== -1) {
      ...
      /*if(type === 'm') {
        console.log('pass 1', {
          eventHash: records[acceptableCount].meta.eventHash,
          requiredBlockHeight,
          basisBlockHeight
        });
      }*/
    ...
    if(basisBlockHeight >= localBlockHeight) {
      ...
      if(witnesses.includes(creator)) {
        // FIXME: remove me
        /*console.log('WITNESS CONVERSION',
          records[acceptableCount].meta.eventHash);*/
        acceptableCount++;
        continue;
      }
    }
    ...
    acceptableCount++;
    // FIXME: remove me
    /*console.log('NOT WITNESS TRUNCATE',
      records[acceptableCount].meta.eventHash);*/
    break;
  }
