...
api.partition = async ({ledgerNode, remoteInfo} = {}) => {
  ...
  const nextRemoteBlockHeight = remoteInfo.basisBlockHeight + 1;

  // get local peer's current block info and event history that occurs after
  // the remote peer's known block height
  const [nextBlockInfo, eventHistory] = await Promise.all([
    _blocks.getNextBlockInfo({ledgerNode}),
    api._getEventHistory(
      {ledgerNode, remoteInfo, minBlockHeight: nextRemoteBlockHeight})
  ]);
