...
api.partition = async ({ledgerNode, remoteInfo} = {}) => {
  ...
  const {'ledger-consensus-continuity': {gossip}} = bedrock.config;
  const maxEvents = gossip.maxEvents - result.totalEvents;
  if(sortedHistory.length > maxEvents) {
    // FIXME: must truncate at a merge event with a `basisBlockHeight` that
    // is <= the client's declared `basisBlockHeight`, so need to keep a map
    // around of the `history` that tells us whether the events we're
    // truncating meet these conditions or not -- note that even though
    // graphLookup will stop at a merge event, we may need to truncate more to
    // stay within the transfer limits
...
api._getConsensusEvents = async function({
  ...
  const records = await api._getConsensusHistory({
    ledgerNode,
    // we need to search for one more than the client's declared
    // `basisBlockHeight` because we need to return the required
    // `basisBlockHeight` in any overflow
    maxBasisBlockHeight: basisBlockHeight + 1,
    minBlockHeight: blockHeight,
    minBlockOrder: blockEventCount,
    maxBlockHeight,
    limit
  });
  ...
  is guaranteed to end in a merge event unless the `maxBasisBlockHeight` was
  reached or the client made an improper request, or there is database
  corruption. Here we always roll back until we find a terminating merge event
  with an appropriate `basisBlockHeight`. If we started with some records and
  roll back all the way to zero, that means the client made an improper request
  and we throw an exception. Otherwise, we will find a trailing merge event
  with a valid `basisBlockHeight` and we save the information for the record
  that comes after it so the client can continue in another request. */
  ...
  while(recordCount > 0) {
    /* Note: This works because the total ordering algorithm causes regular
    events to be inserted just prior to their child merge event instead of
    spreading them out breadth-first like merge events are. Therefore we
    should never have a block with an order like this: [r1,r2,m1,m2] as that
    would create a problem if we truncated after `m1` since `r2` would have no
    merge event transferred along with it. But the total ordering algorithm
    used results in this ordering instead: `[r1,m1,r2,m2]`, so when we truncate
    after `m1` there's nothing that comes before it that is incomplete. */
    const {
  ...
  if(recordCount === 0) {
    // no trailing merge event found, which means the client made an improper
    // request
    throw new BedrockError(
      `The given "blockEventCount" (${blockEventCount}) for "blockHeight" ` +
      `(${blockHeight}) must fall on a merge event boundary.`,
      'DataError', {
        blockHeight,
        blockEventCount,
        httpStatusCode: 400,
  ...
  // `blockHeight`, `blockEventCount`, and `basisBlockHeight` for the following
  // record so that the client can continue when it is ready to make another
  // request
  if(recordCount < records.length) {
    const {
      event: {basisBlockHeight},
      meta: {blockHeight, blockOrder}
    } = records[recordCount];
    result.next = {basisBlockHeight, blockHeight, blockEventCount: blockOrder};
  }
...
api._getConsensusHistory = async function({
  ledgerNode, maxBasisBlockHeight,
  minBlockHeight, minBlockOrder = 0, maxBlockHeight,
  ...
  const query = {
    ...
    'event.basisBlockHeight': {$lte: maxBasisBlockHeight}
  };
  const projection = {
    _id: 0,
  ...
  const cursor = await collection.find(query, {projection})
    ...
    // skips any declared `blockEventCount`
    .skip(minBlockOrder)
    .limit(limit);
  if(explain) {
    return cursor.explain('executionStats');
