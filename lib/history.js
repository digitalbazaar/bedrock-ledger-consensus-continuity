...
bedrock.events.on('bedrock.init', () => {
  ...
  if(gossip.maxEvents < maxEvents) {
    throw new Error(
      'Gossip configuration is invalid; "gossip.maxEvents" ' +
      `(${gossip.maxEvents}) must be greater than or equal to continuity ` +
      `constants "mergeEvents.maxEvents" (${maxEvents}).`);
  }
});
...
api.partition = async ({ledgerNode, remoteInfo} = {}) => {
  ...
  if(sortedHistory.length > maxEvents) {
    // FIXME: must truncate at a merge event, so need to keep a map around
    // of the `history` that tells us whether the events we're truncating
    // are merge events or not -- note that even though graphLookup will
    // stop at a merge event, we may need to truncate more to stay within
    // the transfer limits
...
api._getConsensusEvents = async function({
  ledgerNode, blockHeight, blockEventCount
} = {}) {
  const result = {
    blocks: [],
    next: null,
  ...
  // merge event available for download after we've hit the limit
  const {mergeEvents: {maxEvents}} = _continuityConstants;
  const limit = gossip.maxEvents + maxEvents;
  ...
  /* Note: Another process could be in the middle of writing a block, causing
  `records` to contain some regular events for which the merge event has not
  been marked as having reached consensus yet. Therefore, we roll back until we
  hit the latest full merge event. If we're still over the limit when we hit
  it, then we indicate that we can set `next` and then continue rolling back to
  the latest full merge event that is under the gossip max events limit. If we
  run out of records while looking for the latest merge event, then either the
  client made a bad request or we're in the middle of writing a block; since we
  can't tell the difference, we just return that there is no available
  consensus data. */
  let hasMore = false;
  let recordCount = records.length;
  for(; recordCount > 0; --recordCount) {
    const {meta: {continuity2017: {type}}} = records[recordCount - 1];
    if(type !== 'm') {
      continue;
    }

    // found a trailing merge event, but we're over the gossip event limit,
    // which means there is more consensus data to be returned; `next` will be
    // set using the record after the latest merge event that is under the max
    // gossip event limit
    if(recordCount > gossip.maxEvents) {
      hasMore = true;
      continue;
    }

    // we're at/under the max gossip event limit and terminate in a merge
    // event; if we found another merge event while rolling back, set `next`
    if(hasMore) {
      // return `blockHeight` and `blockEventCount` for the next record to
      // declare as known in the next request; `blockEventCount=0` informs
      // the client about the block so they can request it without having any
      // events for it yet
      const {meta: {blockHeight, blockOrder}} = records[recordCount];
      result.next = {blockHeight, blockEventCount: blockOrder};
    }

    // found terminating merge event, break out
    break;
  }
...
api._getConsensusHistory = async function({
  ...
  const query = {
    ...
    // FIXME: if we place a range here (add a maximum too), we can simplify
    // calls that are made complex by the fact that a block may be getting
