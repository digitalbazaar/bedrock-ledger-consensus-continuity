...
async function _validateParentNonMergeEvent({
  ...
  const {
    event: {
      treeHash: eventTreeHash,
      basisBlockHeight: eventBasisBlockHeight
    }
  } = eventRecord;
  const {
    event: {
      treeHash: parentTreeHash,
      basisBlockHeight: parentBasisBlockHeight
    }
  } = parentRecord;

  if(eventBasisBlockHeight !== parentBasisBlockHeight) {
    throw new BedrockError(
      'Non-merge event parents must have the same "basisBlockHeight" as a ' +
      'child merge event.', 'DataError', {
        eventRecord,
        parentRecord,
        httpStatusCode: 400,
        public: true
...
async function _checkParentHashCommitments({
  ...
  const {getParentHashCommitments} = ledgerNode.storage.events
    .plugins['continuity-storage'];
  const {getWitnessConversions} = ledgerNode.storage.blocks
    .plugins['continuity-storage'];
  const [commitments, conversions] = await Promise.all([
    getParentHashCommitments(
      {nonWitnessHashes, blockHeight: consensusState.blockHeight}),
    getWitnessConversions(
      {nonWitnessEvents: [...nonWitnessEventMap.values()]})
  ]);
...
async function _checkPeerCommitments({
  ...
  const {haveBeenWitnesses} = ledgerNode.storage.blocks
    .plugins['continuity-storage'];
  const pastWitnesses = new Set(await haveBeenWitnesses({peerIds}));
...
async function _checkReplays({worker, mergeEvents, genesisHead, replayMap}) {
  ...
  const {ledgerNode} = worker;
  const {getOperationReplayers} = ledgerNode.storage.operations
    .plugins['continuity-storage'];
  const {getForkers/*, getConfigReplayers*/} = ledgerNode.storage.events
    .plugins['continuity-storage'];
  ...
  const [forkers, operationReplayers/*, configReplayers*/] = await Promise.all([
    ...
    getOperationReplayers({operationSummaries})/*,
    // FIXME: create `configSummaries` above
    getConfigReplayers({configSummaries: []})*/
  ]);
  ...
  const {getLatestReplays} = ledgerNode.storage.events
    .plugins['continuity-storage'];
  const latestReplays = await getLatestReplays(
    {peerIds: [...replayerMap.keys()]});
