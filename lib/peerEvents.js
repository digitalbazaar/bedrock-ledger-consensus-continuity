...
async function _validateEvents({worker, events, neededHashes}) {
  ...
  try {
    ...
    for(const event of events) {
      ...
      // send event for validation
      promises.push(limit(() => _client.validateEvent({
        event,
        ledgerNodeId,
        creatorId: worker.creatorId,
        // FIXME: Find and use the actual session id
        session: worker.session.id
      }).catch(e => logger.error('An error occurred during gossip processing.',
        {error: e}))
      ));
    }
