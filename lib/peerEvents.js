...
async function _validateGraph({worker, eventMap, remotePeerId}) {
    ...
    for(const eventRecord of current) {
      ...
      for(const parentHash of event.parentHash) {
        ...
        if(parentRecord && parentRecord.meta.continuity2017.type === 'm' &&
          ...
          // FIXME: ensure error type is `DataError`
          const err = new Error('PROTOCOL VIOLATION 1');
          err.name = 'DataError';
          throw err;
        }
      }
      ...
      for(const parentHash of event.parentHash) {
        ...
        if(!parentRecord) {
          ...
          if(!parentRecord) {
            ...
            // FIXME: ensure error type is `DataError`
            const err = new Error('PROTOCOL VIOLATION 2');
            err.name = 'DataError';
            throw err;
          }
        }
      ...
      if(!_temp.processed) {
        ...
        eventRecord.meta.continuity2017.hasParentHashCommitment =
          !!eventRecord.event.parentHashCommitment;
  ...
  // FIXME: need to check peer commitments; if the event's tree parent is the
  // genesis event and the peer has never been a witness, then we must check to
  // make sure that the peer has a parent that has committed to it
