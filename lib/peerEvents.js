...
api.addBatch = async ({worker, events, neededHashes}) => {
  ...
  for(const {event, meta, _temp} of eventMap.values()) {
    ...
    // FIXME: we could add a simple incrementing counter to `meta` to every
    // event... which could be used to assist in improving gossip efficiency
    ...
    for(const eventRecord of current) {
      ...
      for(const parentHash of event.parentHash) {
        ...
        if(parentRecord && parentRecord.meta.continuity2017.type === 'm' &&
          ...
          defer = true;
          // FIXME: throw here instead; reject entire batch if order is not
          // proper, it's a protocol violation
          break;
        }
      }
      if(defer) {
      ...
      for(const parentHash of event.parentHash) {
        ...
        if(!parentRecord) {
          ...
          if(!parentRecord) {
            ...
            // FIXME: throw instead, protocol violation
            break;
          }
        }
...
async function _checkForks({worker, mergeEvents, genesisHead}) {
  ...
  for(const {peerId, forkNumber, forkDetectedBlockHeight} of latestForks) {
    const forker = forkerMap.get(peerId);
    forker.forkNumber = forkNumber;
  ...
  //   `forkNumber` and add it to the event's meta.
  ...
  for(const {event, meta} of mergeEvents) {
    ...
    if(treeHashSet.has(event.treeHash)) {
      // 2. Event is a new fork, increment `forkNumber` and add it.
      if(forker.forkNumber === null) {
        // first `forkNumber` will be `1` due to increment on next line,
        // which is desirable
        forker.forkNumber = 0;
      }
      meta.forkNumber = ++forker.forkNumber;
    }
    // 3. Add `forkDetectedBlockHeight` if any.
    if(forkDetectedBlockHeight !== null) {
      meta.forkDetectedBlockHeight = forkDetectedBlockHeight;
    }
  }
}
...
async function _getLatestForks({ledgerNode, peerIds, explain = false}) {
  ...
  const cursor = collection.aggregate([
    ...
    // sort by `forkNumber`, descending, to get highest one per creator
    ...
    // group by creator to return just `creator`
    {
      $group: {
        _id: '$meta.continuity2017.creator',
        // can safely use `$first` here because we sorted by `forkNumber`
        eventHash: {$first: '$meta.forkNumber'}
      }
    },
    ...
    // map `_id` to `peerId`
    {
      $project: {
        _id: 0,
        peerId: '$_id',
        forkNumber: '$meta.forkNumber',
        forkDetectedBlockHeight: '$meta.forkDetectedBlockHeight'
      }
    }
  ], {allowDiskUse: true});
  if(explain) {
