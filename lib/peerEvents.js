...
async function _validateEvents({worker, events, neededHashes}) {
  ...
  try {
    ...
    for(const event of events) {
      ...
      tasks.push({worker, event, totalEventCount, pendingValidations});
    }
...
async function _validateEventViaService({
  worker, event, totalEventCount, pendingValidations
}) {
  ...
  while(!worker.halt()) {
    ...
    try {
      ...
      if(_canRetryValidation(e)) {
        ...
        // wait for validation service to be less busy
        await _waitForValidationService();
        continue;
      }
      throw e;
    } finally {
...
async function _waitForValidationService({pendingValidations}) {
  while(true) {
    if(pendingValidations.size === 0) {
      // if there are no pending validations, wait for a second before retry
      // FIXME: consider whether or not to make this configurable
      return delay(1000);
    }

    try {
      // wait for any validation to settle
      await Promise.race([...pendingValidations]);
      // validation resolved (did NOT reject), so consider this a success
      return;
    } finally {
      // validation rejected, keep waiting
    }
  }
}
...
function _canRetryValidation(e) {
  ...
  // these types of errors are all non-critical errors related to the
  // validation service being too busy or timing out
  return ((e.name === 'FetchError' && e.code === 'ECONNRESET') ||
    ...
    for(const eventRecord of current) {
      ...
      for(const parentHash of event.parentHash) {
        ...
        const {meta: {continuity2017: {
          ...
          localForkNumber: parentLocalForkNumber,
          forkDetectedBlockHeight: parentForkDetectedBlockHeight
        }}} = parentRecord;

        if(parentHash === event.treeHash) {
          // set event's generation and fork info based on its tree parent's
          eventRecord.meta.continuity2017.generation = parentGeneration + 1;
          eventRecord.meta.continuity2017.localForkNumber =
            parentLocalForkNumber;
          eventRecord.meta.continuity2017.forkDetectedBlockHeight =
            parentForkDetectedBlockHeight;
...
async function _checkForks({worker, mergeEvents, genesisHead}) {
  ...
  // 3. Add `forkDetectedBlockHeight`, if not `-1`, to the event's meta.
  for(const {event, meta} of mergeEvents) {
    ...
    // if `forkDetectedBlockHeight` is `-1`, it means the fork hasn't been
    // detected yet
    const {forkDetectedBlockHeight, treeHashSet} = forker;
    const {basisBlockHeight} = event;
    if(forkDetectedBlockHeight !== -1 &&
    ...
    if(treeHashSet.has(event.treeHash)) {
      ...
      // note: first `localForkNumber` for an actual fork will be `1`; which is
      // desirable
