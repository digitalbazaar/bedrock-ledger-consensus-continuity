...
async function _validateGraph({worker, eventMap}) {
  ...
  while(next.length > 0) {
    ...
    for(const eventRecord of current) {
      ...
      if(!_temp.processed) {
        ...
        eventRecord.meta.continuity2017.isLocalContributor =
          (operationSet.size > 0 || configSet.size > 0);
  ...
  // FIXME: concurrently with `_checkReplays` below, check that the
  // `lastLocalContributor` event referenced in all merge events with
  // `isLocalContributor` set to `true` has reached consensus
