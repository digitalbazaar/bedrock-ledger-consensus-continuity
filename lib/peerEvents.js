      ...
      for(const parentHash of event.parentHash) {
        ...
        if(!parentRecord) {
          ...
          if(!parentRecord) {
            ...
            console.log('PROTOCOL VIOLATION 2', parentHash);
            process.exit(1);
...
async function _checkParentHashCommitments({
  ...
  for(const eventRecord of mergeEvents) {
    ...
    if(blockHeight < (consensusState.blockHeight + 1)) {
      ...
      // FIXME: remove me
      //console.log('1 witnesses', blockHeight, witnesses);
  ...
  for(const eventHash of nonWitnessHashes) {
    ...
    if(commitmentBlockHeight === undefined &&
      ...
      if(eventRecord === lastEventRecord &&
        ...
        mustWithhold = true;
        // FIXME: remove me
        /*console.log('MUST WITHHOLD', {
          eventHash,
          creator: creator.substr(-4),
          remotePeerId: remotePeerId.substr(-4),
          witnesses: consensusState.witnesses
        });*/
        break;
      }
      // FIXME: remove me
      console.log('PROTOCOL VIOLATION 4', {
        nonWitnessEventMap,
        commitmentMap,
        conversionMap,
        creator: creator.substr(-4),
        lp: worker.localPeerId.substr(-4),
        rp: remotePeerId.substr(-4),
        eventHash
      });
      for(const {event, meta} of nonWitnessEventMap.values()) {
        console.log('nw', JSON.stringify({event, meta}, null, 2));
      }
      process.exit(1);
      throw new BedrockError(
  ...
  // FIXME: remove me
  //console.log('withheld', eventHash);
  const foo = worker.withheldCache.get(remotePeerId);
  if(foo) {
    console.log('ANOTHER WITHHELD EVENT', foo.mergeEvent.meta.eventHash);
    process.exit(1);
  }

  //console.log('event map size after', eventMap.size);
  /*const {witnesses} = await _witnesses.getBlockWitnesses(
    {blockHeight: withheld.mergeEvent.event.basisBlockHeight + 1,
      ledgerNode: worker.ledgerNode});
  if(witnesses.includes(withheld.mergeEvent.meta.continuity2017.creator)) {
    console.log('2 witnesses', withheld.mergeEvent.event.basisBlockHeight + 1,
      witnesses);
    console.log('456 WITNESS WITHHELD!',
  ...
  if((withheld.mergeEvent.event.basisBlockHeight + 2) <=
    ...
    const {witnesses} = await _witnesses.getBlockWitnesses(
      {blockHeight: withheld.mergeEvent.event.basisBlockHeight + 2,
        ledgerNode: worker.ledgerNode});
    if(witnesses.includes(withheld.mergeEvent.meta.continuity2017.creator)) {
      console.log('3 witnesses', withheld.mergeEvent.event.basisBlockHeight + 2,
        witnesses);
      console.log('ZZZ WITNESS WITHHELD!',
        withheld.mergeEvent.meta.continuity2017.creator.substr(-4));
      console.log('withheld', withheld);
      process.exit(1);
    }
