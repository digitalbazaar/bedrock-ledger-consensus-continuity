...
const LEDGER_NODE_WORK_SESSION_CACHE = new LRU({max: 100});
const WORK_SESSION_EXISTENCE_CACHE = new LruMemoize({max: 100});

const api = {};
module.exports = api;

api.validateEvent = _validateEvent;
api.validateOperation = _validateOperation;
api.validateSession = _validateSession;
...
async function _validateEvents({worker, events, neededHashes, remotePeerId}) {
  ...
  try {
    ...
    // requests to event validation service
    const tasks = [];
    const totalTaskCount = events.length;
    ...
    for(const event of events) {
      ...
      tasks.push({
        ...
        ledgerNodeId: worker.ledgerNodeId,
        localPeerId: worker.localPeerId,
        halt: () => worker.halt(),
        totalTaskCount,
        sharedState,
        validate: _client.validateEvent,
        getValidationServiceUrl: _client.getEventValidationServiceUrl
      });
    }
    ...
    let chunkSize;
    const concurrency = chunkSize = eventValidation.workers *
      eventValidation.concurrency;

    let results;
    try {
      results = await _util.processChunked({
        tasks,
        fn: _validateTaskViaService,
        concurrency,
        chunkSize
      });
    } catch(e) {
...
async function _validateTaskViaService({
  sessionId, ledgerNodeId, localPeerId, validationServiceUrl, basisBlockHeight,
  halt, task, totalTaskCount, sharedState, validate, getValidationServiceUrl
}) {
...
async function _validateEvent({event, ledgerNodeId, url, sessionId}) {
  const {valid} = await api.validateSession({ledgerNodeId, sessionId});
  if(!valid) {
    throw new BedrockError(
      'Permission Denied. Invalid Session.',
      'NotAllowedError', {
        httpStatusCode: 403,
        sessionId,
        public: true,
  ...
  if(meta.continuity2017.type === 'r') {
    ...
    const validationServiceUrl = origin +
      '/consensus/continuity2017/validation/operations';
    const totalTaskCount = processedEvent.operationRecords.length;
    const pendingValidations = new Set();
    const sharedState = {pendingValidations, highWaterMark: Infinity};
    const tasks = processedEvent.operationRecords.map(({operation}) => ({
      task: operation,
      sessionId,
      ...
      totalTaskCount,
      sharedState,
      validate: _client.validateOperation,
      getValidationServiceUrl: _client.getOperationValidationServiceUrl
    }));
    // concurrency should be calculated based on the number of workers
    // and the maximum number of concurrent validation operations each of
    // them should perform
    ...
    let chunkSize;
    const concurrency = chunkSize = operationValidation.workers *
      operationValidation.concurrency;
    ...
    try {
      ...
      logger.error(
        'An error occurred during operation validation.', {error: e});
      throw e;
    }
...
async function _validateOperation({
  operation, ledgerNodeId, basisBlockHeight, sessionId
}) {
  const {valid} = await api.validateSession({ledgerNodeId, sessionId});
  if(!valid) {
    throw new BedrockError(
...
async function _validateSession({ledgerNodeId, sessionId}) {
  // const valid = await WORK_SESSION_EXISTENCE_CACHE.memoize({
  //   key: `${ledgerNodeId}|${sessionId}`,
  //   fn: async () => _hasWorkSession({ledgerNodeId, sessionId})
  // });
  const valid = await _hasWorkSession({ledgerNodeId, sessionId});
  logger.info('valid', {ledgerNodeId, sessionId, valid});
  return {valid};
}

async function _hasWorkSession({ledgerNodeId, sessionId}) {
  // 1. Check if `sessionId` is found in the cache of all work sessions
  logger.info('_hasWorkSession', {
    ledgerNodeId,
    sessionId,
    hasSession: LEDGER_NODE_WORK_SESSION_CACHE.has(sessionId)
  });
  if(LEDGER_NODE_WORK_SESSION_CACHE.has(sessionId)) {
    logger.info('cache hit');
    return true;
  }
  // 2. Cache miss, query database for latest work session
  const {
    meta: {workSession}
  } = await brLedgerNode.getNodeRecord(null, ledgerNodeId);
  logger.info('workSession', {workSession});
  if(!(workSession && workSession.id)) {
    throw new Error(`LedgerNode Record with 'ledgerNode.id' "${ledgerNodeId}"` +
       `is missing 'workSession.id'`);
  }
  // 3. Reject `sessionId` when ledger node is claimed by scheduler. No work
  //    should be processed unless the ledger node is claimed by a valid work
  //    session.
  if(workSession.scheduler) {
    logger.info('scheduling mode', {workSession});
    return false;
  }
  // 4. The ledger node is claimed by a valid work session. Add to cache.
  const maxAge = workSession.expires - Date.now();
  LEDGER_NODE_WORK_SESSION_CACHE.set(workSession.id, true, maxAge);
  // 5. Validate the `sessionId` matches the current `workSession.id`
  logger.info('cache miss', {
    workSessionId: workSession.id,
    sessionId,
    maxAge,
    valid: workSession.id === sessionId
  });
  return workSession.id === sessionId;
}
...
async function _checkParentHashCommitments({
  ...
  for(const eventHash of nonWitnessHashes) {
    ...
    if(commitmentBlockHeight === undefined &&
      ...
      throw new BedrockError(
        'There was no valid commitment or witness conversion found for a ' +
