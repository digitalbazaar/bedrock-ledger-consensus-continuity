...
async function _validateParentMergeEvent({
  ...
  // FIXME: ensure
  // `parentRecord.meta.continuity2017.replayDetectedBlockHeight` <
...
async function _checkForks({worker, mergeEvents, genesisHead}) {
  ...
  for(const {event, meta} of mergeEvents) {
    ...
    // if `replayDetectedBlockHeight` is `-1`, it means the fork hasn't been
    // detected yet
    const {replayDetectedBlockHeight, treeHashSet} = forker;
    const {basisBlockHeight} = event;
    ...
    if(treeHashSet.has(event.treeHash)) {
      // 2. Event is a new fork, increment `localReplayNumber` and add it.
      // note: first `localReplayNumber` for an actual fork will be `1`; which
      // is desirable
      meta.continuity2017.localReplayNumber = ++forker.localReplayNumber;
    }
    // 3. Add `replayDetectedBlockHeight` if any.
    if(replayDetectedBlockHeight !== -1) {
      meta.continuity2017.replayDetectedBlockHeight = replayDetectedBlockHeight;
    }
  }
}
