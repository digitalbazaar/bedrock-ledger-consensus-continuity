...
async function _validateSession({ledgerNodeId, sessionId}) {
  logger.debug('_validateSession', {ledgerNodeId, sessionId});
  const workSession = await _getLedgerNodeWorkSession({ledgerNodeId});
  logger.debug('_validateSession:workSession', {workSession});
  const valid = sessionId === workSession.id;
  logger.debug('_validateSession:workSession',
    {workSessionId: workSession.id, sessionId});
  return {valid};
}

async function _getLedgerNodeWorkSession({ledgerNodeId}) {
  const workSession = LEDGER_NODE_WORK_SESSION_CACHE.get(ledgerNodeId);
  logger.debug('_getLedgerNodeWorkSession', {workSession});
  // if the ledger node's current work session is not expired
  logger.debug('_getLedgerNodeWorkSession.expires', {
    workSession,
    now: Date.now(),
    expires: (workSession || {}).expires,
    notExpired: workSession ? Date.now() < workSession.expires : 'none'
  });
  if(workSession && Date.now() < workSession.expires) {
    return workSession;
  }

  // delete stale work session if it exists
  if(workSession) {
    logger.debug('delete stale work session if it exists', {
      workSession,
    });
    LEDGER_NODE_WORK_SESSION_CACHE.del(ledgerNodeId);
  }

  const {meta} = await brLedgerNode.getNodeRecord(null, ledgerNodeId);
  if(!(meta.workSession && meta.workSession.id)) {
    throw new Error(`LedgerNode Record with 'ledgerNode.id' "${ledgerNodeId}"` +
       `is missing 'meta.workSession.id'`);
  }

  LEDGER_NODE_WORK_SESSION_CACHE.set(ledgerNodeId, meta.workSession);
  logger.debug('LEDGER_NODE_WORK_SESSION_CACHE.set', {
    'meta.workSession': meta.workSession,
  });
  return meta.workSession;
}
