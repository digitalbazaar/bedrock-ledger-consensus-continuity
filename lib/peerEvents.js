...
async function _createPeerMergeEventRecord({event, ledgerNode}) {
  ...
  const meta = {
    ...
    continuity2017: {
      creator,
      // generation, isLocalContributor, localAncestorGeneration,
      // localReplayNumber, and replayDetectedBlockHeight will be updated later
      generation: null,
...
async function _validateGraph({worker, eventMap}) {
  ...
  while(next.length > 0) {
    ...
    for(const eventRecord of current) {
      ...
      for(const parentHash of event.parentHash) {
        ...
        // FIXME: ensure the last non-empty merge event in a given lineage is
        // tracked so it can be later checked for consensus
  ...
  // FIXME: concurrently with `_checkReplays`, check that all gathered
  // non-empty merge events have a most recent ancestor non-empty merge
  // event that has reached consensus (do this in a single aggregate call)
  ...
  await _checkReplays({worker, mergeEvents, genesisHead, replayMap});
}
...
async function _getOperationReplayers({
  ...
  for(const summary of operationSummaries) {
    // FIXME: include `basisBlockHeight` for the set of operations if
    // it is decided that `basisBlockHeight` must be part of an operation
    const {creator, /*basisBlockHeight,*/ operationHashes} = summary;
    $or.push({
      'operation.creator': creator,
      //'operation.basisBlockHeight': basisBlockHeight,
