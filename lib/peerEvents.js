...
async function _validateGraph({worker, eventMap}) {
  ...
  // FIXME: pass in "commitments" from the batch and add them to the
  // `outsideBatchHashes`
  ...
  while(next.length > 0) {
    ...
    for(const eventRecord of current) {
      ...
      if(!_temp.processed) {
        ...
        // FIXME: if event is to be withheld, build a `withheld` event record
        // and delete the event and its regular event parents from the
        // `eventMap`
        // FIXME: consider using `deferred` instead of `withheld` terminology
      }
    }

    // FIXME: return {withheld};
  }

  /* Now that the event lineages have been validated, make sure that local
  contribution rules and replay rules are not violated.
