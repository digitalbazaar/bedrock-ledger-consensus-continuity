...
api.addBatch = async ({worker, events, neededHashes, remotePeerId}) => {
  ...
  const {valid, error, eventMap, withheld} = await _validateEvents(
    {worker, events, neededHashes, remotePeerId});
  if(!valid) {
    return {valid, error, mergeEventsReceived, withheld};
  }
...
async function _validateEvents({worker, events, neededHashes, remotePeerId}) {
  ...
  try {
    ...
    return {valid: false, error: e, eventMap, withheld};
  }

  return {valid: true, error: null, eventMap, withheld};
}
...
async function _createPeerConfigurationEventRecord({event}) {
  ...
  const meta = {
    blockHeight: -1,
    consensus: false,
    continuity2017: {type: 'c', replayDetectedBlockHeight: -1},
    eventHash
  };
...
async function _createPeerRegularEventRecord({event, ledgerNode}) {
  ...
  const meta = {
    blockHeight: -1,
    consensus: false,
    continuity2017: {type: 'r', replayDetectedBlockHeight: -1},
    eventHash
  };
  // event not validated yet, try to add `creator` to meta
...
async function _validateGraph({worker, eventMap, remotePeerId}) {
  ...
  contribution rules, parent hash commitment rules, and replay rules are not
  violated.
...
async function _checkParentHashCommitments({
  ...
  const {ledgerNode, consensusState: {blockHeight}} = worker;

  // gather hashes for all non-witness merge events
  ...
  for(const eventRecord of mergeEvents) {
    ...
    // get witnesses that are applicable to the given event, which is always
    // computed using a `blockHeight` that is `1` more than the
    // `basisBlockHeight` of the event
    ...
    // track non-witness events
    if(!witnesses.includes(creator)) {
      nonWitnessEventMap.set(eventHash, eventRecord);
    }
  }

  // check database for commitments for all non-witness merge events
  const nonWitnessHashes = [...nonWitnessEventMap.keys()];
  const results = await _getParentHashCommitments({
    ledgerNode, nonWitnessHashes, blockHeight
  });

  // ensure the results are all `valid` and obtain a withheld event, if any
  let withheld = null;
  const validCommitments = new Set();
  for(const {parentHashCommitment, valid} of results) {
    if(valid.includes(true)) {
      validCommitments.set(parentHashCommitment);
    }
  }

  // all non-witness hashes have valid commitments
  if(validCommitments.size === nonWitnessHashes.length) {
    return {withheld};
  }

  // if there is more than one event lacking a valid commitment, then it
  // is a protocol violation
  if(validCommitments.size < (nonWitnessHashes.length - 1)) {
    // FIXME: enable
    /*throw new BedrockError(
      'There were insufficient valid commitments for the non-witness merge ' +
      'events in the batch.', 'DataError', {
        validCommitments: validCommitments.size,
        requiredCommitments: nonWitnessHashes.length - 1
      });*/
  }

  // the event that is lacking a valid commitment MUST be from the remote peer
  for(const eventRecord of nonWitnessEventMap.values()) {
    const {
      event: {parentHash},
      meta: {continuity2017: {creator}, eventHash}
    } = eventRecord;
    if(validCommitments.has(eventHash)) {
      continue;
    }
    if(creator !== remotePeerId) {
      // FIXME: enable
      /*throw new BedrockError(
        'There was no valid commitment found for a non-witness merge ' +
        'event was not created by the remote peer.',
        'DataError', {eventHash, creator, remotePeerId});*/
    }
    // build withheld information
    withheld = {
      regularEvents: [],
      mergeEvent: eventRecord
    };
    // FIXME: delete the merge event and its regular events from `eventMap`

    // at this point, it has already been verified that every regular event
    // is in the batch and therefore in the `eventMap`
...
async function _getParentHashCommitments({
  ...
  const cursor = collection.aggregate([
    ...
    // group by committed hash; at most two values for `valid` true/false can
    // be returned and we must see `true` for every commitment
    {
      $group: {
        _id: '$parentHashCommitment',
        valid: {$addToSet: '$valid'}
      }
    },
    // map `_id` to `parentHashCommitment`
    {
      $project: {
        _id: 0,
        parentHashCommitment: '$_id',
        valid: '$valid'
      }
    }
  ], {allowDiskUse: true});
