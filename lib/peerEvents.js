...
api.addBatch = async ({worker, events, needed}) => {
  ...
  for(const {event, meta, _temp} of eventMap.values()) {
    ...
    if(!valid) {
      ...
      // algorithm that ensures we breadth-first validate
      continue;
    }
...
async function _validateEvents({worker, events, needed}) {
