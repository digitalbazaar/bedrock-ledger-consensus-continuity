...
const SESSION_VALIDATION_CACHE = new LruCache({
  max: 100,
  disposeOnGet: true
});
  ...
  try {
    ...
    try {
      results = await _util.processChunked({
        tasks,
        fn: _client.validateTaskViaService,
        concurrency,
        chunkSize
      });
    } catch(e) {
...
api.validateEvent = async opts => {
  const {ledgerNodeId, sessionId} = opts;
  ...
  return _validateEvent(opts);
};

async function _validateEvent({event, ledgerNodeId}) {
  ...
  if(meta.continuity2017.type === 'r') {
    const fn = async ({operation}) => _validateOperation({
      operation,
      ledgerNode,
      basisBlockHeight
    });
...
api.validateOperation = async opts => {
  const {ledgerNodeId, sessionId} = opts;
  const {valid} = await api.validateSession({ledgerNodeId, sessionId});
  if(!valid) {
    throw new BedrockError(
  ...
  return _validateOperation(opts);
};

async function _validateOperation({
  operation, ledgerNode, ledgerNodeId, basisBlockHeight
}) {
  if(!ledgerNode) {
...
async function _validateSession({ledgerNodeId, sessionId}) {
  const valid = await SESSION_VALIDATION_CACHE.memoize({
