  ...
  try {
    ...
    for(const event of events) {
      ...
      if(!(Number.isInteger(basisBlockHeight) &&
        ...
        // FIXME: we should throw here when this happens, the server should
        // NOT send us data we didn't ask for and a peer that does that should
        // not be gossipped with, it is violating protocol, we should not even
        // bother validating the events
        continue;
      }
    ...
    await _validateGraph({worker, eventHashes, eventMap, ledgerNode});
  } catch(e) {
...
async function _validateGraph({worker, eventHashes, eventMap, ledgerNode}) {
  ...
  const outsideBatchEvents = await _events.getEvents(
    {worker, eventHash: outsideBatchHashes, ledgerNode});
