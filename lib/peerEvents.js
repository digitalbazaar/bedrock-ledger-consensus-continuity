...
api.addBatch = async ({worker, events, neededHashes}) => {
  ...
  // FIXME: write withheld events to worker

  // FIXME: return whether or not events were withheld
...
async function _validateEvents({worker, events, neededHashes}) {
  ...
  try {
    ...
    // FIXME: get `withheld` events and return them
    ...
    return {valid: false, error: e, eventMap};
  }

  return {valid: true, error: null, eventMap};
}
...
async function _validateGraph({worker, eventMap}) {
  ...
  const [{withheld}] = await Promise.all([
    _checkParentHashCommitments({worker, mergeEvents, eventMap}),
...
async function _checkParentHashCommitments({
  ...
  for(const eventRecord of mergeEvents) {
    ...
    if(witnesses.includes(creator)) {
      nonWitnessHashes.push(eventHash);
    }
  }
  ...
  let requiredCommitments = nonWitnessHashes.length;
  for(const {/*parentHashCommitment,*/ valid} of results) {
    if(valid.includes(true)) {
      validCommitments++;
      continue;
    }
    // FIXME: if the commitment is not valid, determine if the merge event
    // came from the remote peer -- in which case the merge event and all
    // of its regular events should become a withheld event; but there can
    // be only one of these per batch or it is a protocol violation
    withheld = null;
    // FIXME: decrement `requiredCommitments` by 1 if `withheld` is set
    requiredCommitments--;
  }

  if(validCommitments < requiredCommitments) {
    // FIXME: enable
    /*throw new BedrockError(
      'Not all of the non-witness merge events in the batch have valid ' +
      'commitments that have reached consensus.',
      'DataError', {validCommitments, requiredCommitments});*/
  }
