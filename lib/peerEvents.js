...
async function _validateGraph({worker, eventMap}) {
    ...
    for(const eventRecord of current) {
      ...
      for(const parentHash of event.parentHash) {
        ...
        if(parentHash === event.treeHash) {
          // set event's generation and replay info based on its tree parent's
        ...
        if(parentType === 'm') {
          ...
          // FIXME: this is an error, all non-merge event parents MUST be
          // present in the batch; ensure valid batches are written in an
          // ACID transaction or drop dangling regular events at worker
          // init to prevent violating this client side
  ...
  // they do not descend from a detected replayer, and if not, add any replay
  // meta data to any events that represent a replay that is not yet detected;
  ...
  // replay checks to erroneously flag a well-behaved peer as a replayer
...
async function _validateParentNonMergeEvent({
  ...
  if(parentType === 'r') {
    // FIXME: operation hashes must not repeat
...
// FIXME: determine where operation replay is to be detected; also, what
// about regular event replay detection? ... any replayed regular event
// is necessarily an operation (or configuration) replay, so perhaps it is
...
async function _checkForks({worker, mergeEvents, genesisHead}) {
  ...
  creator has already been detected as a forker through the consensus process.

  If not, we need to determine whether any forks have occurred in the batch
  itself or if the batch will introduce any new forks based on what is already
  in the database. Either way, we cannot immediately reject the forks until
  they have been detected via the consensus process.
  ...
  for(const eventRecord of mergeEvents) {
    const {
      event,
      meta: {continuity2017: {creator, replayDetectedBlockHeight}}
    } = eventRecord;
  ...
  for(const latestReplay of latestForks) {
    const {peerId, localReplayNumber} = latestReplay;
    const forker = forkerMap.get(peerId);
    forker.localReplayNumber = localReplayNumber;
  }

  // for all merge events in the batch, if its creator is a forker and the
  // event's `treeHash` is in the forker's `treeHashSet`, increment
  // `localReplayNumber` and add it to the event's meta
  for(const {event, meta} of mergeEvents) {
    ...
    if(treeHashSet.has(event.treeHash)) {
      // event is a new fork, increment `localReplayNumber` and add it; note
      // that first `localReplayNumber` for an actual fork will be `1`; which
      // is desirable
      meta.continuity2017.localReplayNumber = ++forker.localReplayNumber;
    }
  }
}
