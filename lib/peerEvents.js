...
async function _checkReplays({worker, mergeEvents, genesisHead, replayMap}) {
  ...
  for(const eventRecord of mergeEvents) {
    ...
    // check if operations or configs have been replayed within the batch
  ...
  const [forkers, operationReplayers, configReplayers] = await Promise.all([
    ...
    // FIXME: create `operationSummaries` above
    _getOperationReplayers({
      ledgerNode, operationSummaries: []
    }),
    // FIXME: create `configSummaries` above
    _getConfigReplayers({
      ledgerNode, configSummaries: []
    })
  ]);
...
async function _getOperationReplayers({
  ...
  // FIXME: need this query to do creator+basisBlockHeight+operationHash,
  // which means we need to pass that data in

  // FIXME: make this a covered query
  const $or = [];
  for(const summary of operationSummaries) {
    // FIXME: include `basisBlockHeight` for the set of operations
