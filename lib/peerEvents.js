  ...
  while(!worker.halt()) {
    ...
    try {
      ...
      if(!_canRetryValidation(e)) {
        const validationService = _client.getEventValidationServiceUrl({
          localPeerId
        });
...
async function _validateEvent({event, ledgerNodeId, url, session}) {
  const {valid} = await api.validateSession({ledgerNodeId, session});
  if(!valid) {
    throw new BedrockError(
...
async function _validateOperation({
  operation, ledgerNodeId, basisBlockHeight, session
}) {
  const {valid} = await api.validateSession({ledgerNodeId, session});
  if(!valid) {
    throw new BedrockError(
...
async function _validateSession({ledgerNodeId, session}) {
  const workSession = await _getLedgerNodeWorkSession({ledgerNodeId});
  const valid = session === workSession.id;
  return {valid};
}
