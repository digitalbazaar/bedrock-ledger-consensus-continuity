...
api.aggregateHistory = ({
  creatorFilter, eventHashFilter, eventTypeFilter, ledgerNode, startHash,
  treeHash
}, callback) => {
  logger.debug('Start aggregateHistory');
  const collection = ledgerNode.storage.events.collection;
  const restrictSearchWithMatch = {$or: []};
  // FIXME: optimize conditionals
  if(Array.isArray(eventHashFilter) && eventHashFilter.length !== 0
    && treeHash) {
    restrictSearchWithMatch.$or.push(
      {eventHash: {$nin: eventHashFilter.concat(treeHash)}});
  } else if(Array.isArray(eventHashFilter) && eventHashFilter.length !== 0) {
    restrictSearchWithMatch.$or.push(
      {eventHash: {$nin: eventHashFilter}});
  } else if(treeHash) {
    restrictSearchWithMatch.$or.push({eventHash: {$ne: treeHash}});
  }
  if(Array.isArray(creatorFilter) && creatorFilter.length !== 0) {
    restrictSearchWithMatch.$or.push({
      meta: {continuity2017: {creator: {$nin: creatorFilter}}}
    });
  }
  if(eventTypeFilter) {
  ...
  // },
  if(restrictSearchWithMatch.$or.length === 0) {
    delete restrictSearchWithMatch.$or;
  }
...
api.getHistory = ({
  creatorFilter, creatorId, eventHashFilter, eventTypeFilter, ledgerNode,
  treeHash
}, callback) => {
  ...
  async.auto({
    ...
    if(!results.firstDescendant) {
      logger.debug('No firstDescendant found, getHistory returns [].');
      return callback(null, []);
    }
