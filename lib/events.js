 ...
 */
api.getRecentHistory = ({ledgerNode}, callback) => {
  const storage = ledgerNode.storage;
...
api.mergeBranches = (ledgerNode, callback) => {
  // FIXME: these collections should be retained and exposed via events API?
  const eventMap = {};
  const events = [];
  const storage = ledgerNode.storage;
  const event = {
  ...
  async.auto({
    ...
    events: ['creator', 'treeHash', (results, callback) => {
      ...
      storage.events.collection.find(query)
        .forEach(doc => {
          doc._children = [];
          events.push(doc);
    ...
    process: ['events', (results, callback) => {
      ...
      event.parentHash = [
        event.treeHash,
        ...events.filter(e => !e._children.length).map(e => e.eventHash)
      ];
      callback();
    }],
    sign: ['process', (results, callback) => jsigs.sign(event, {
    ...
    write: ['eventHash', (results, callback) => {
      ...
      // console.log('LLLLLL', JSON.stringify(results.sign, null, 2));
