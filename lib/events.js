...
api.processor = require('./eventProcessor');

api._addTestEvent = async ({event, ledgerNode}) => {
  const ledgerNodeId = ledgerNode.id;
  const eventMap = new Map();
  const {event: processedEvent, meta} = await api.processor.processPeerEvent(
...
api.addBatch = async ({
  ...
  const {eventMap} = await api.processor.validateEvents({
    blockHeight, events, ledgerNode, needed
  });
...
api.prepBatch = async ({events, ledgerNode, needed}) => {
  ...
  const {eventMap} = await api.processor.processBatch(
...
async function _writeEvent({event, eventHash, genesis, ledgerNode}) {
  ...
  if(genesis) {
    ...
    eventRecord = await api.processor.processLocalEvent(
      {event, eventHash, ledgerNode});
  }
