...
function _processPeerRegularEvent({event, ledgerNode, needed}, callback) {
  ...
  async.auto({
    operationHash: callback => async.map(
      ops, (operation, callback) => _util.hasher(
        operation, (err, operationHash) => {
          ...
          operationRecords.push({meta: {operationHash}, operation, recordId});
    ...
    for(let i = 0; i < operationRecords.length; ++i) {
      ...
      meta.eventHash = ledgerNode.storage.driver.hash(eventHash);
      meta.eventOrder = i;
    }
...
function _processPeerEvent({event, ledgerNode, needed}, callback) {
  async.auto({
    ...
    process: ['checkAncestors', (results, callback) => {
      ...
      _processPeerRegularEvent({event, ledgerNode, needed}, callback);
    }]
