...
api.addBatch = async ({events, ledgerNode, needed}) => {
  ...
  // identify merge events, then validate the regular event ancestors
  // ensure that `operation.creator` in WebLedgerOperationEvent is proper
  // ensure that `event.creator` in WebLedgerConfigurationEvent is proper
  for(let i = eventHashes.length - 1; i >= 0; --i) {
    ...
    if(meta.continuity2017.type === 'm') {
      ...
      for(const ancestorHash of event.parentHash) {
        ...
        ancestor = eventMap.get(ancestorHash);
        if(!ancestor) {
          outsideBatch = true;
          [ancestor] = await api.getEvents(
            {eventHash: ancestorHash, ledgerNode});
        }
        const {meta: {continuity2017: {creator, generation, type}}} = ancestor;
        let treeHash;
        if(generation !== 0) {
          ({event: {treeHash}} = ancestor);
        }
        // the only ancestors that are not in the batch must be merge events
        if(outsideBatch && type !== 'm') {
          throw new BedrockError(
            'Invalid ancestor detected.',
            'DataError', {
              ancestor: ancestor.event,
              event,
              httpStatusCode: 400,
              public: true,
            });
        ...
        if(type !== 'm') {
          ...
          if(type === 'r') {
            ...
            if(ancestor.event.creator !== requiredCreator) {
              throw new BedrockError(
                '`event.creator` must correspond to the creator of ' +
                ' the merge event.', 'DataError', {
                  event: ancestor.event,
                  httpStatusCode: 400,
                  mergeEvent: event,
  ...
  eventMap.forEach(record => {
    if(!record._temp.valid) {
      throw new BedrockError(
        'An unnecessary event was detected in the batch.', 'DataError', {
          event: record.event,
          httpStatusCode: 400,
          public: true,
        });
...
async function _writeEvent({
  ...
  let eventRecord;
  // FIXME: remove, peer events no longer pass here.
  if(fromPeer) {
