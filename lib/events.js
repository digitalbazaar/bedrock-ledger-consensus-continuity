  ...
  async.auto({
    ...
    process: ['events', (results, callback) => {
      for(let i = 0; i < events.length; ++i) {
        if(events[i].event.type.includes('ContinuityMergeEvent')) {
          const parents = events[i].event.parentHash;
          for(let j = 0; j < parents.length; ++j) {
            if(!eventMap[parents[j]]) {
              continue;
            }
            events[i]._parents.push(eventMap[parents[j]]);
            eventMap[parents[j]]._children.push(events[i].eventHash);
          }
        }
      }
  ...
  async.auto({
    ...
    history: ['creator', (results, callback) => api.getRecentHistory(
      {creatorId: results.creator.id, ledgerNode}, (err, result) => {
        ...
        event.parentHash = [
          event.treeHash,
          ...result.events.filter(e => !e._children.length)
            .map(e => e.eventHash)
        ];
        // FIXME: sometimes treeHash is being duplicated, removing dup
