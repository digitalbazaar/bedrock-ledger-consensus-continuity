...
api.aggregateHistory = ({
  creatorFilter, creatorRestriction, eventTypeFilter, fullEvent = true,
...
api.getRecentHistory = (
  ...
  async.auto({
    creator: callback => {
      ...
      voters.get({ledgerNodeId: ledgerNode.id}, (err, result) => {
        if(err) {
          return callback(err);
        }
        creatorId = result.id;
...
api.partitionHistory = (
  {creatorHeads, creatorId, eventTypeFilter, fullEvent = true, peerId},
  callback) => {
  ...
  async.auto({
    ...
    startHash: ['localHeads', 'genesisMerge', (results, callback) => {
      ...
      logger.debug('Truncating history, peer is catching up.', {
        localNodeHead, maxDepth, peerLocalNodeHead
      });
...
api.getHeads = ({eventHashes, ledgerNode}, callback) => {
  ...
  async.auto({
    ...
    headGenerationMap.forEach(v => {
      if(v === null) {
        throw new Error('444444444444');
      }
    });
...
api._getLocalBranchHead = ({creatorId, ledgerNode}, callback) => {
  ...
  async.auto({
    headCache: callback => cache.client.hmget(
      headCacheKey, 'h', 'g', (err, result) => {
        if(err) {
          return callback(err);
        }
...
function _processPeerMergeEvent({event, ledgerNode}, callback) {
  async.auto({
    ...
    callback(null, {creator: results.signature.creator});
  });
}
...
function _processPeerEvent({event, ledgerNode}, callback) {
  ...
  _processPeerMergeEvent({event, ledgerNode}, (err, result) => {
    ...
    const _meta = {continuity2017: {creator}};
...
function _writeEvent(ledgerNode, event, options, callback) {
  ...
  async.auto({
    ...
    count: ['writeEvent', (results, callback) => {
      // peer events are counted in the EventWriter
      if(fromPeer) {
        return callback();
      }
      const eventCountCacheKey = _cacheKey.eventCountLocal({
        ledgerNodeId: ledgerNode.id,
        second: Math.round(Date.now() / 1000)
      });
      cache.client.incr(eventCountCacheKey, callback);
    }]
  }, (err, results) => {
    if(err) {
      return callback(err);
