...
api.addBatch = async ({events, ledgerNode, neededSet}) => {
  // if this does not throw, we have a cryptographically validated merge event
  ...
  // `treeHash` must already be in cache/storage
  const eventMap = new Map();
  const eventHashes = [];
  for(const event of events) {
    const {event: processedEvent, meta} = await _processPeerEvent(
      {event, eventMap, ledgerNode, neededSet});
    eventHashes.push(meta.eventHash);
    eventMap.set(meta.eventHash, {
        ...
        if(ancestor) {
          ...
          if(type !== 'm') {
            if(treeHash !== mergeTreeHash) {
              ...
              console.log('2222222222222222222222', !!event);
              if(!event) {
                event = await api.getEvents(
                  {ledgerNodeId: ledgerNode.id, eventHash: treeHash});
                console.log('333333333333333333');
              }
              console.log('5', requiredCreator, JSON.stringify(event, null, 2));
...
api.getEvents = async (
  {eventHash, includeMeta = false, ledgerNodeId, stringify = false}) => {
  eventHash = [].concat(eventHash);

  const events = await _cache.events.getEvents(
    {eventHash, includeMeta, ledgerNodeId});
  // if all events were found in the cache, return them and end
  console.log('PPPPPPPPPPPPPP', events);
  const eventStrings = events.map(e => e.event);
  if(!eventStrings.includes(null)) {
    return stringify ? eventStrings : eventStrings.map(JSON.parse);
  }
  ...
  for(let i = 0; i < eventHash.length; ++i) {
    const event = stringify ? events[i] : JSON.parse(events[i]);
  ...
  await fn(({event}) => {
    const e = stringify ? JSON.stringify({event}) : {event};
    hashMap.set(needed[counter], e);
    counter++;
  });
