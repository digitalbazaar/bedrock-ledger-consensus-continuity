...
api.aggregateHistory = ({
  ...
  // FIXME: optimize conditionals / remove
  ...
  // FIXME: remove
  // treeHash restriction is now handled by a creatorRestriction
  // if(treeHash) {
  //   restrictSearchWithMatch.$nor.push({eventHash: treeHash});
  // }
  ...
  // console.log('AAAAAAa', 'Aggregate history parameters', JSON.stringify(restrictSearchWithMatch, null, 2), startHash);
  // logger.debug(
...
api.getHistory = ({
  creatorFilter, creatorRestriction, creatorId, eventHashFilter,
  eventTypeFilter, ledgerNode, startHash, treeHash
}, callback) => {
