...
// FIXME: documentation
api.mergeBranchesNEW = ({ledgerNode, history}, callback) => {
  const storage = ledgerNode.storage;
  const event = {
    '@context': config.constants.WEB_LEDGER_CONTEXT_V1_URL,
  ...
  async.auto({
    ...
    creator: callback => voters.get(ledgerNode.id, callback),
    history: callback => {
      // FIXME: move this check outside of this function -- it needs to be
      // run in the gossip worker (along with other checks) to determine
      // if this function (mergeBranches) should even be run
      const eventsWithNoChildren = history.events
        .filter(e => !e._children.length)
        .map(e => e.eventHash);
      if(eventsWithNoChildren.length === 1 &&
        eventsWithNoChildren[0] === history.localBranchHead) {
        return callback(new BedrockError(
          'No events found since the last merge event.',
          'NotFoundError', {
            httpStatusCode: 400,
      ...
      if(eventsWithNoChildren.includes(event.treeHash)) {
        event.parentHash = eventsWithNoChildren;
      } else {
        event.parentHash = [event.treeHash, ...eventsWithNoChildren];
      }
      callback();
    },
    sign: ['history', (results, callback) => jsigs.sign(event, {
    ...
    write: ['eventHash', (results, callback) => {
      ...
      console.log('merge eventHash', eventHash);
      console.log('merge collection', storage.events.collection.s.name);
...
api.mergeBranches = ({ledgerNode}, callback) => {
  ...
  async.auto({
    // FIXME: maybe creator will be supplied as a parameter?
    creator: callback => voters.get(ledgerNode.id, callback),
    history: ['creator', (results, callback) => api.getRecentHistory(
      {creatorId: results.creator.id, ledgerNode}, (err, result) => {
        if(err) {
          return callback(err);
        }
        const eventsWithNoChildren = result.events
          .filter(e => !e._children.length)
          .map(e => e.eventHash);
        if(eventsWithNoChildren.length === 1 &&
          eventsWithNoChildren[0] === result.localBranchHead) {
          return callback(new BedrockError(
            'No events found since the last merge event.',
            'NotFoundError', {
              httpStatusCode: 400,
              localBranchHead: result.localBranchHead,
              public: true,
            }));
        }
        // if no local events have been added, it is correct that
        // localBranchHead would be included along with events from other nodes
        event.treeHash = result.localBranchHead;
        if(eventsWithNoChildren.includes(event.treeHash)) {
          event.parentHash = eventsWithNoChildren;
        } else {
          event.parentHash = [event.treeHash, ...eventsWithNoChildren];
