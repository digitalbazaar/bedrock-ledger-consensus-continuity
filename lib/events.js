    ...
    localMerge: ['headCache', (results, callback) => {
      ...
      eventsCollection.find(query, projection)
        ...
        .limit(1).toArray((err, result) => {
          ...
          console.log('LLLLLLLLLLLLLL', eventHash, generation);
          cache.client.mset(
...
function _processPeerMergeEvent({event, ledgerNode}, callback) {
  async.auto({
    ...
    head: ['signature', (results, callback) => {
      console.log('TTTTTTTTTTTTTTTTt', results.signature.creator);
      api._getLocalBranchHead(
        {creatorId: results.signature.creator, ledgerNode}, (err, result) => {
