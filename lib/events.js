...
function _processLocalEvent({event, ledgerNode}, callback) {
  ...
  // const meta = {continuity2017: {}};
  async.auto({
    creatorId: callback => voters.get(ledgerNodeId, (err, result) => {
      if(err) {
        return callback(err);
      }
    ...
    head: ['creatorId', (results, callback) => api._getLocalBranchHead(
      {creatorId: results.creatorId, ledgerNode}, (err, result) => {
        if(err) {
          return callback(err);
        }
        // add the hash of the local merge event as the treeHash of this event
        _event.treeHash = result.eventHash;
        _event.parentHash = [result.eventHash];
        callback();
      })],
    validate: ['head', (results, callback) =>
      validate('continuity.webLedgerEvents', results.head, callback)],
  }, err => err ? callback(err) : callback(null, {event: _event, meta: {}}));
}
...
function _processPeerEvent({event, ledgerNode}, callback) {
  ...
  async.auto({
    signature: callback => {
      // only check signatures on ContinuityMergeEvent from peers
    ...
    hasMerge: ['signature', (results, callback) => {
      ...
      const d = event.parentHash.length - _.uniq(event.parentHash).length;
      if(d !== 0) {
        throw new Error('1111');
      }
      // console.log(
      //   'DIFF`', event.parentHash.length, _.uniq(event.parentHash).length);
  ...
  }, err => err ? callback(err) : callback(null, {event, meta: _meta}));
}
...
function _writeEvent(ledgerNode, event, options, callback) {
  ...
  console.log('TTTTTTTTTTTT', event, options);
  async.auto({
    ...
    eventHash: ['eventValidator', (results, callback) => {
      const {event} = results.process;
    ...
    writeEvent: ['eventHash', (results, callback) => {
      ...
      if(!fromPeer) {
        console.log('EEEEEE', event.treeHash);
      }
      // FIXME: disable write queue
      // if(fromPeer) {
