...
api.addBatch = async ({events, ledgerNode, neededSet}) => {
  ...
  for(let i = eventHashes.length - 1; i >= 0; --i) {
    ...
    if(meta.continuity2017.type === 'm') {
      ...
      const {treeHash: requiredTreeHash} = event;
      ...
      for(const ancestorHash of event.parentHash) {
        ...
        if(ancestor) {
          ...
          if(type !== 'm') {
            if(treeHash !== requiredTreeHash) {
              console.log('+++++++++++++++++++++++++++++++++++++', ancestor);
              // const bar = await api.getEvents(
              //   {ledgerNodeId: ledgerNode.id, eventHash: treeHash});
              // console.log('^^^^^^^^^^', requiredCreator, JSON.stringify(bar, null, 2));
  ...
  for(let i = 0; i < eventHashes.length; ++i) {
    ...
    await _cache.events.setEventGossip(
      {event: events[i], eventHash, ledgerNodeId});
...
api.getEvents = async (
  {eventHash, ledgerNodeId, stringify = false}) => {
  ...
  if(!events.includes(null)) {
    return stringify ? events : events.map(JSON.parse);
  }
