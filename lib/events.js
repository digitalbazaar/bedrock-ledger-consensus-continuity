...
api.getEventsForGossip = async ({eventHashes, ledgerNodeId}) => {
  ...
  const toFetch = [];
  // this promise will be awaited in `_deferredGetEventsForGossip`
  // in the next tick, which will allow the loop below to fill the
  // `toFetch` array with event hashes that do not yet have a pending promise
...
api.getEvents = async ({worker, eventHash, ledgerNode}) => {
  let eventHashes;
