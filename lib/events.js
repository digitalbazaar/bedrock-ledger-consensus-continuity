...
api.addBatch = async ({
  ...
  for(let i = 0; i < eventHashes.length; ++i) {
    ...
    if(_util.hasValue(event, 'type', 'WebLedgerOperationEvent')) {
    ...
    } else if(_util.hasValue(event, 'type', 'WebLedgerConfigurationEvent')) {
    ...
    } else if(_util.hasValue(event, 'type', 'ContinuityMergeEvent') &&
...
async function _processLocalEvent({event, eventHash, ledgerNode}) {
  ...
  if(_util.hasValue(event, 'type', 'WebLedgerConfigurationEvent')) {
...
async function _processPeerEvent({event, eventMap, ledgerNode}) {
  ...
  if(_util.hasValue(event, 'type', 'ContinuityMergeEvent')) {
