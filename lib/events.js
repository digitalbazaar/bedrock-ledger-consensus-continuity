...
bedrock.events.on('bedrock.init', () => {
  ...
  // that must be executed against specific ledger node storage; if a promise
  // was shared between two or more ledger nodes with different storage, it
  // would resolve with the wrong result
...
api.create = async ({ledgerNode, worker}) => {
  ...
  const {basisBlockHeight, hasMore, operations} = await queue.getNextChunk();
...
api.getNonConsensusEvents = async ({ledgerNode, localPeerId, head}) => {
  ...
  for(const {event, meta} of result) {
    const {
      ...
      continuity2017: {
        ...
        forkNumber = null, forkDetectedBlockHeight = null
      }
    } = meta;
    ...
    if(meta.continuity2017.type === 'm') {
      ...
      historyMap.set(eventHash, {
        ...
        meta: {
          continuity2017: {
            creator, generation, forkNumber, forkDetectedBlockHeight
          }
        }
      });
    }
...
async function _writeEvent({event, eventHash, genesis, ledgerNode, worker}) {
  ...
  if(genesis) {
    // FIXME: set `meta.continuity2017.eventNumber = 1`
    ...
    // FIXME: throw if `worker` is not set, pass `worker` here
...
async function _createLocalEventRecord({event, eventHash, ledgerNode}) {
  ...
  const meta = {
    ...
    eventHash,
    // FIXME: set `meta.continuity2017.eventNumber = worker.nextEventNumber++`
  };
