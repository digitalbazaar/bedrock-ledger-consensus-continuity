...
function _processPeerRegularEvent({event, needed}, callback) {
  const _event = bedrock.util.clone(event);
  const {operation: ops} = _event;
  delete _event.operation;
  async.auto({
    operationHash: callback => async.map(
      ops, (o, callback) => _util.hasher(o, callback), callback),
    eventHash: ['operationHash', (results, callback) => {
      ...
      _event.operationHash = operationHash;
...
// FIXME: need to handle `WebLedgerConfigurationEvent`
  ...
  async.auto({
    ...
    process: ['checkAncestors', (results, callback) => {
      if(!jsonld.hasValue(event, 'type', 'ContinuityMergeEvent')) {
        return _processPeerRegularEvent({event, needed}, callback);
      }
      _processPeerMergeEvent({event, ledgerNode, needed}, callback);
    }]
...
function _queueEvent({event, ledgerNodeId, meta}, callback) {
  ...
  if(type === 'r') {
    ...
    multi.incrby(opCountKey, event.operation.length);
  }
  if(type === 'm') {
    const headGenerationKey = _cacheKey.headGeneration(
      {eventHash, ledgerNodeId});
...
function _writeEvent(
  ...
  async.auto({
    ...
    writeEvent: ['validator', (results, callback) => {
