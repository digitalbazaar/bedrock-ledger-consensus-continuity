 ...
 */
api.add = (
  {continuity2017, event, genesis, genesisBlock, ledgerNode}, callback) => {
  const ledgerNodeId = ledgerNode.id;
  const storage = ledgerNode.storage;
  _writeEvent({
    continuity2017, event, genesis, ledgerNode
  }, (err, eventRecord) => {
    if(err) {
      return callback(err);
    }
...
function _writeEvent({continuity2017, event, genesis, ledgerNode}, callback) {
  const ledgerNodeId = ledgerNode.id;
  const storage = ledgerNode.storage;
