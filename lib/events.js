  ...
  _writeEvent({
    ...
    async.auto({
      ...
      updateEvent: ['writeBlock', (results, callback) => storage.events.update({
        eventHash: database.hash(eventRecord.meta.eventHash), patch: [{
          op: 'set',
          changes: {
            meta: {
              consensus: true,
...
api.aggregateHistory = ({
  ...
  const pipeline = [
    ...
    {$project: {
      _id: 0, '_parents.eventHash': 1, '_parents.event.parentHash': 1
    }},
    {$unwind: '$_parents'},
  ...
  collection.aggregate(pipeline, {allowDiskUse: true}).toArray(
    (err, result) => {
      ...
      for(let i = 0; i < result.length; ++i) {
        hashMap[result[i].eventHash] = result[i];
      }
      ...
      function visit(e) {
        ...
        e._p = true;
        sortedHistory.push(e.eventHash);
      }
    });
};
...
api.create = ({ledgerNode}, callback) => {
  logger.debug('Attempting to creating an event.');
  const ledgerNodeId = ledgerNode.id;
  ...
  async.auto({
    ...
    cache: ['event', (results, callback) => {
      logger.debug('Operation cache clean-up.');
      cache.client.multi()
        .srem(opSetKey, results.opKeys)
        .del(results.opKeys)
        .exec((err, result) => {
          logger.debug('Cache cleanup result', {err1: err, result});
          callback(err);
        });
    }]
  }, err => {
  ...
  async.auto({
    childless: callback => cache.client.smembers(
      childlessKey, (err, result) => {
        ...
        callback(null, result.map(h => parseInt(h)));
      }),
    head: ['childless', (results, callback) => api._getLocalBranchHead(
    ...
    merge: ['head', (results, callback) => {
      ...
      async.auto({
        ...
        cache: ['write', (results, callback) => {
          ...
          const eventHash = database.hash(results.eventHash);
  ...
  async.auto({
    latest: callback => {
      ...
      cache.client.hmget(latestPeerHeadKey, 'h', 'g', (err, result) => {
        ...
        callback(null, {
          eventHash: parseInt(result[0], 10),
  ...
  async.auto({
    genesisCache: callback => cache.client.get(
      genesisCacheKey, (err, result) => {
        ...
        callback(null, {eventHash: parseInt(result, 10), generation: 0});
      }),
...
// FIXME: this should also be a WebLedgerEvent?
function _genesisProofCreate({ledgerNode, eventHash}, callback) {
  const doc = {
    '@context': config.constants.WEB_LEDGER_CONTEXT_V1_URL,
    type: 'ContinuityMergeEvent',
    parentHash: [database.hash(eventHash)],
  };
  const ledgerNodeId = ledgerNode.id;
  async.auto({
...
function _writeEvent(
  ...
  async.auto({
    ...
    cacheUpdate: ['writeEvent', (results, callback) => {
      ...
      const eventHash = database.hash(results.process.meta.eventHash);
      if(fromPeer) {
...
function _writeGenesisBlock({block, eventRecord, storage}, callback) {
  async.auto({
    ...
    writeBlock: ['hashBlock', (results, callback) => {
      ...
      // TODO: this should also include the genesis merge event
      block.event = [database.hash(eventRecord.meta.eventHash)];
