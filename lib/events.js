...
api.getEventsForGossip = async ({eventHash, ledgerNodeId}) => {
  // FIXME: add an LRU cache w/stringified events
  ...
  const records = await ledgerNode.storage.events.getMany(
    {eventHashes: eventHash}).toArray();
  const eventMap = new Map();
  for(const record of records) {
    eventMap.set(record.meta.eventHash, record.event);
  }
  const results = [];
  for(const hash of eventHash) {
    const event = eventMap.get(hash);
    if(event) {
