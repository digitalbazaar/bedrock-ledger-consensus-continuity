  ...
  async.auto({
    ...
    write: ['eventHash', (results, callback) => {
      ...
      const meta = {continuity2017: {
        creator: creator.id,
        generation: history.localBranchHead.generation + 1
      }, eventHash};
    ...
    cache: ['write', (results, callback) => {
      ...
      cache.client.multi()
        .set(_cacheKey.head({creatorId, ledgerNodeId}), results.eventHash)
  ...
  heads: ['localCreators', (results, callback) => {
    ...
    async.each(combinedCreators, (creatorId, callback) =>
      api._getLocalBranchHead({
        ...
        // TODO: include generation here
        creatorHeads[creatorId] = result.eventHash;
        callback();
...
api.partitionHistory = ({
  ...
  async.auto({
    ...
    startHash: ['localHeads', 'genesisMergeHash', (results, callback) => {
      // TODO: make this configurable
      const maxDepth = 15;
      const peerLocalNodeHead = _creatorHeads[creatorId] ||
        results.genesisMergeHash;
...
api._getLocalBranchHead = (
  ...
  const genesisCacheKey = _cacheKey.genesis({creatorId, ledgerNodeId});
  ...
  async.auto({
    ...
    genesisCache: ['localMerge', (results, callback) => {
      ...
      cache.client.get(genesisCacheKey, (err, result) => {
        if(err) {
          return callback(err);
        }
        if(!result) {
    ...
    genesisMerge: ['genesisCache', (results, callback) => {
      ...
      eventsCollection.findOne(query, projection, (err, result) => {
        if(err) {
          return callback(err);
        }
        if(!result) {
        ...
        cache.client.set(genesisCacheKey, eventHash, err => {
          if(err) {
            return callback(err);
          }
          // generation for genesis is always zero
          callback(null, {eventHash, generation: 0});
        });
      });
    }]
  }, (err, results) => {
  ...
  async.auto({
    ...
    process: ['ledgerNode', (results, callback) => {
      ...
      async.eachSeries(results.unzip, (e, callback) => async.auto({
        exists: callback => {
          // TODO: also check the event queue
...
function _queueEvent({event, ledgerNodeId, meta}, callback) {
  const eventCacheKey = _cacheKey.event(
    {eventHash: meta.eventHash, ledgerNodeId});
  async.series([
    callback => cache.client.set(
      eventCacheKey, JSON.stringify({event, meta}), callback),
    callback => cache.client.rpush(
      _cacheKey.eventQueue(ledgerNodeId), eventCacheKey, callback)
  ], callback);
}
...
function _writeEvent(ledgerNode, event, options, callback) {
  ...
  async.auto({
    ...
    localCreator: callback => {
      ...
      voters.get(ledgerNodeId, (err, result) => {
        if(err) {
          return callback(err);
        }
    ...
    signature: callback => {
      ...
      signature.verify({doc: event, ledgerNodeId}, (err, result) => {
        if(err) {
          return callback(err);
        }
    ...
    head: ['localCreator', 'signature', (results, callback) => {
      // genesis or regular event from peer
      if(options.genesis || (fromPeer && !results.signature)) {
        return callback();
      }
      // local regular events and merge events from peer pass through here
      let creatorId;
      if(results.signature) {
        // set in signature
        creatorId = meta.continuity2017.creator;
      } else {
        creatorId = results.localCreator.id;
      }
      api._getLocalBranchHead({
        creatorId, eventsCollection: storage.events.collection, ledgerNodeId
      }, (err, result) => {
        if(err) {
          return callback(err);
        }
        if(fromPeer) {
          // continuity2017 object setup in signature
          meta.continuity2017.generation = result.generation + 1;
        } else {
    ...
    validate: ['head', (results, callback) => {
    ...
    if(err) {
      logger.error('Error in write event', err);
      return callback(err);
    }
