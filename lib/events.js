...
api.addBatch = async ({events, ledgerNode, neededSet}) => {
  ...
  // const [mergeEvent] = events.slice(-1);
  ...
  for(let i = events.length - 1; i >= 0; --i) {
    ...
    eventMap.set(meta.eventHash, {event, meta, _temp: {}});
  }
      ...
      for(const ancestorHash of event.parentHash) {
        ...
        if(ancestor) {
          ...
          if(type !== 'm') {
            ...
            if(type === 'r') {
              // console.log('7777777777', JSON.stringify(ancestor, null, 2));
              const {operationRecords} = ancestor.event;
              for(const {operation} of operationRecords) {
                console.log('OOOOOOOOOOOOOO', operation);
...
async function _processPeerEvent(
  {event, ledgerNode, neededSet, requiredCreator}) {
  ...
  if(jsonld.hasValue(event, 'type', 'ContinuityMergeEvent')) {
    ...
    // await _checkTree({creator, ledgerNode, treeHash});
    ({event, meta} = await _processPeerMergeEvent({event, ledgerNode}));
  }
  if(jsonld.hasValue(event, 'type', 'WebLedgerConfigurationEvent')) {
    // const {treeHash} = event;
    // await _checkTree({creator, ledgerNode, treeHash});
