...
api.merge = callbackify(async ({creatorId, ledgerNode}) => {
  ...
  const parentHashes = await cache.client.smembers(childlessKey);
...
api.partitionHistory = callbackify(async (
  ...
  const [genesisHead, localHeads] = await Promise.all([
    api.getGenesisMergeSummary({ledgerNode}),
    await api.getCreatorHeads({ledgerNode, peerId})]);
...
api._getLocalBranchHead = callbackify(async (
  ...
  // *still* no head, so use genesis merge event
  if(!head) {
    head = await api.getGenesisMergeSummary({ledgerNode});
  }

  return head;
});

// TODO: rename to `getGenesisHead`
api.getGenesisMergeSummary = callbackify(async ({ledgerNode}) => {
...
async function _hasOutstandingRegularEvents({ledgerNode}) {
  ...
  const record = await collection.findOne(query, {_id: 1});
  return !!record;
}
...
async function _processPeerConfigurationEvent({event, ledgerNode}) {
  ...
  return ledgerNode.config.validate(event.ledgerConfiguration);
}
...
async function _verifyGenesisEvents({eventHash, genesisBlock, ledgerNodeId}) {
  ...
  const {keyOwner: creator} = await signature.verify(
    {doc: mergeEvent, ledgerNodeId});
  const proofHash = await _util.hasher(proof);
  const mergeHash = await _util.hasher(mergeEvent);
