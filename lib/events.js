...
api._addTestEvent = async ({event, ledgerNode}) => {
...
api.addBatch = async ({
  ...
  const {eventHashes, eventMap} = await _validateEvents({
    blockHeight, events, ledgerNode, needed
  });
...
api.getRecentHistory = async ({creatorId, ledgerNode}) => {
...
async function _hashOperations({ledgerNode, operations}) {
  const hashedOperations = await _processChunked({
...
/**
 *  Gossip Events Validation Worker
 */
...
async function _validateOperation({operation}, ledgerNode, basisBlockHeight) {
...
async function _processChunked({tasks, fn, chunkSize, args = []}) {
  const finishedTasks = [];
  let batch = [];
  ...
  for(const task of tasks) {
    ...
    if(batch.length === chunkSize) {
      await pImmediate();
      const ops = await Promise.all(batch);
