...
api.getCreatorHeads = ({ledgerNode, peerId}, callback) => async.auto({
  ...
  heads: ['localCreators', (results, callback) => {
    ...
    async.each(creators, (creatorId, callback) =>
...
api._getLocalBranchHead = (
  {advance = false, creatorId, ledgerNode}, callback) => {
  const ledgerNodeId = ledgerNode.id;
  ...
  async.auto({
    ...
    const headAdvanceKey = _cacheKey.headAdvance({creatorId, ledgerNodeId});
    cache.client.incr(headAdvanceKey, (err, result) => {
      if(err) {
        return callback(err);
      }
      head.generation += parseInt(result, 10);
      callback(null, head);
    });
  });
};
...
function _processPeerMergeEvent({event, ledgerNode}, callback) {
  async.auto({
    ...
    head: ['signature', (results, callback) => {
      api._getLocalBranchHead(
        {advance: true, creatorId: results.signature.creator, ledgerNode},
        (err, result) => {
          console.log('______________________________________________________');
          console.log('EEEEEEEEEEE', err, result);
          callback(err, result);
        })
    }],
  }, (err, results) => {
