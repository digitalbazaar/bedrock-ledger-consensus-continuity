...
api.addBatch = async ({
  ...
  const {deferredEvents, eventHashes, eventMap} = await _validateEvents({
    blockHeight, events, ledgerNode, needed
  });
...
async function _validateEvents({
  ...
  const neededSet = new Set(needed);
  const deferredEvents = [];
  for(const [index, {event, deferred, meta}] of results.entries()) {
    const {eventHash} = meta;
    if(deferred) {
      console.log('EVENT WAS DEFERRED');
      // FIXME: handled deferred events
      deferredEvents.push(eventHash);
      continue;
    }
    eventHashes.push(eventHash);
  ...
  return {eventHashes, eventMap, deferredEvents};
}
...
async function _validateEvent({data: {blockHeight, event, ledgerNodeId}}) {
  ...
  if(Number.isInteger(basisBlockHeight) && basisBlockHeight > blockHeight) {
    // do not attempt to validate yet
    return {event: processedEvent, deferred: true, meta};
  }
