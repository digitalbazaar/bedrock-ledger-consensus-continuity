  ...
  async.auto({
    childless: callback => {
      ...
      multi.exec((err, result) => {
        ...
        if(result[0] !== null) {
          return callback(new BedrockError(
            'Gossip is behind, delaying merge.', 'AbortError'));
        }
        // if there are no childless events, do not merge
        const childless = result[1];
        if(childless.length === 0) {
          return callback(new BedrockError(
            'No childless events, delaying merge.', 'AbortError'));
        }
        callback(null, childless);
      });
    },
    // if there are no outstanding events, there is no need to merge
    outstandingEvents: ['childless', (results, callback) =>
      _hasOutstandingEvents({ledgerNode}, (err, result) => {
        if(err) {
          return callback(err);
        }
        if(!result) {
    ...
    head: ['outstandingEvents', (results, callback) => api._getLocalBranchHead(
...
function _hasOutstandingEvents({ledgerNode}, callback) {
...
function _processPeerConfigurationEvent({event, ledgerNode, needed}, callback) {
  async.auto({
    validate: callback => ledgerNode.config.validate(
      event.ledgerConfiguration, callback),
    eventHash: ['validate', (results, callback) => _util.hasher(
      event, callback)],
  }, (err, results) => {
    if(err) {
      return callback(err);
...
function _processPeerEvent({event, ledgerNode, needed}, callback) {
  async.auto({
    ...
    process: ['checkAncestors', (results, callback) => {
      ...
      if(jsonld.hasValue(event, 'type', 'WebLedgerConfigurationEvent')) {
        return _processPeerConfigurationEvent(
          {event, ledgerNode, needed}, callback);
      }
      // unknown event type
