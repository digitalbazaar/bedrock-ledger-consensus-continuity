...
api.addBatch = async ({
  blockHeight, events, ledgerNode, processedEventHashes = new Set()
}) => {
  const ledgerNodeId = ledgerNode.id;
  let mergePermitsConsumed = 0;

  const {deferredEvents, eventMap} = await _validateEvents({
    blockHeight, events, ledgerNode
  });

  for(const [eventHash, {event, meta, rawEvent}] of eventMap) {
...
api.addBatch_SAVE = async ({
  ...
  for(let i = 0; i < eventHashes.length; ++i) {
    ...
    if(hasValue(event, 'type', 'WebLedgerOperationEvent')) {
      await Promise.all(event.operationRecords.map(async ({operation}) => {
...
bedrock.events.on('bedrock.start', async () => {
  jobQueue = brJobs.addQueue({name: 'validate-events'});
  // FIXME: make configurable
  const gossipProcessingConcurrency = 5;
...
async function _validateEvents({
  blockHeight, events, ledgerNode
}) {
  const ledgerNodeId = ledgerNode.id;
  ...
  for(const event of events) {
    const job = await jobQueue.add({
      blockHeight, event, ledgerNodeId
    }, {
  ...
  const eventMap = new Map();
  for(const [index, {event, meta}] of results.entries()) {
    eventMap.set(meta.eventHash, {event, meta, rawEvent: events[index]});
    if(meta.continuity2017.type === 'm') {
...
async function _validateEvent({data: {blockHeight, event, ledgerNodeId}}) {
  ...
  // events here could be regular, configuration or merge events
  if(hasValue(processedEvent, 'type', 'WebLedgerOperationEvent')) {
    await Promise.all(
      processedEvent.operationRecords.map(async ({operation}) => {
        const result = await ledgerNode.operations.validate(
          {basisBlockHeight, ledgerNode, operation});
        if(!result.valid) {
          throw result.error;
  ...
  } else if(hasValue(processedEvent, 'type', 'WebLedgerConfigurationEvent')) {
    const result = await ledgerNode.config.validate({
      basisBlockHeight, ledgerConfiguration: event.ledgerConfiguration,
      ledgerNode
    });
  ...
  } else if(hasValue(processedEvent, 'type', 'ContinuityMergeEvent')) {
    if(deferredEvents.length > 0 && _temp.insideBatch.length > 0) {
      // if no events have been deferred, it is not necessasry to check the
      // processedEventHashes against the insideBatch list. We know that the
      // ancestors were ordered in the batch prior to this event. If none
