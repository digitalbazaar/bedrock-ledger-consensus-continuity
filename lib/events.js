...
const {config, jsonld, util: {callbackify, BedrockError}} = bedrock;
...
api.addBatch = async ({
  ...
  for(let i = 0; i < eventHashes.length; ++i) {
    ...
    if(Number.isInteger(basisBlockHeight) && basisBlockHeight > blockHeight) {
      const {gossip: {blockHeightThreshold}} = cfg;
      if(basisBlockHeight - blockHeight > blockHeightThreshold) {
        throw new BedrockError('blockHeightThreshold exceeded.',
          'DataError', {
            basisBlockHeight,
            blockHeight,
    ...
    } else if(jsonld.hasValue(event, 'type', 'WebLedgerConfigurationEvent')) {
    ...
    } else if(jsonld.hasValue(event, 'type', 'ContinuityMergeEvent') &&
...
api.merge = callbackify(async ({creatorId, ledgerNode}) => {
  ...
  let truncated = false;
  let {mergeEvents: {maxEvents}} = _continuityConstants;

  // subtract one because treeHash will be added to parentHash
  maxEvents--;
