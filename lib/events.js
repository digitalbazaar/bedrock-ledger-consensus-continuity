    ...
    async.auto({
      ...
      createIndexes: callback => {
        ...
        database.createIndexes([{
          ...
          options: {sparse: false, background: false, name: 'continuity1'}
        }], callback);
      },
      mergeEvent: callback => {
...
api.mergeBranches = ({ledgerNode, history}, callback) => {
  ...
  async.auto({
    ...
    write: ['eventHash', (results, callback) => {
      ...
      // merge events can be re-created in the event of a failure
      _queueEvent({event: results.sign, ledgerNodeId, meta}, callback);
    }],
