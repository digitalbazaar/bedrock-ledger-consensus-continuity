...
api.getMergeStatus = async ({ledgerNode, creatorId, priorityPeers = []}) => {
  ...
  if(peerChildlessHashes.length === 0 &&
    ...
    const head = await api.getHead({creatorId, ledgerNode});
    if(head.generation === 0) {
      // head is the genesis head which has no operation events, safe to merge
      return {mergeable: true, ...status};
    }

    // TODO: this could perhaps be optimized to a database query such that
    // none of this data must be pulled from the database; however, before
    // writing such an optimization we should determine if other more sweeping
    // changes to merge behavior would render this check unnecessary

    // get parents of head merge event
    const [{event: headMergeEvent}] = await api.getEvents(
      {eventHash: head.eventHash, ledgerNode});
