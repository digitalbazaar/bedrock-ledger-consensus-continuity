...
api.difference = async ({eventHashes, ledgerNode}) => {
  // FIXME: use an in-memory LRU cache instead of redis cache
...
async function _writeEvent({event, eventHash, genesis, ledgerNode}) {
  ...
  /* Note: The cache needs to be updated here as a local event was committed
  to storage and it now needs to be merged; if the cache is not updated, then
  a merge event could be created that excludes it, causing all operations in
  the regular event to be lost or causing a subsequent merge event to be
  invalid by merging in a regular event that is not its sibling; if this
  update fails, the work session should exit and it will be corrected when
  the new work session begins. The "childless" events cache is re-primed
  at the start of every work session, which will cause the cache to be brought
  into sync with the database. */
