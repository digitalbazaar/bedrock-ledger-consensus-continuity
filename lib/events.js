...
api.addBatch = async ({
  blockHeight, events, eventMap, ledgerNode,
  ...
  for(let i = 0; i < eventHashes.length; ++i) {
    ...
    if(hasValue(event, 'type', 'WebLedgerOperationEvent')) {
      ...
      deferredEvents.length > 0 && _temp.insideBatch.length > 0) {
  ...
  return {deferredEvents, processedEventHashes};
};
...
api.merge = callbackify(async ({creatorId, ledgerNode, mergeStatus}) => {
  if(!mergeStatus) {
    // merge status was not provided, retrieve it on demand
    mergeStatus = await api.getMergeStatus({ledgerNode, creatorId});
  }
...
api.getMergeStatus = async ({ledgerNode, creatorId, checkOperations}) => {
  const ledgerNodeId = ledgerNode.id;
  const status = await _cache.events.getMergeStatus({ledgerNodeId});
  const {
    gossipBehind, priorityPeers, peerChildlessHashes
  } = status;
  ...
  // if there are no peer events to merge and there are `priorityPeers` other
  // than `creatorId`, then do not merge until more peer events come in
  if(peerChildlessHashes.length === 0 && (priorityPeers.length > 1 ||
    (priorityPeers.length === 1 && priorityPeers[0] !== creatorId))) {
    return {mergeable: false, ...status};
  }
