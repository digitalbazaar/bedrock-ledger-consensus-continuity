...
api.aggregateHistory = ({
  ...
  if(creatorRestriction && creatorRestriction.length !== 0) {
    // const s = {
    //   $and: [{'event.type': 'ContinuityMergeEvent'}]
    // };
    // const y = {
    //   $or: []
    // };
    ...
    // s.$and.push(y);
    // restrictSearchWithMatch.$or.push(s);
  }
  ...
  console.log('@@@@@ RESTRICT SEARCH', JSON.stringify(restrictSearchWithMatch, null, 2));
...
api.getCreatorHeads = ({ledgerNode}, callback) => async.auto({
  ...
  heads: ['localCreators', (results, callback) => {
    const creatorHeads = {};
    async.each(results.localCreators, (creatorId, callback) =>
...
api.partitionHistory = (
  ...
  async.auto({
    ...
    history: ['startHash', (results, callback) => {
      ...
      console.log('HEADS FROM ORIGINATOR IN PARTITION', JSON.stringify(_creatorHeads, null, 2));
...
function _diffManifest({ledgerNode, manifest, eventHashes}, callback) {
  ...
  _diff({eventHashes, ledgerNode}, (err, result) => {
    ...
    if(dupCount !== 0) {
      console.log('DDDDDDDDDDDDDDDDDDDDDdUUUUUUUUUP', dupCount);
...
function _checkAncestors({eventHashes, ledgerNode}, callback) {
  ...
  console.log('ooooooooooooooooo ancestors', eventHashes);
  _diff({eventHashes, ledgerNode}, (err, result) => {
    if(err) {
      return callback(err);
    }
    if(result.length !== 0) {
      console.log('mmmmmmmmmmmmmmmmmmmmmmmmm mising ancestors', ledgerNode.storage.events.collection.s.name, result);
...
api.processCompressed = ({creatorId, eventsGz}, callback) => {
  ...
  async.auto({
    ...
    process: ['ledgerNode', (results, callback) => {
      ...
      console.log('$$$$$$$$$$ EVENT COUNT', results.unzip.length);
      // console.log('#########', JSON.stringify(results.unzip, null, 2));
      // create a list of all the eventHashes

      // dups should be removed from records before doing any work with
      // ancestors
      // manifest itself is dup free, put manifest into redis set

      // remove ancestors that are included in the manifest
      // _.pull(ancestors, ...manifest);
      // check the queue for ancestors, then check database
      // aim is to make sure all ancestors in `ancestors` exist
      // ancestors could be in queue or database

      // aim is to remove dups from `manifest`
      // dups could be in queue or database
      ...
      async.eachSeries(results.unzip, (e, callback) => async.auto({
        ...
        // TODO: no need to check ancestors of duplicates
        ancestors: ['diff', (results, callback) => {
          ...
          // manifest itself
          console.log('rrrrrrrrrrrrrrrrr RAWManifest', rawManifestEventHashes);
        ...
        addEvent: ['ancestors', (results, callback) => {
          if(results.exists) {
            return callback();
          }
          console.log('AAAAAAAAdding', e.eventHash);
          ledgerNode.events.add(
            e.event, {continuity2017: {peer: true}}, callback);
        }]
      }, callback), callback);
    }]
  }, err => callback(err));
...
function _processPeerEvent({event, ledgerNode}, callback) {
  ...
  _processPeerMergeEvent({event, ledgerNode}, (err, result) => {
    ...
    const {creator, generation} = result;
    const nextGeneration = generation + 1;
    const _meta = {continuity2017: {creator, generation: nextGeneration}};
...
function _queueEvent({event, ledgerNodeId, meta}, callback) {
  ...
  const eventQueueSetKey = _cacheKey.eventQueueSet(ledgerNodeId);
  // NOTE: multi's are atomic so there should be no need for a lock
  cache.client.multi()
    // add the hash to the set used to check for dups and ancestors
    .sadd(eventQueueSetKey, eventHash)
