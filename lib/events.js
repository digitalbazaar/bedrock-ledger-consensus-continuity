...
api.getNonConsensusEvents = async ({ledgerNode}) => {
  ...
  if(genesis) {
    eventRecord = {
      ...
      meta: {
        ...
        continuity2017: {type: 'c', localEventNumber: 0}
      }
    };
  } else {
...
async function _createLocalEventRecord({worker, event, eventHash}) {
  ...
  const peer = await _peers.get({ledgerNodeId});
  const meta = {
    blockHeight: -1,
    consensus: false,
    continuity2017: {
