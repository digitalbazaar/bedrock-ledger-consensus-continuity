...
api.processCompressed = ({creatorId, eventsGz}, callback) => {
  ...
  async.auto({
    ...
    process: ['ledgerNode', (results, callback) => {
      ...
      console.log('ZZZZZZZZ', JSON.stringify(results.unzip, null, 2));
...
function _processPeerEvent({event, ledgerNode}, callback) {
  ...
  async.auto({
    ...
    hasAncestors: callback => {
      ...
      // FIXME: it is sometimes the case that mongo reports missing documents
      // believed to be due to inability for a thread to read its own writes
      // in mongo 3.4.  This issue may be resolved in mongo 3.6 which involves
      // using the mongo 3.x driver
