...
api.getEvents = async (
  {eventHash, ledgerNodeId, meta = false, stringify = false}) => {
...
async function _processPeerEvent({event, ledgerNode, neededSet}) {
  ...
  if(jsonld.hasValue(event, 'type', 'WebLedgerOperationEvent')) {
    const {creator, treeHash} = event;
    await _checkTree({creator, ledgerNode, treeHash});
    ({event, meta} = await _processPeerRegularEvent({event, ledgerNode}));
  }
  ...
  if(jsonld.hasValue(event, 'type', 'WebLedgerConfigurationEvent')) {
    const {creator, treeHash} = event;
    await _checkTree({creator, ledgerNode, treeHash});
    ({event, meta} = await _processPeerConfigurationEvent({event, ledgerNode}));
  }
  if(!meta) {
    throw new BedrockError(
      'Unknown event type.',
