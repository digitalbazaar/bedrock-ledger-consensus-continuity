...
api.generate = async ({ledgerNodeId} = {}) => {
  ...
  let peerId;
  try {
    peerId = await _generateLocalPeer({ledgerNodeId});
  } catch(e) {
 ...
 *   key pair.
 */
api.getKeyPair = async ({ledgerNodeId, peerId}) => {
  const keyPair = await LOCAL_PEER_KEY_PAIR_CACHE.memoize({
    key: ledgerNodeId,
    fn: async () => _getKeyPair({ledgerNodeId, peerId})
  });
 ...
 *   privateKeyBase58 - The private key material.
 *
...
api.storage.get = async ({ledgerNodeId, peerId, keyPair = false}) => {
  ...
  if(keyPair) {
    projection['localPeer.privateKeyBase58'] = 1;
  }
...
async function _generateLocalPeer({ledgerNodeId}) {
  const kp = await Ed25519VerificationKey2018.generate();
...
async function _getKeyPair({peerId, ledgerNodeId}) {
  ...
  localPeer.keyPair = {
    privateKeyBase58: record.localPeer.privateKeyBase58,
    publicKeyBase58: api.getPublicKeyFromId({peerId: localPeer.peerId})
  };
  return localPeer;
}
