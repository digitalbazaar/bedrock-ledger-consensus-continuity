...
function _tally({ledgerNode, event, yByElector, electors}) {
  ...
  // in the case of a tie, clear the top choice
  if(tally.length > 1 && topChoice.count === tally[1].count) {
    topChoice = null;
  }
...
function _hasAncestors(
  ...
  // TODO: currently possible to get stuck in this loop -- only `checked` is
  //   preventing a loop, so need to investigate how a cycle is getting formed
