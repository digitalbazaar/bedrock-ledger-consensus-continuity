...
function _findMergeEventProofCandidates({tail, electors}) {
  ...
  const totalXCandidates = Object.values(xByElector).filter(c => c).length;

  if(totalXCandidates < supermajority) {
...
function _findDiversePedigreeMergeEvent({
  ...
  console.log('DDDDDDD', Object.keys(x._des));
  ...
  for(let descendant of descendants) {
    ...
    // recurse
    console.log('rrrrrrrrrrrrr RECURSING');
...
function _isHistoryEntryRegularEvent(x, elector) {
  return x._des && x._des[elector].meta.continuity2017.creator ===
    x.meta.continuity2017.creator;
}
