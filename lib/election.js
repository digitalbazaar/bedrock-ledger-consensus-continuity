  ...
  while(next.length > 0) {
    ...
    for(let event of current) {
      ...
      if(!event._supporting) {
        ...
        if(_.find(votingEvents, e => e !== event && !('_supporting' in e))) {
...
function _tally({ledgerNode, event, yByElector, blockHeight, electors}) {
  ...
  */
  // TODO: consider deciding on confirmations if precommits are locked
  //   rather than waiting for an endorsement of confirmations -- if proof
  //   demonstrates all confirmations will be the same and precommits are
  //   non-concurrent with vote locking enabled
  if(event._confirmation) {
    // TODO: need to ensure `event._confirmation` is the earliest that
    //   the node sees... either it falls out from the current algorithm
    //   or we need more precise code here to check ancestry for an
    //   earlier decision
    ...
    // find largest confirmed support set
    return _findLargestConfirmationSet(event);
  }

  // propagate accepted status
  if(event._treeParent._accepted) {
  ...
  */
  ...
  // TODO: consider locking choice is a precommit is seen in ancestry

  /*
  Find an ancestor that has a confirmation event that is accepted with the
  largest support set (ignoring byzantine?):
    Then set next choice to the confirmation.
    And continue...
  */
  const largest = _findLargestConfirmationSet(event);
  if(largest) {
    nextChoice = _.find(tally, _findSetInTally(largest));
  ...
  */
  if(!nextChoice) {
    ...
    const union = _.uniq(_.values(event._votes).map(r => r._y));
  ...
  */
  if(hasSupermajority) {
    /*
    If the event is a potential confirmation (i.e. it has a linked precommit):
      Compare the nextChoice to the precommit and if it match and has at
      least a supermajority of support, create a diverse pedigree decision
      point in the future and return.
      Otherwise, continue...
    */
    if(event._preCommit && !('_accepted' in event)) {
      if(_compareSupportSet(event._preCommit._supporting, nextChoice.set)) {
        //console.log('precommit supports same set, confirmed');

        // preCommit is confirmed, confirmation accepted
        event._preCommit._confirmed = true;
        event._accepted = true;
        ...
        // preCommit is not confirmed, confirmation rejected
        event._preCommit._confirmed = false;
      ...
      // event is not a potential confirmation, create or reuse a precommit...

      // optimize to reuse another precommit from a tree ancestor if it is
      // current, has not been rejected, and supports the same set
      let preCommit = false;
      let parent = event._treeParent;
      while(parent) {
        if(parent._isPreCommit) {
          if(parent._confirmed !== false) {
            preCommit = parent;
          } else {
            // FIXME: remove me
            /*console.log('could not reuse a precommit with status', parent._confirmed);
            console.log('same set?', _compareSupportSet(nextChoice.set, parent._supporting));
            */
          }
          break;
        }
      ...
      if(preCommit && !_compareSupportSet(nextChoice.set, parent._supporting)) {
        // preCommit exists but does not support the same set, reject it
        preCommit._confirmed = false;
        if(preCommit._potentialConfirmation) {
          preCommit._potentialConfirmation._accepted = false;
        }
      }
      ...
      if(!preCommit) {
        /*console.log('previous precommit not found, creating new one at',
          event._generation);*/
        // mark event as a precommit
        event._isPreCommit = true;
        preCommit = event;
      }

      // FIXME: why would we need to try and recompute this multiple times,
      //   i.e. how could it fail and then be successful on a subsequent
      //   run if the history doesn't change?

      // compute potential confirmation event
      if(!preCommit._potentialConfirmation) {
        const ancestryMap = _buildAncestryMap(preCommit);
        const potentialConfirmation = _findDiversePedigreeMergeEvent(
          {ledgerNode, x: preCommit, electors, supermajority, descendants: {},
            ancestryMap: ancestryMap});
        if(potentialConfirmation) {
          /*console.log('marking potential confirmation event',
            potentialConfirmation._generation,
            'for precommit', preCommit._generation);*/
          potentialConfirmation._preCommit = preCommit;
          preCommit._potentialConfirmation = potentialConfirmation;
        } else {
          // FIXME: remove me
          /*console.log('no potential confirmation event yet for precommit',
...
function _findLargestConfirmationSet(event) {
  let largest;
  _.values(event._votes).forEach(r => {
    if(!r._accepted) {
      return;
    }
    if(!largest || largest.length < r._supporting.length) {
      largest = r._supporting;
    }
  });
  return largest;
}
