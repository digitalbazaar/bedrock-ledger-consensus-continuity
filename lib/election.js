...
function _tally({ledgerNode, event, yByElector, blockHeight, electors}) {
  ...
  */
  if(event._confirmation) {
    ...
    // decide using the smallest confirmation set
    return _findSmallestConfirmationSet(event);
  }

  // propagate accepted status
  if(event._treeParent._accepted && !event._accepted) {
    event._accepted = event._treeParent._accepted;
  }
  ...
  */
  ...
  /*
  Find the union of all ancestral confirmation event support sets
  (ignoring byzantine?):
    Then set next choice to the confirmation.
    And continue...
  */
  const unioned = _findUnionConfirmationSet(event);
  if(unioned) {
  ...
  // TODO: perhaps additionally, if no next choice has been set, and if
  //   required for consistency, find the union of all ancestry precommit
  //   event support sets (ignoring byzantine?):
  //     Then set next choice to union and continue
  // TODO: note - that this rule may be introduced if required to prevent
  //   late joining y's from causing trouble; if later joiners (post precommit)
  //   had to use (non-rejected?) precommit support sets only, then this
  //   would not be possible by rule
    ...
    */
    if(event._preCommit && !('_accepted' in event)) {
      if(_compareSupportSet(event._preCommit._supporting, nextChoice.set)) {
        ...
        event._accepted = event._preCommit;
...
function _findUnionConfirmationSet(event) {
  let union;
  _.values(event._votes).forEach(r => {
    ...
    if(!union) {
      ...
      union = _.uniq(union.concat(r._supporting));
    }
  });
  return union;
}

function _findSmallestConfirmationSet(event) {
  let smallest;
  _.values(event._votes).forEach(r => {
    if(!r._accepted) {
      return;
    ...
    if(!smallest) {
      smallest = r._accepted._supporting;
    } else if(r._accepted._supporting.length < smallest.length) {
      smallest = r._accepted._supporting;
    }
  });
  return smallest;
