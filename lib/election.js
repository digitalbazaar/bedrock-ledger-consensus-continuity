...
function _findConsensusMergeEventProof(
  ...
  allYs.forEach(y => {
    console.log('Start sync _allSeenYs');
    const supporting = _allSeenYs({event: y, allYs, yAncestryMaps});
    console.log('End sync _allSeenYs');
    // always include self
    supporting.push(y);
...
function _tally(
  ...
  if(!topChoice || topChoice.count === 1) {
    ...
    if(!topChoice) {
      ...
      // all seen Ys
      union = _allSeenYs({event, allYs, yAncestryMaps});
    } else {
...
function _allSeenYs({event, allYs, yAncestryMaps}) {
  const found = new Set();
  _hasAncestors({
    target: event,
    candidates: allYs,
