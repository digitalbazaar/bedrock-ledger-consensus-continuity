...
function _verifyVoteSignature(ledgerNodeId, vote, callback) {
  let cacheKey;
  let newKey;
  async.auto({
    verify: callback => jsigs.verify(vote, {
    ...
    storeKey: ['verify', (results, callback) => {
      ...
      // ignore all errors
      api._storage.keys.addPublicKey(ledgerNodeId, newKey, () => callback());
    }]
  }, callback);

  function _getPublicKey(keyId, options, callback) {
    async.auto({
      cache: callback => api._storage.keys.getPublicKey(
