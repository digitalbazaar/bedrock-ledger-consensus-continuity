...
api.findConsensus = (ledgerNode, history, callback) => {
  ...
  // TODO: const result = _findConsensusMergeEventProof(
 ...
 */
function _findConsensusMergeEventProof({ledgerNode, tail, electors}) {
  ...
  // TODO: if there is a supermajority of Y merge events, then...
  // TODO: do consistent cuts voting and experiments, etc.

  // track the values each elector is supporting
  const supporting = {};

  // TODO: potentially skip this loop if we can reuse `yCandidatesByElector`
  for(const elector in yCandidatesByElector) {
    // each elector begins by supporting its own candidate
    supporting[elector] = yCandidatesByElector;
  }

  // run experiments in order of elector hashed with last block
  // FIXME: does this mean a byzantine actor can swing a vote in their favor?

  // TODO: can anything be done with the supermajority that has seen each
  // `Y` to determine the votes?
  // TODO: has everyone in the supermajority seen everyone else's candidates?
  // TODO:  (note: we *can* assume they will eventually... minus byzantineness)
 ...
 *           branch-native.
 */
function _findMergeEventYCandidates({historyTail, electors}) {
  const supermajority = api.twoThirdsMajority(electors.length);
  ...
  for(const elector of electorsWithTails) {
    ...
    for(const tail of electorsWithTails[elector]) {
      const result = _findEarliestCandidate(
        {x: tail, elector, electors, supermajority, cache});
  ...
  for(let elector in xByElector) {
    ...
    const result = _findEarliestCandidate(
      {x, elector, electors, supermajority});
    // TODO: can this be run? if we wait longer and run this algorithm
    //   a bit later, won't it cause `y` to be returned?... that seems to
    //   violate a safety constraint.... unless it would never be picked
    //   unless it was the winner
...
/**
 * Recursively find the earliest merge event proof candidate for an elector.
 * The search starts at the oldest event in history on a particular elector
 * branch (this constitutes generation `0`) and proceeds forward through
 * history.
 *
 ...
 *
 * @return the earliest candidate found and its generation.
 */
function _findEarliestCandidate({
  ...
  const remaining = _.difference(branches - totalEndorsements);
