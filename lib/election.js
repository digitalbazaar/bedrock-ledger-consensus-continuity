...
api.getBlockElectors = callbackify(async ({ledgerNode, blockHeight}) => {
  const [ledgerConfiguration, {eventBlock}] = await Promise.all([
    ...
    // FIXME: why? it doesn't look like expanded events are used here... can
    // we simplify and just get the block summary?
    _blocks.getLatest(ledgerNode)
  ]);
  ...
  return electorSelectionMethod.api.getBlockElectors(
...
async function _getAncestors({blockHeight, hashes, ledgerNode}) {
  ...
  while(remaining > 0) {
    query['meta.eventHash'].$in = parentHashes.slice(start, start + batchSize);
    start += batchSize;
    remaining -= query['meta.eventHash'].$in.length;
    const records = await collection.find(query, projection).toArray();
    nonConsensusHashes.push(...(records.map(r => r.meta.eventHash)));
  }
