 ...
 */
api.certify = (electionTopic, votes, callback) => {
...
function _createFirstRoundEventVote(
  ...
  _createEventManifest(ledgerNode, blockHeight, (err, manifest) => {
    ...
    async.auto({
      recommendElectors: _recommendElectors(
      ...
      storeVote: ['signVote', (results, callback) => {
        api._storage.votes.add(
          ledgerNode.id, 'Events', results.signVote, err => {
...
function _createInstantRunoffEventVote(
  ...
  _selectInstantRunoffEventManifest(
    ...
    // TODO: create and store new vote with vote.round + 1 and same
    //   recommended electors
