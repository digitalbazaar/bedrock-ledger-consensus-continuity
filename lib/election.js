...
function _findMergeEventProof({ledgerNode, history, tails, electors}) {
  ...
  return _findConsensusMergeEventProof(
    {eventMap: history.eventMap, yCandidatesByElector, supermajority});
}
...
function _findConsensusMergeEventProof({eventMap, yByElector, supermajority}) {
  ...
  for(let elector in yByElector) {
    ...
    while(event) {
      ...
      haltAncestryMap[event._treeParent.eventHash] = true;
    }
  }
  ...
  if(allCs.length > 1) {
    ...
    // prune all C's that are descenants of other C's
...
function _hasAncestors(
  ...
  for(const parentHash of target.parentHash) {
    if(parentHash in haltAncestryMap) {
