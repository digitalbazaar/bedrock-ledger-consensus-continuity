 ...
 *           byzantine behavior).
 */
function _getElectorBranches({events, electors}) {
  const electorTails = {};

  // group events by elector
  const byElector = {};
  for(const event of events) {
    const creator = _getCreator(event);
    if(electors.includes(creator)) {
      if('creator' in byElector) {
        byElector[creator].push(event);
      } else {
        byElector[creator] = [event];
      }
    }
  }

  // find tail (earliest event) for each elector; note that there is only one
  // for correct nodes but we must account for byzantine nodes reporting more
  // than one -- a tail will have a `treeHash` that is not present in the
  // other nodes in the same group
  for(let elector in byElector) {
    const electorEvents = byElector[elector];
    const tails = electorEvents.filter(candidate => !electorEvents.some(
      r => candidate.event.treeHash === r.eventHash));
    electorTails[elector] = tails;

    // FIXME: disabled until determined necessary
    // create branch for every tail
    /*for(const tail of tails) {
      _createBranch({tail, branch: elector, electors});
    }*/
  }
...
function _createBranch({event, branch, electors}) {
  if(event._children.length === 0) {
    // reached head of branch
    return;
  }
  ...
  // initialize branch
  if(!event._branches[branch]) {
    event._branches[branch] = [];
  }

  // TODO: rewrite as non-recursive in the future to to avoid deep DAGs
  for(const child of event._children) {
    event._branches[branch].push(child);
    _createBranch({child, branch, electors});
  }
}
 ...
 *           branch of history.
 */
function _getElectorBranchesOLD({
  event,
  electors,
  child = null,
...
//    if(event._children.length === 0) {
      // event has no children, it is a head for the current branch; note
      // that only one head should be present for a correct node, but
      // byzantine nodes must be accounted for
...
function _findMergeEventProofCandidates({ledgerNode, tails, electors}) {
  ...
  //console.log('TAILS', util.inspect(tails, {depth:5}));
  ...
  for(const elector of electorsWithTails) {
    ...
    if(earliest.candidate) {
      console.log('***X found for', elector, ' at generation ',
        earliest.generation, earliest.candidate.eventHash);
  ...
  for(const elector in xByElector) {
    ...
    if(result.candidate) {
      yByElector[elector] = result.candidate;
    }
  }
...
function _findDiversePedigreeMergeEvent({
  ...
  if(electors.includes(creator)) {
    ...
    console.log('ENTRY GENERATION', entry.generation);
  ...
  // FIXME: disabled this optimization -- as it may actually be more costly
  //   to create the branches required to use it than it is helpful now that
  //   children are precomputed
  ...
  const descendants = x._children;//branches[branch];
  if(descendants.length === 0) {
    // no more descendants so no more candidates
    console.log('no more descendants');
    return result;
  }
  ...
  for(let descendant of descendants) {
    ...
    if(_getCreator(descendant) === branch) {
      nextGeneration++;
    }
    ...
    // recurse
    console.log('rrrrrrrrrrrrr RECURSING', descendant.eventHash);
