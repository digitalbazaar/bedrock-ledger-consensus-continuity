...
function _findConsensusMergeEventProof(
  ...
  console.log('Start sync _findConsensusMergeEventProof: _tallyBranch');
  ...
  console.log('End sync _findConsensusMergeEventProof: _tallyBranch', {
    duration: Date.now() - startTime
  });
...
function _tallyBranch(
  ...
  let next = [event];
  while(next.length > 0 && !result.consensus) {
    const current = next;
    next = [];
    for(const event of current) {
      _tally(
