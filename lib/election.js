...
function _tally(
  ...
  if(!topChoice || topChoice.count === 1) {
    ...
    if(topChoice) {
      // top choice already exists, increment its count
      topChoice.count++;
    } else {
      // create new top choice
      topChoice = {set: union, count: 1};
    }
  ...
  // if vote changed and new top choice has a supermajority...
  if(previousChoice !== topChoice && topChoice.count >= supermajority) {
    // consensus reached
