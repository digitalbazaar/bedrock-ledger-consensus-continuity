...
api.certify = (ledgerNode, electionTopic, votes, callback) => {
  async.each(votes, (vote, callback) => {
    _verifyVote(ledgerNode, vote, err => {
      if(err) {
...
function _verifyVote(ledgerNode, vote, callback) {
  ...
  _verifyVoteSignature(vote, err => {
    ...
    _getManifest(ledgerNode, vote.voter, vote.manifestHash, callback);
  });
}
...
function _getManifest(ledgerNode, peerId, manifestHash, callback) {
    ...
    if(err) {
      if(err.name === 'NotFound') {
        ...
        return _getManifestFromPeer(
          ledgerNode, peerId, manifestHash, callback);
      }
      return callback(err);
    }
...
function _getManifestFromPeer(ledgerNode, peerId, manifestHash, callback) {
  async.auto({
    hashes: callback => ledgerNode.storage.events.getHashes(
      {consensus: false, sort: 1}, callback),
  ...
  async.auto({
    ...
    addEvent: ['hashEvent', (results, callback) => {
      ...
      ledgerNode.storage.events.add(
        results.getEvent, {eventHash: results.hashEvent}, callback);
    }]
  }, callback);
}
