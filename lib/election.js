...
api.findConsensus = ({ledgerNode, history, electors}, callback) => {
  ...
  const proof = _findMergeEventProof({ledgerNode, tails, electors});
...
// TODO: move lower-level utility functions below proof finding functions
...
function _flattenDescendants({ledgerNode, x, descendants}) {
  ...
  for(let child of x._children) {
    // skip regular events from `x` children
...
function _updateToMostRecentVotes(
  {ledgerNode, y, descendants, electorSet, votes}) {
  let next = [y];/*
  for(let child of y._children) {
    // skip regular events from `y` children
    if(_isHistoryEntryRegularEvent(child)) {
      if(!child._children || child._children.length !== 1) {
        continue;
      }
    ...
    if(child.eventHash in descendants) {
      ...
      if(electorSet.has(creator)) {
        _useMostRecentVotingEvent(
          {ledgerNode, elector: creator, votes, votingEvent: child});
      }
      next.push(child);
    }
  }*/
  //console.log('Y children', next.map(r=>({
  //  creator: _getCreator(r),
  //  generation: r._generation,
  //  hash: r.eventHash
  //})));
...
function _findConsensusMergeEventProof(
  {ledgerNode, xByElector, yByElector, electors}) {
  ...
  let consensus = _tallyBranches({ledgerNode, yByElector, electors});
...
function _tallyBranches({ledgerNode, yByElector, electors}) {
  /* Algorithm:
    ...
    for(let event of current) {
      ...
      if(!event._supporting) {
        if(!event._votes) {
          ...
          event._votes = event._treeParent._votes;
          ...
          for(const elector of electorSet) {
            ...
            _updateToMostRecentVotes(
              {ledgerNode, y, descendants, electorSet, votes: event._votes});
          }
        }
        ...
        if(_.values(event._votes).some(e => !('_supporting' in e))) {
      ...
      const result = _tally({ledgerNode, event, yByElector, electors});
...
function _tally({ledgerNode, event, yByElector, electors}) {
  ...
  console.log('votes at', event._generation, Object.keys(event._votes).map(k => ({
    elector: k,
    set: event._votes[k]._supporting.map(r=>r._generation)
  })));
  ...
  if(!topChoice || topChoice.count === 1) {
    ...
    console.log('new top choice', topChoice.set.map(r=>r.eventHash));
  ...
  console.log('next top choice', topChoice.set.map(r=>r.eventHash));
