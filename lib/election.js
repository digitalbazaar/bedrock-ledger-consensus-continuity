 ...
 *
 * @param ledgerNode the ledger node to get the history for.
 * @param electors the current electors.
 * @param callback(err, history) called once the operation completes.
 */
api.getRecentHistory = (ledgerNode, electors, callback) => {
  // TODO: getLocalBranchHead

  // // TODO: implement $graphLookup
  // ledgerNode.storage.events.collection.aggregate([{
  //   $graphLookup: {
  //     from: ledgerNode.storage.events.collection.name,
  //     startWith: '<local branch head hash goes here>',
  //     connectFromField: ['treeHash', 'parentHash'],
  //     connectToField: 'meta.eventHash',
  //     //maxDepth: 10, // max generations to go back (need to be infinite and
  //     //  handle out-of-memory degenerate case?)
 ...
 *          result.event the merge event that has reached consensus or `null`.
 *          result.consensusProof the merge event proving consensus or `null`.
 */
api.findConsensus = (ledgerNode, history, callback) => {
  // TODO: implement

  // TODO: _getElectorBranches({event: history, electors})

  // TODO: const result = _findMergeEventProof(
  //   {ledgerNode, history.tail, electors})

  // TODO: if result has x and y, consensus found... may need to compute
  //   more than one x for the given y?
};
...
function _getElectorBranches({
  ...
  if(child === null) {
    ...
    for(const elector of electors) {
      let heads = allBranches.head[elector];
      if(heads) {
...
/**
 * Find a consensus merge event Y that has ancestors from a supermajority of
 * electors that are descendants of merge event X, where merge event X includes
 * ancestors from a supermajority of electors. This indicates that merge event
 * X has both endorsed a merge events from a supermajority of electors and it
 * has been shared with a supermajority of electors because merge event Y
 * includes endorsements of merge event X from a supermajority of electors. It
 * means that "merge event Y proves that merge event X has been endorsed by
 ...
 *
 * @return a merge event Y that proves a merge event X has been endorsed by
 *           a super majority of electors.
 */
function _findMergeEventProof({ledgerNode, tail, electors}) {
  const supermajority = api.twoThirdsMajority(electors.length);

  const yCandidatesByElector = _findMergeEventYCandidates({tail, electors});

  // TODO: find an intersection of merge events Y

  // TODO: support that merge events Y

  // TODO: do consistent cuts voting and experiments, etc.

  // TODO: result contains x and y
  return result;
}

/**
 * Find all consensus merge event Y candiates for each elector that have
 * ancestors from a supermajority of electors that are descendants of merge
 * events X, where merge events X include ancestors from a supermajority of
 * electors.
 *
 ...
 *           a super majority of electors.
 */
function _findMergeEventYCandidates({tail, electors}) {
  const supermajority = api.twoThirdsMajority(electors.length);

  const branches = Object.keys(tail);
  if(tail.length < supermajority) {
  ...
  For each elector, find the earliest merge events X that include ancestors
  from a supermajority of electors. Then, for each elector, find merge events
  Y that ensure a supermajority of other merge events endorse each of its
  merge events X.
  ...
  const xCandidatesByElector = {};
  const yCandidatesByElector = {};

  // find candidates for merge event X for each elector
  for(const elector of branches) {
    // iterate over each `tail` merge event for each elector
    const candidates = [];
    for(const x of branches[elector]) {
      const result = _findCandidates({x, elector, electors, supermajority});
      candidates.push(...result.candidates
        .filter(c => c.generation === result.maxGeneration)
        .map(c => c.x));
    }
    xCandidatesByElector[elector] = _.uniq(candidates);
  }

  if(Object.keys(xCandidatesByElector).length < supermajority) {
    // non-consensus events X from a supermajority of electors have not yet
    // been collected, so return early
    return null;
  }

  // find candidates for merge event Y for each elector
  for(let elector in xCandidatesByElector) {
    const candidates = [];
    for(const x of branches[elector]) {
      const result = _findCandidates({x, elector, electors, supermajority});
      candidates.push(...result.candidates
        .filter(c => c.generation === result.maxGeneration)
        .map(c => {y: c.x, x})
        // `y` must have a supermajority of elector-descendants or it cannot
        // win a vote
        .filter(r => Object.keys(r.y._des || {}).length >= supermajority));
    }
    yCandidatesByElector[elector] = _.uniqWith(
      candidates, (a, b) =>  a.y === b.y);
    // TODO: are Ys unique with respect to X? Don't yet see why they would be,
    //   but if they aren't, then the solution is to vote on unique Ys but
    //   include multiple Xs in the block.
  }

  return yCandidatesByElector;
}

/**
 * Recursively find merge event proof candidates for an elector. The search
 * starts at the oldest event in history on a particular elector branch (this
 * constitutes generation `0`) and proceeds forward through history.
 *
 ...
 * @param result the max generation to search until and current candidates.
 *
 * @return the max generation for valid candidates and the candidates found.
 */
function _findCandidates({
  x, branch, endorsements = [], electors, supermajority, generation = 0,
  result = {maxGeneration: 0, candidates: []}
}) {
  // TODO: consider optimizing tracking unique endorsements using maps and
  // counters instead of arrays

  if(result.maxGeneration && result.maxGeneration > generation) {
    // max generation threshold passed, no new candidates found
    return result;
  }

  if(electors.includes(x.creator) && !endorsements.includes(x.creator)) {
    endorsements.push(x.creator);
    if(endorsements.length === supermajority) {
      // TODO: can we optimize by checking x._anc to build other elector
      // candidate lists ... or do we need to care about generations to
      // prevent conflicts? (generations will be different for different
      // electors, so each may have arrived at the same candidates at different
      // generations ... meaning they wouldn't necessarily have supported
      // the candidates in the same order)

      // supermajority of endorsements reached, add new candidate and
      // update max generation
      result.candidates.push({generation, x});
      result.maxGeneration = generation;
      return result;
    }
  }

  // if the remaining elector branches that endorse `x` are insufficient to
  // meet a supermajority, then we will find no new results
  const branches = x._des || {};
  const remaining = _.difference(branches - endorsements);
  const required = supermajority - endorsements.length;
