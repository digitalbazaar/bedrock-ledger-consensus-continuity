...
function _findConsensusMergeEventProof(
  ...
  for(let elector in yByElector) {
    ...
    while(!result.consensus && next) {
      _tally(
        {history, event: next, yByElector, electors, yAncestryMaps, result});
      next = next._treeChild;
    }
  }
...
function _tally({history, event, yByElector, electors, yAncestryMaps, result}) {
  ...
  const ancestors = _getPathAncestors({history, event, yAncestryMaps});
  ...
  for(const ancestor of ancestors) {
    ...
    if(!ancestor._supporting) {
      if(y === ancestor) {
        ...
        _tally(
          {history, event: ancestor, yByElector, electors,
  ...
  const previousChoice = (ancestors.length === 0) ? null : _.find(
  ...
  if(!previousChoice) {
    ...
    tally.push({set: event._supporting, count: 1});
  }
...
function _getPathAncestors({history, event, yAncestryMaps, ancestors = []}) {
  ...
  for(const parent of event._parents) {
    ...
    if(parent.eventHash in yAncestryMaps) {
      // do not recurse into parent, it is in the halt map
      continue;
    }
    ancestors.push(parent);
    _getPathAncestors({history, event: parent, yAncestryMaps, ancestors});
  }
