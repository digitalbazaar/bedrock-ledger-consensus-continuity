    ...
    consensus: ys.map(y => {
      ...
      return {
        ...
        proof: _flattenDescendants(
          {ledgerNode, x, y, descendants: y._descendants})
      };
    }),
...
function _findMergeEventProofCandidates({ledgerNode, tails, electors}) {
  ...
  for(const elector of electorsWithTails) {
    ...
    // TODO: optimization: create `descendants` map here externally for each X
    //   and then pass it as a "halting map" to the respective search Y below
    //   i.e., any event that appears in the X `descendants` map cannot
    //   possibly include endorsements *of* X and its paths can be skipped
    //   when searching for a Y.

    // find earliest `x` for the elector's tail
    const result = _findDiversePedigreeMergeEvent(
      {ledgerNode, x: electorTails[0], electors, supermajority});
    if(result) {
  ...
  for(const elector in xByElector) {
    ...
    if(result) {
      ...
      result._descendants = descendants;
    }
  }
...
function _findDiversePedigreeMergeEvent(
  ...
  const ancestryMap = _buildAncestryMap(x);
  const electorMap = {};
  electors.forEach(e => electorMap[e.id] = true);
  ...
  while(treeDescendant) {
    ...
    if(_hasSufficientEndorsements(
      {ledgerNode, x, descendants, electorMap, supermajority})) {
...
function _hasSufficientEndorsements(
  ...
  const endorsements = {[_getCreator(x)]: true};
  let total = 1;
  // remove regular events from `x` children
  let next = [];
  for(let child of x._children) {
    ...
    if(child.eventHash in descendants) {
      ...
      if(!(creator in endorsements) && creator in electorMap) {
        endorsements[creator] = true;
        total++;
        //console.log('total', total, 'supermajority', supermajority);
        //console.log('electors', electorMap);
...
function _findConsensusMergeEventProof(
  ...
  allYs.forEach(y => {
    console.log('Start sync _hasAncestors');
    const found = {};
    _hasAncestors({
      target: y,
      candidates: allYs,
    ...
    console.log('End sync _hasAncestors');
...
function _tally(
  ...
  if(!topChoice || topChoice.count === 1) {
    ...
    if(!topChoice) {
      ...
      // all seen Ys
      union = _allSeenYs({event, history, allYs, yAncestryMaps});
    } else {
...
function _hasAncestors(
  {target, candidates, min = candidates.length,
    eventMap, candidateAncestryMaps, found}) {
  for(const parent of target._parents) {
    // FIXME: this can loop indefinitely (should never happen and be prevented
    // elsewhere)
    //console.log('_hasAncestors parent', parent.eventHash);
...
api.getBlockElectors = (ledgerNode, blockHeight, callback) => {
  async.auto({
    ...
    electors: ['config', 'latestBlock', (results, callback) => {
      ...
      // TODO: reduce electors to height multiple of `3f + 1` , i.e.
      // electors % 3 === 1 or electors < 4. -- electors MUST be a multiple
      // of `3f + 1` for validity
