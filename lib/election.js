...
function _verifyVote(vote, callback) {
  ...
  _verifyVoteSignature(vote, err => {
    ...
    _getManifest(vote.voter, vote.manifestHash, callback);
  });
}
...
function _getManifest(voterId, manifestHash, callback) {
  ...
  async.auto({
    ...
    getLocal: ['getVoter', (results, callback) => api._storage.manifests.get(
    ...
    getPeer: ['getLocal', (resultsAlpha, callback) => {
      ...
      async.auto({
        ...
        syncEvents: ['hashes', 'manifest', (resultsBeta, callback) => {
          const peerEvents = resultsBeta.manifest.item;
          const toGet =
            peerEvents.filter(e => !resultsBeta.hashes.includes(e));
