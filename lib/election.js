 ...
 *
 * @param history the view of history from one ledger node.
 * @param electors the current electors.
 *
...
function _getElectorBranches({
  ...
  currentBranches.forEach(x => {
    ...
    if(child !== null && child._anc[x]) {
      ...
      if(!event._des[x].includes(child)) {
...
/**
 * Finds the earliest merge events that were endorsed by a supermajority of
 * electors.
 *
 * Merge events are endorsed when they are merged into another peer's branch
 * via a child merge event. Therefore, if merge event A has another
 * merge event B in its ancestry, then merge event A is said to endorse
 * merge event B.
 *
 * @param ledgerNode the local ledger node.
 * @param history recent history rooted at the ledger node's local branch.
 ...
 *
 * @return a earliest set of merge events (can be empty) that were endorsed by
 *           a supermajority of the given electors.
 */
function _findEarliestEndorsedEvents({ledgerNode, history, electors}) {
  // TODO: implement
}
