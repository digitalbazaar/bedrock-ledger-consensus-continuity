    ...
    xAncestors: callback => _getAncestors(
      {ledgerNode, eventHash: allXs}, (err, result) => {
        ...
        for(let i = 0; i < result.length; ++i) {
          rVal.event.push(result[i].event[0]);
...
function _findMergeEventProof({ledgerNode, history, tails, electors}) {
  ...
  const ys = _findConsensusMergeEventProof(
    {ledgerNode, eventMap: history.eventMap,
    yByElector: yCandidatesByElector, supermajority});
  if(ys.length === 0) {
    // no consensus yet
    return ys;
  }
...
function _findConsensusMergeEventProof(
  {ledgerNode, eventMap, yByElector, supermajority}) {
  logger.verbose(
  ...
  // find every C, the point at which an elector can reach consensus, known
  // at this point in history (based on all Y's known to electors)
  const cByElector = {};
  for(let elector in yByElector) {
    const localY = yByElector[elector];

    // map for tracking Y's found
    const found = {};
    // exclude localY (already found)
    const required = supermajority - 1;
    const candidates = allYs.filter(y => y !== localY);
    candidates.forEach(y => found[y.eventHash] = false);

    let event = localY;
    console.log('CHECKING LOCAL Y', event);
    const haltAncestryMap = {};
    while(event) {
      _hasAncestors({
        target: event,
        ...
        found,
        haltAncestryMap: haltAncestryMap
      });
      console.log('FOR EVENT', event, 'FOUND Ys:', found);
      const total = Object.keys(found)
        .filter(e => found[e])
        .map(eventHash => eventMap[eventHash]);
      if(total.length >= required) {
        // ensure localY is included in Ys
        total.push(localY);
        cByElector[elector] = {
          c: event,
          y: total
      ...
      // do not recheck tree parent
      haltAncestryMap[event.eventHash] = true;
      // move onto tree child
      event = event._treeChild;
    }
  }

  logger.verbose(
    'Continuity2017 C merge event candidates found for ledger node ' +
    ledgerNode.id, {ledgerNode: ledgerNode.id, cByElector});
  console.log(
    'Continuity2017 C merge event candidates found for ledger node ' +
    ledgerNode.id, {ledgerNode: ledgerNode.id, cByElector});

  let allCs = _.values(cByElector).map(e => e.c);

  // if no C's were found, we're done, no result
  if(allCs.length === 0) {
    return [];
  }

  // if more than 1 C was found, prune descendant C's
  if(allCs.length > 1) {
    // build map of each C's ancestry for quick checks of halting conditions
    const cAncestryMaps = _buildAncestryMaps(allCs);

    // prune all C's that are descendants of other C's
    allCs = _pruneDescendants(
      {events: allCs, ancestryMaps: cAncestryMaps, eventMap});
  }

  // pick any remaining C and get its associated entry, the consensus algorithm
  // is designed so that any C will use the same Y's
  const entry = cByElector[_getCreator(allCs[0])];

  // prune all Y's that are descendants of other Y's and return them
  entry.y = _pruneDescendants(
    {events: entry.y, ancestryMaps: yAncestryMaps, eventMap});

  logger.verbose(
    'Continuity2017 C merge event found for ledger node ' +
    ledgerNode.id, {ledgerNode: ledgerNode.id, entry});
  console.log(
    'Continuity2017 C merge event found for ledger node ' +
    ledgerNode.id, {ledgerNode: ledgerNode.id, entry});
...
function _asyncConsensusWithoutRounds({history, yByElector, electors}) {
  const result = {consensus: null};
  for(let elector in yByElector) {
    // start by supporting elector's own Y
    const start = yByElector[elector];
    start._supporting = [start];

    let next = start;
    while(!result.consensus && next._treeChild) {
      _tally({history, event: next._treeChild, yByElector, electors, result});
    }
  }
...
function _tally({history, event, yByElector, electors, result}) {
  // consensus reached, no further tallying required
  if(result.consensus) {
    return;
  }

  // gather all events from event to event._treeParent and put them in set S.
  // (need to perhaps pull this code from the ancestor check or the endorsement
  // check)
  const ancestors = _getPathAncestors(
    {history, event, parent: event._treeParent});

  let tally = [];

  for(const ancestor in ancestors) {
    const creator = _getCreator(ancestor);
    const y = yByElector[creator];
    if(!y) {
      // no `Y` for this creator, so it can't vote yet, skip it
      continue;
    }

    if(!ancestor._supporting) {
      if(y === ancestor) {
        // if `ancestor` is a `Y`, it automatically supports itself
        ancestor._supporting = ancestor;
      } else {
        // use `tally` to recursively compute what `ancestor` supports
        _tally(ancestor);
        if(result.consensus) {
          break;
        }
    ...
    // FIXME: we need to ensure we only count votes once, and the vote we want
    //   to count from each elector is the most recent one, i.e. it's possible
    //   that we'll go through the same elector many times between our two
    //   merge events

    // tally result so far
    const tallyResult = _.find(tally, _findSetInTally(ancestor._supporting));
    if(!tallyResult) {
      tally.push({
        set: ancestor._supporting,
        count: 1
      });
    } else {
      // ensure same instance of set is used for faster comparisons
      ancestor._supporting = tallyResult.set;
      tallyResult.count++;
    }
  }
  ...
  tally.sort((a, b) => {
    ...
    // the hash of the sorted Ys is combined with the elector id
    a.hash = a.hash || (_hashSet(a.set) + _sha256(a.id));
    b.hash = b.hash || (_hashSet(b.set) + _sha256(b.id));
...
/*
Create a map/array T to tally support for sets of Ys.
For each event `e` in S, see if `_supporting` is set. If not:
If `e` is a Y, set `e._supporting` to [Y]
If `e` is not a Y, call tally(e)
Note: there’s a pretty big optimization to be found here where we can skip refinding a bunch of the same ancestors more than once when we recurse
Add `e._supporting` to the appropriate spot in T, creating it as needed.
Sort T by support.
If the first element in T is supported by a supermajority, then return the set of Ys as the consensus result (set consensus = set of Ys).
Otherwise, total all support for sets of Y other than `event._treeParent._supporting`. If this number is less than ⅓ of the electors, then set `e._supporting` to `event._treeParent._supporting`.
Otherwise, filter T to include only all sets of Y with the most support.
If there is only one element, set `e._supporting` to it and return.
If there is more than one, hash each set of Ys, sorting the Ys in lexicographical order, and then concatenate the node’s ID and then hash the whole thing again. Then use these hashes to sort T. (They can be computed on the fly within the sort function like we do with electors). This step introduces pseudorandomness, guaranteeing eventual termination with a probability of one.
Set `e._supporting` to the first element in T and return.
*/
}
...
function _findSetInTally(set) {
  return tallyResult => {
    ...
    return _.difference(tallyResult.set, set).length === 0;
  };
}
...
function _hashSet(set) {
  ...
  return set.hash = _sha256(
    set.sort((a, b) => a.hash.localeCompare(b.hash)).join(''));
}

function _buildAncestryMaps(events) {
  const ancestryMaps = {};
  events.forEach(e => {
    const map = ancestryMaps[e.eventHash] = {};
    let parent = e;
    while(parent) {
