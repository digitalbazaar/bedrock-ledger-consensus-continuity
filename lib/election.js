...
bedrock.events.on('bedrock.start', async () => {
  ...
  consensusPool = workerpool.pool(
    path.join(__dirname, 'continuity-workerpool-worker.js'), {maxWorkers});
 ...
 */
api.findConsensus = async ({
  ledgerNode, history, blockHeight, electors, state
}) => {
  ...
  try {
    result = await _runConsensusInPool({
      ledgerNode, history, blockHeight, witnesses: electors, state
    });
  } finally {
 ...
 *
 * The electors will be returned using the given data structure:
 *
 ...
 */
api.getBlockElectors = async ({ledgerNode, blockHeight}) => {
  ...
  await _cache.consensus.setElectors(
    {blockHeight, electors: result, ledgerNodeId});
