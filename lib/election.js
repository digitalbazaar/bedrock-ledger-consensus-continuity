 ...
 */
    // TODO: get all events
    // TODO: validate all events, serially in order, those that fail
    // must be removed from the list of events to include in the manifest
function _createEventManifest(ledgerNode, blockHeight, callback) {
  ...
  async.auto({
    ...
    getEvents: ['getHashes', (results, callback) =>
      async.map(results.getHashes, (eventHash, callback) =>
    ...
    validate: ['getConfig', 'getEvents', (results, callback) => {
      ...
      if(!(configEvent.eventValidator &&
        ...
        // no validators for this ledger, pass all hashes
        return callback(null, results.getHashes);
      }
      const requireEventValidation =
        configEvent.requireEventValidation || false;
      async.filter(
        results.getEvents, event => brLedger.consensus._validateEvent(
          event, configEvent.eventValidator, {requireEventValidation}, err => {
            if(err) {
              // the event did not pass validation, should the event be marked?
            }
            callback(null, true);
          }
        ));
    ...
    create: ['validate', (results, callback) => {
      if(results.validate.length === 0) {
        return callback();
      }
      _createManifest(
        ledgerNode.id, blockHeight, 'Events', results.validate, callback);
    }]
  }, (err, results) => {
...
function _getManifest(voterId, manifestHash, callback) {
  ...
  // TODO: implement the remainder of this in `client.js` ... except
  //  `validateEvents` which we run after the client runs
  ...
  // TODO: once all events are received, run brLedger.consensus.validateEvents
