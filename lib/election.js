...
api.findConsensus = ({ledgerNode, history, electors}, callback) => {
  ...
  console.log('XXXXXXXXXXXXXXXX', allXs);
  async.auto({
    xAncestors: callback => _getAncestors(
      {ledgerNode, eventHash: allXs}, (err, result) => {
        ...
        proofResult.event = result.map(e => e.event[0]);
        callback();
      })
  }, err => {
    if(err) {
 ...
 *
 * @return an array of merge event X and Y pairs where each merge event Y and
 *         its history proves its paired merge event X has been endorsed by a
 *         super majority of electors -- another key, `proof` is also returned
 *         that includes `y` and its direct ancestors until `x`, these, in
 *         total, constitute endorsements of `x`.
 */
