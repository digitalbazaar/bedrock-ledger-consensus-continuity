...
function _findCandidates({
  ...
  let descendants = x._des[branch];
  if(!descendants) {
    // no more descendants so no more candidates
    return result;
  }

  // if the remaining descendants that endorse `x` are insufficient to meet
  // a supermajority, then we will find no new results
  const remaining = _.difference(descendants - endorsements);
