...
function _findDiversePedigreeMergeEvent({
  ...
  console.log('CREATOR OF EVENT', creator, x._generation);
  if(electors.includes(creator) && !endorsements.includes(creator)) {
    endorsements.push(creator);
    console.log('NEW ENDORSEMENT ON BRANCH', branch, x._generation, endorsements);
  }
  ...
  for(let child of x._children) {
    ...
    // recurse into child
    console.log('RECURSING into child', child.eventHash, ' of parent', x.eventHash);
