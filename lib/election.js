...
function _findConsensusMergeEventProofOLD(
...
// FIXME: rename to `_findConsensusMergeEventProof` once old one is replaced
function _findConsensusMergeEventProof(
  ...
  for(const elector in yByElector) {
    ...
    let next = start;
    while(!result.consensus && next) {
      _tally(
        {history, event: next, yByElector, allYs,
...
function _tally(
  {history, event, yByElector, allYs, electors, yAncestryMaps, result}) {
  console.log('FIND SUPPORT FOR event:', event);
  ...
  // FIXME: optimize to avoid retrieving these many times
  // gather all ancestors between `event` and all `Ys`
  const ancestors = _getPathAncestors({history, event, allYs, yAncestryMaps});
  console.log('ancestor count', ancestors.length, ancestors.map(r => r.eventHash));
  ...
  for(const ancestor of ancestors) {
    ...
    if(!ancestor._supporting) {
      if(y === ancestor) {
        ...
        console.log('_tally RECURSING INTO', ancestor);
        _tally(
          {history, event: ancestor, yByElector, allYs, electors,
    ...
    if(!ancestors.includes(ancestor._treeChild)) {
      console.log('ANCESTOR VOTE COUNTED', _getCreator(ancestor), ancestor);
      // tally result so far
      ...
      console.log('ALREADY COUNTED VOTE', _getCreator(ancestor), ancestor);
    }
  }
  console.log('YOGI');
  if(result.consensus) {
  ...
  let previousChoice = (ancestors.length === 0) ? null : _.find(
    tally, _findSetInTally(event._treeParent._supporting));
  if(!previousChoice) {
    // if there's no previous choice, we're at `Y`, vote for self
    event._supporting = [event];
    previousChoice = {set: event._supporting, count: 1};
    tally.push(previousChoice);
  }
  ...
  tally.sort((a, b) => {
    ...
    // the hash of the sorted Ys is combined with the current event's creator
    a.hash = a.hash || _sha256(_hashSet(a.set) + _sha256(creator));
    b.hash = b.hash || _sha256(_hashSet(b.set) + _sha256(creator));

    // 2. sort by hash
    return a.hash.localeCompare(b.hash);
  });

  console.log('TALLIES SO FAR', tally);
  ...
  // TODO: consider creating a new set of Ys with as many Ys as are seen
  //   rather than simply supporting a particular choice of Y, see FIXME below

  // choose top valid choice for this event, based on its ancestry
  for(const tallyResult of tally) {
    ...
    // TODO: use `event._pathAncestors` to check this instead?
    if(_hasAncestors({
      target: event, candidates: tallyResult.set, eventMap: history.eventMap,
      candidateAncestryMaps: ancestryMaps, found: {}})) {
      event._supporting = tallyResult.set;
      // FIXME: if `event._supporting` === `previousChoice.set`, then try to
      //   add in more Ys that are ancestors to this event
      break;
    }
  }
}
...
function _getPathAncestors(
  {history, event, allYs, yAncestryMaps, ancestors = []}) {
  if(!event._pathAncestors) {
    event._pathAncestors = [];
    for(let parent of event._parents) {
      ...
      if(ancestors.includes(parent)) {
        // parent already included
        continue;
      }
      // TODO: optimize having to compute this (likely can get cheaper from
      //   other previously executed algorithms, or could at least check own
      //   set of parents for `_descendsFromY`)
      if(!('_descendsFromY' in parent)) {
        parent._descendsFromY = _hasAncestors({
          target: parent,
          candidates: allYs,
          min: 1,
          candidateAncestryMaps: yAncestryMaps,
      ...
      if(parent._descendsFromY) {
        // parent descends from a Y, accept it and recurse
        ancestors.push(parent);
        _getPathAncestors(
          {history, event: parent, allYs, yAncestryMaps,
            ancestors: event._pathAncestors});
      }
    }
  }

  if(ancestors.length === 0) {
    ancestors.push(...event._pathAncestors);
  } else {
    // add uniquely
    for(const ancestor of event._pathAncestors) {
