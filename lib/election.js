    ...
    electors: ['config', 'latestBlock', (results, callback) => {
      ...
      if(electors.length > electorCount) {
        ...
        electors.sort((a, b) => {
          ...
          return _sha256(a.id).localeCompare(_sha256(b.id));
        });
...
api.certify = (ledgerNode, electionTopic, votes, callback) => {
  // TODO: should be able to see if the vote manifest is already stored
  //   and skip certification if so
...
function _getManifestFromPeer(
  ...
  async.auto({
    ...
    sync: ['hashes', 'manifest', (results, callback) => {
      if(type === 'Events') {
        const peerEvents = results.manifest.item;
        const toGet = peerEvents.filter(e => !results.hashes.includes(e));
        return async.each(
          toGet,
          (eventHash, callback) => _getEventFromPeer(
            ledgerNode, peerId, eventHash, callback),
          callback);
      }
      // type is 'RollCall', we already have the events votes from the peer
      ...
      // TODO: make sure manifest.items are in the database as events votes
      // TODO: must also ensure that manifest.items has >= twoThirds of
      //  electors.length... otherwise the roll-call is insufficient to get
      //  consensus!
      callback();
    }],
    ...
    store: ['validate', (results, callback) => {
      ...
      api._storage.manifests.add(ledgerNode.id, results.manifest, callback);
    }]
...
function _validateEvents(ledgerNode, hashes, blockHeight, callback) {
  if(blockHeight === undefined) {throw new Error('YO!');}
  async.auto({
