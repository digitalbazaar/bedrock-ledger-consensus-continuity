...
function _flattenDescendants({ledgerNode, x, descendants}) {
  ...
  for(let child of x._children) {
    ...
    // TODO: move up, inside conditional?
    next.push(child);
  }
...
function _findMostRecentVotingDescendants(
  {ledgerNode, x, descendants, electorSet}) {
  const result = {};
  // remove regular events from `x` children
  let next = [];
  for(let child of x._children) {
    ...
    if(child.eventHash in descendants) {
      ...
      if(electorSet.has(creator)) {
        _useMostRecentVotingEvent(
          {ledgerNode, voting: result, elector: creator, votingEvent: child});
      }
      next.push(child);
    }
  }
  ...
  while(next.length > 0) {
    ...
    next = [];
    for(const event of current) {
      //console.log('_findMostRecentVotingDescendants event', {
      //  creator: _getCreator(event),
      //  generation: event._generation,
      //  hash: event.eventHash
      //});
      ...
      if(d) {
        ...
        _useMostRecentVotingEvent(
          {ledgerNode, voting: result, elector: creator, votingEvent: event});
        // `event` is in the computed path of descendants
        next.push(...d);
      }
    }
...
function _useMostRecentVotingEvent({ledgerNode, voting, elector, votingEvent}) {
  ...
  if(elector in voting) {
    const existing = voting[elector];
...
function _findConsensusMergeEventProof(
  ...
  allYs.forEach(y => {
    ...
    // TODO: consider removing `_markYDescendants` if unnecessary or does
    //   not assist in optimizing
    //_markYDescendants(y);
  });
  ...
  let consensus = _tallyBranches(
    {ledgerNode, yByElector, allYs, electors, yAncestryMaps});
...
function _tallyBranches(
  ...
  1. Iterate through each Y branch, starting at Y and moving down its
     tree children.
  2. Find all descendants between the current and the previous tree child.
  3. Filter the descendants into a map of elector => most recent event
     created by that elector, creating the set of events that are
     participating in an experiment to see what Y candidates the various
     nodes are supporting. If a byzantine node is detected, mark the
     elector's entry as `false`.
  ...
  */
  ...
  while(next.length > 0) {
    ...
    next = [];
    for(const event of current) {
      // TODO: compute votesToTally if not yet computed (if event._supporting
      //   then skip tallying in _tally)
      if(!event._supporting) {
        if(!event._voting) {
          // ancestry map used to halt searches (includes as many ancestors
          // of the tree parent as are known at this point)
          const parentAncestryMap = {};
          if(event._treeParent._experimentAncestors) {
            for(let key in event._treeParent.experimentAncestors) {
              parentAncestryMap[key] = true;
            }
          }
          if(event._xDescendants) {
            for(let key in event._xDescendants) {
              parentAncestryMap[key] = true;
            }
          }
          // determine ancestors that will partipicate in the experiment
          event._experimentAncestors = {};
          _findDescendantsInPath({
            ledgerNode,
            x: event._treeParent,
            y: event,
            descendants: event._experimentAncestors,
            ancestryMap: parentAncestryMap
          });
          event._voting = _findMostRecentVotingDescendants(
            {ledgerNode, x: event._treeParent,
              descendants: event._experimentAncestors, electorSet});
        }
        if(_.values(event._voting).some(e => !('_supporting' in e))) {
      ...
      const result = _tally(
        {ledgerNode, event, allYs, electors, yAncestryMaps});
...
function _tally(
  ...
  const creator = _getCreator(event);
  if(event._supporting) {
    // event is already supporting a candidate (should only happen with
    // initial Y)
    event._voting = {[creator]: event};
  }

  // tally votes
  const tally = [];
  _.values(event._voting).forEach(e => {
    if(e === false) {
  ...
  tally.sort((a, b) => {
    ...
    // FIXME: may be better to only run `_compareTallies` as a secondary
    //  sort once only tied tallies remain, otherwise hashing may be run
    //  unnecessarily on low-count tallies
  ...
  if(event._voting[creator] === false) {
  ...
  // choice)
  const previousChoice = event._voting[creator] ? _.find(
    tally, _findSetInTally(event._voting[creator]._supporting)) : null;
  if(previousChoice) {
...
function _tallyOLD(
  ...
  // TODO: ensure logger.verbose of `tally` is not slow
  /*logger.verbose('Continuity2017 _tally tally',
    {ledgerNode: ledgerNode.id, tally});*/
  // TODO: remove me
  /*console.log('VOTE TALLY', ledgerNode.id, tally.map(t => ({
