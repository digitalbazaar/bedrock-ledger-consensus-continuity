...
function _createBranchOLD({event, branch, electors}) {
 ...
 * @param treeParent the current tree parent event, used to track generations.
...
function _findDiversePedigreeMergeEvent({
  ...
  console.log('CREATOR OF EVENT', creator, generation);
  if(electors.includes(creator) && !endorsements.includes(creator)) {
    endorsements.push(creator);
    console.log('NEW ENDORSEMENT ON BRANCH', branch, generation, endorsements);
  }
  ...
  if(creator === branch) {
    ...
    console.log('NEW GENERATION, ' + generation + ', ON BRANCH', branch);
  ...
  for(let child of x._children) {
    ...
    if(child._treeParent === treeParent) {
      nextGeneration++;
      nextTreeParent = child;
    }
    ...
    _findDiversePedigreeMergeEvent({
      ...
      generation: nextGeneration, treeParent: nextTreeParent,
      result});
  }
