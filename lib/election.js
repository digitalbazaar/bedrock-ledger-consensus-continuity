  ...
  async.auto({
    ...
    nonConsensus: ['exists', (results, callback) => {
      async.whilst(() => remaining > 0, callback => {
        query.eventHash.$in = parentHashes.slice(start, start + batchSize)
          .map(h => ledgerNode.storage.driver.hash(h));
        start += batchSize;
...
function _getElectorSelectionMethod({config}, callback) {
  ...
  brLedgerNode.use(electorSelectionMethod.type, callback);
}
