...
function _tally(
  ...
  // TODO: presently, electors may vote if they have any descendants of
  //   any Y -- but we could change this so they must have their own Y
  //   in order to vote -- advantages/disadvantages need to be considered

  // get previously supported value (there will not be one if the tree parent
  // is not a descendant of any Y, in which case we are making our first
  // choice)
  const previousChoice = event._treeParent._supporting ? _.find(
    tally, _findSetInTally(event._treeParent._supporting)) : null;
  if(previousChoice) {
  ...
  // TODO: if we are the deciding vote, is it safe to mark consensus as
