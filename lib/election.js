...
function _tally({ledgerNode, event, yByElector, blockHeight, electors}) {
  ...
  */
  ...
  If the event is a confirmation (i.e. it has a precommit):
    Compare the top choice to the precommit and if it match, create
    a diverse pedigree decision point in the future and return.
    Otherwise, continue...
  */
  if(event._preCommit) {
    ...
    electors: ['config', 'latestBlock', (results, callback) => {
      ...
      electors.sort((a, b) => {
        ...
        if(a.weight !== b.weight) {
          // FIXME: re-enable, temporarily disabled to force elector cycling
