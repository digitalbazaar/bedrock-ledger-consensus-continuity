  ...
  return {
    ...
    consensus: ys.map(y => {
      ...
      return {
        y,
        x,
        proof: _findAncestorsInPath({ledgerNode, x, y})
      };
    }),
  ...
  for(const elector of electorsWithTails) {
    ...
    */
    const result = _findDiversePedigreeMergeEvent(
      {x: electorTails[0], branch: elector, electors, supermajority, cache});
    if(result) {
      //console.log('***X found for', elector, ' at generation ',
      //  result._generation, result);
      //console.log('cache', cache);
      //console.log(
      //  'TOTAL ENDORSEMENTS', cache[result.eventHash].endorsements.length);
...
function _findDiversePedigreeMergeEvent({
  ...
  for(let child of x._children) {
    ...
    candidate = _findDiversePedigreeMergeEvent({
      ...
      endorsements: endorsements.slice(), cache, candidate});
...
function _findAncestorsInPath({ledgerNode, x, y}) {
  const ancestryMap = _buildAncestryMap(x);
  const results = [];
  let next = y._parents.map(p => ({ancestors: [y], event: p}));
  while(next.length > 0) {
    const current = next;
    next = [];
    for(const {ancestors, event} of current) {
      if(event.eventHash === x.eventHash) {
        // path to X found
      ...
      next.push(...event._parents.map(
        p => ({ancestors: ancestors.slice(), event: p})));
    }
  }
  return _.uniq([].concat(...results));
}
