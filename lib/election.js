...
function _tally({ledgerNode, event, yByElector, blockHeight, electors}) {
  ...
  as long as there are *AT LEAST* two to union (ignoring byzantine?):
    Then set next choice to precommit union.
    And continue...

  Note: This rule prevents new little ys from entering the system and
  getting precommitted concurrently, causing an inconsistency.

  If only one precommit and node doesn't fail, it is necessarily confirmed.
  If two and neither fails, the larger will be confirmed and smaller is not.
  If larger fails, it will come back to life and confirm or something larger
     exists that will be confirmed.
...
set as your precommit.
  ...
  */
  if(hasSupermajority) {
    /*
    If the event is a confirm point:
...
function _findUnionPreCommitSet(event) {
  ...
  _.values(event._votes).forEach(r => {
    ...
    if(!union) {
      ...
      union = _.uniq(union.concat(supporting));
      count++;
    }
  });
  // must be AT LEAST 2 that were unioned together; this rule is applied to
  // prevent non-precommitted nodes that contain a late little y from coming
  // in and getting their votes switched by a smaller concurrent precommit
  // (another larger one is concurrent), leading to an inconsistency where
  // the smaller set can get confirmed because its support did not see the
  // precommit of the larger set
  if(count < 2) {
    return null;
  }
  return union;
