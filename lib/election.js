...
function _findConsensusMergeEventProof(
  ...
  // initialize all Ys to support themselves and mark all Y descendants
  allYs.forEach(y => {
    y._supporting = [y];
    _markYDescendants(y);
  });

  // go through each Y's descendants looking for consensus
  const result = {consensus: null};
  for(const elector in yByElector) {
    let next = yByElector[elector];
...
function _tally(
  ...
  console.log('FIND VOTES SEEN BY event:', event);
  event._votes = {};
  ...
  for(const parent of event._parents) {
    ...
    if(!parent._votes) {
      ...
      console.log('_tally RECURSING INTO', parent);
      _tally(
        {history, event: parent, yByElector, allYs, electors,
    ...
    console.log('parent got votes', parent._votes);
    ...
    for(const elector in parent._votes) {
      ...
      if(elector in event._votes) {
        ...
        if(latest === votingEvent) {
          console.log('EXISTING ANCESTOR VOTE REPLACED', elector, votingEvent);
          event._votes[elector] = votingEvent;
        }
      } else {
        console.log('NEW ANCESTOR VOTE FOUND', elector, votingEvent);
        event._votes[elector] = votingEvent;
      }
    }
  }
  ...
  // TODO: consider creating a new set of Ys with as many Ys that are seen
