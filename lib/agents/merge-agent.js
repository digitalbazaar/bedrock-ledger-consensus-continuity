...
module.exports = class MergeAgent extends ContinuityAgent {
  ...
  _validateCache({creatorId, ledgerNode}, callback) {
    const ledgerNodeId = ledgerNode.id;
    async.auto({
      cacheHead: callback => _events._getLocalBranchHead(
        {creatorId, ledgerNode}, callback),
      mongoHead: callback => _events._getLocalBranchHead(
        {useCache: false, creatorId, ledgerNode}, callback),
      consistencyCheck: ['cacheHead', 'mongoHead', (results, callback) => {
        ...
        console.log('EVENTS REPAIRCACHE');
  ...
  _work() {
    ...
    async.auto({
      validateCache: callback => this._validateCache(
        {creatorId, ledgerNode}, callback),
