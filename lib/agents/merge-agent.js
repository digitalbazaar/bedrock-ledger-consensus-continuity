...
module.exports = class MergeAgent extends ContinuityAgent {
  ...
  _work() {
    ...
    const {creatorId, ledgerNode, ledgerNodeId} = this;
    async.auto({
      notifyFlag: callback => callbackify(_cache.gossip.notifyFlag)(
      ...
      notify: ['merge', 'notifyFlag', (results, callback) => {
        // if there is no new merge, and a gossip session has occurred
        // (notifyFlag was cleared) since the last merge, return
        if(!results.merge && results.notifyFlag === null) {
          return callback();
        }
        ...
        callbackify(async () => {
          return Promise.all([
            ...
            await this.gossipAgent.sendNotification()
          ]);
        }).bind(this)(err => {
          if(err) {
            logger.error(
