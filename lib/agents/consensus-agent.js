...
module.exports = class ConsensusAgent extends ContinuityAgent {
  ...
  _onMessage(channel, message) {
    if(message === 'merge') {
      // FIXME: check if ledgerNode's peer ID is in `priorityPeers`; if not,
      // ignore this message (must be implemented simultaneously with listening
      // for `priorityGossip` messages or can get stuck)
      // if(!this.priorityPeers.has(this.peerId)) {
      //   return;
      // }
  ...
  async _extendBlockchain() {
    ...
    while(!this.halt) {
      ...
      if(!this.newPriorityMergeEvent) {
        // FIXME: check if priorityPeers has changed before updating it
        // because we may need to run consensus one more time after writing
        // a new value to the cache because a `gossipPriority` message may
        // not have been generated because the gossip agent was using a stale
        // cache
        this.priorityPeers = new Set(result.priorityPeers);
        // FIXME: write `this.priorityPeers` set to cache
        if(this.priorityPeers.has(peerId)) {
          await cache.client.publish(
            `continuity2017|event|${ledgerNodeId}`, 'needsMerge');
        }
        break;
      }
    }
