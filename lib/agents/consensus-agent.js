...
module.exports = class ConsensusAgent extends ContinuityAgent {
  ...
  _work() {
    this.working = true;
    async.auto({
      hasRegular: callback => this._hasNewRegularEvents(callback),
      extend: ['hasRegular', (results, callback) => {
  ...
  _hasNewRegularEvents(callback) {
    ...
    const fields = {_id: 0, 'meta.continuity2017.type': 1};
    const hint = {'continuity2': 1};
    collection.findOne(query, {fields, hint}, (err, result) =>
      callback(err, !!result));
  }
