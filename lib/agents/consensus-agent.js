...
module.exports = class ConsensusAgent extends ContinuityAgent {
  ...
  _extendBlockchain(callback) {
    ...
    async.until(condition, loop => {
      async.auto({
        initState: callback => this._updateState({state}, callback),
        // NOTE: ***DO NOT LOG CONSENSUS TO DEBUG OR CONSOLE, IT CAN BE HUGE***
        consensus: ['initState', (results, callback) => this._findConsensus(
          {state}, (err, result) => {
            console.log(`${Date.now()} STATE ELECTORS BLOCKHEIGHT`,
              err, state.electors.length, state.blockHeight);
            if(err) {
              // TODO: RECOVERY_MODE: change electors to `recoveryElectors`
              if(err.name === 'NewElectorsRequiredError') {
                console.log('############## ENABLE RECOVERY ################');
                throw new Error('^^^^^^^^^^^^');
                // TODO: set `recoveryMode` flag to `true` and update
                // state (or at least electors) to get new ones ... and
                // can call `callback()` and swallow the error
              }
  ...
  _updateState({state}, callback) {
    ...
    async.auto({
      ...
      getElectors: ['consistencyCheck', (results, callback) =>
        // TODO: RECOVERY_MODE: keep a `recoveryMode` flag that gets reset to
        // `false` whenever consensus is found ... and gets set to `true`
        // when the above error is caught, pass as `recoveryMode` to this
        // function
      ...
      update: ['getElectors', (results, callback) => {
        state.electors = results.getElectors.electors;
        state.recoveryElectors = results.getElectors.recoveryElectors;
        state.recoveryGenerationThreshold =
          results.getElectors.recoveryGenerationThreshold;
        state.recoveryDecisionThreshold =
          results.getElectors.recoveryDecisionThreshold;
