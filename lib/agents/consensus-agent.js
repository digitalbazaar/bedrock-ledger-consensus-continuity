...
module.exports = class ConsensusAgent extends ContinuityAgent {
  ...
  _extendBlockchain(callback) {
    ...
    async.until(condition, loop => {
      async.auto({
        initState: callback => this._updateState(
          {recoveryMode, state}, callback),
        ...
        consensus: ['initState', (results, callback) => this._findConsensus(
          {state}, (err, result) => {
            ...
            if(result.consensus) {
              ...
              console.log('SETTING RECOVERY MODE OFF');
  ...
  _updateState({recoveryMode, state}, callback) {
    const {ledgerNode, ledgerNodeId} = this;
    async.auto({
      ...
      getElectors: ['consistencyCheck', (results, callback) => {
        const {blockHeight} = results.nextBlock;
        _election.getBlockElectors(
          {blockHeight, ledgerNode, recoveryMode}, callback);
      }],
      update: ['getElectors', (results, callback) => {
        console.log(`${Date.now()} UPDATESTATE`, results.getElectors.electors);
