...
module.exports = class ConsensusAgent extends ContinuityAgent {
  ...
  async _extendBlockchain() {
    ...
    while(!this.halt) {
      ...
      try {
        ...
        recoveryMode = true;
        // write recovery electors as priority peers
        await _cache.consensus.setPriorityPeers({
          priorityPeers: [...this.priorityPeers],
          ledgerNodeId
        });
        continue;
      }
  ...
  async _findConsensus(state) {
    ...
    // now that history has been retrieved, clear flag indicating any
    // priority merge events have been received; this keeps history in
    // sync with notifications indicating it should be read
