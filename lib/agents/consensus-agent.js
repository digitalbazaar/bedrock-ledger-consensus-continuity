...
module.exports = class ConsensusAgent extends ContinuityAgent {
  ...
  async _extendBlockchain() {
    ...
    while(!this.halt) {
      ...
      multi.publish(`continuity2017|needsGossip|${ledgerNodeId}`, 'consensus');
      await multi.exec();
      break;
    }
  }
  ...
  async _findConsensus(state) {
    ...
    if(!consensusResult.consensus) {
      ...
      await _cache.events.setMergePermits({ledgerNodeId, permits});
      // TODO: remove consensus behind flag and just use merge permits
      // clear consensus behind flag
      this.consensusCount = 0;
      await _cache.consensus.setConsensusBehind({ledgerNodeId, behind: false});
    ...
    logger.verbose(
      ...
      `${this.consensusCount} consecutive time(s).`);

    // notify agents that consensus is behind if consensus is reached
    // consecutively 2 times to provide backpressure on merging/gossiping
    if(this.consensusCount === 2) {
      logger.verbose('Consensus agent is not keeping up with history; ' +
        'applying backpressure.');
      // TODO: remove consensus behind flag and just use merge permits
      await _cache.consensus.setConsensusBehind({ledgerNodeId, behind: true});
    }
