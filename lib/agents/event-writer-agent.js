...
module.exports = class EventWriter extends ContinuityAgent {
  ...
  _clearQueue() {
    async.auto({
      range: callback => this.client.lrange(
        this.eventQueueKey, 0, -1, callback),
      ...
      generation: ['prepare', (results, callback) =>
        this._calculateGeneration(results.prepare, callback)],
      store: ['generation', (results, callback) => this._storeEvents({
        generation: results.generation,
        prepareData: results.prepare,
  ...
  _storeEvents({generationData, prepareData, rangeData}, callback) {
    async.auto({
      store: callback => {
        if(!prepareData) {
          return callback();
        ...
        logger.debug(
          `STORING RECORDS. EVENTSLENGTH ${prepareData.events.length}`);
        const {events} = prepareData;
      ...
      cache: ['store', (results, callback) => {
        ...
        multi.incrby(eventCountCacheKey, prepareData.events.length);
        ...
        multi.del(rangeData);
        multi.srem(this.eventQueueSetKey, ...prepareData.eventHashes);
