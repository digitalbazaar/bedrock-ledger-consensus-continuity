...
module.exports = class EventWriter extends ContinuityAgent {
  ...
  _clearQueue() {
    async.auto({
      range: callback => cache.client.lrange(
        this.cacheKey.eventQueue, 0, -1, (err, result) => {
          if(err) {
            return callback(err);
          }
  ...
  _work() {
    ...
    async.auto({
      ...
      range: callback => cache.client.lrange(
        this.cacheKey.eventQueue, 0, this.config.maxEvents, (err, result) => {
          if(err) {
            return callback(err);
          }
      ...
      more: ['store', (results, callback) => cache.client.llen(
        this.cacheKey.eventQueue, (err, result) => callback(err, !!result))]
    }, (err, results) => {
      this.working = false;
