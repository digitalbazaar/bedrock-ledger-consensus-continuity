      ...
      async.auto({
        ...
        get: ['range', (results, callback) => {
          ...
          this.client.mget(queue, (err, result) => {
            logger.debug(`lrange/mget events ${err}`, {
              events: result
            });
            if(err) {
              return callback(err);
            ...
            logger.debug(`VALID JSON events length: ${eventsJson.length}`);
