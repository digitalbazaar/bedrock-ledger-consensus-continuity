  ...
  async _storeEvents({processedData, rangeData}) {
    ...
    if(creators.length !== 0) {
      ...
      if(newHeads.length !== 0) {
        ...
        // new heads, publish to inform other agents/listeners that a merge
        // event has been written
        const message = `merge|${JSON.stringify([...newHeadCreators])}`;
        multi.publish(`continuity2017|event|${ledgerNodeId}`, message);
      }
    }
