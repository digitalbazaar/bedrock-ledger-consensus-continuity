...
module.exports = class GossipAgent extends ContinuityAgent {
  constructor({agentName, ledgerNode}) {
    ...
    this.cacheKeys = {
      gossipBehind: _cacheKey.gossipBehind(this.ledgerNodeId),
    };
  }
  ...
  _workLoop() {
    ...
    async.auto({
      ...
      loadList: ['creator', (results, callback) => _voters.getPeerIds(
        {creatorId: this.creatorId, ledgerNode, limit: 100}, (err, result) => {
          if(err) {
            return callback(err);
          }
          if(result.length === 0) {
