...
module.exports = class GossipAgent extends ContinuityAgent {
  ...
  async _gossipCycle() {
    const {ledgerNodeId, creatorId} = this;
    while(!this.halt) {
      // if consensus is behind, create a promise to wait until gossip is
      ...
      // FIXME: figure out how many merge permits to grab
      const peer = await this.peerSelector.getPeer();
      if(peer) {
        await this._gw({peer});

        // if this ledger node is a priority peer, notify merge agent that
        // more gossip is available for merging
        const peers = await _cache.consensus.getPriorityPeers(
          {ledgerNodeId});
        if(peers.includes(creatorId)) {
