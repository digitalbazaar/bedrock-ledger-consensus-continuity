...
module.exports = class GossipAgent extends ContinuityAgent {
  ...
  _getAvailablePeer(callback) {
    const peerIndex = this.peerList.findIndex(p =>
      !this.contactMap.has(p) || this.contactMap.get(p) < Date.now());
    if(peerIndex === -1) {
      if(this.halt) {
        return callback(new BedrockError('Nothing to do.', 'AbortError'));
      }
      return setTimeout(() => this._getAvailablePeer(callback), 250);
    }
    callback(null, this.peerList[peerIndex]);
  }

  _gw({peerId}, callback) {
    const {coolDownPeriod} = config['ledger-consensus-continuity'].gossip;
    this.contactMap.set(peerId, Date.now() + coolDownPeriod);
    async.doWhilst(callback => _gossip.gossipWith(
      ...
      }), result => !(result.done || this.halt), callback);
  }
