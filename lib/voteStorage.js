  ...
  createIndexes: ['openCollections', (results, callback) =>
    database.createIndexes([{
      ...
      options: {
        unique: true,
        background: false,
        name: 'continuity2017_vote.aggregation'
      }
    }, {
      collection: 'continuity2017_vote',
      fields: {
      ...
      options: {
        unique: true,
        background: false,
        name: 'continuity2017_vote.by_voter'
      }
    }, {
      collection: 'continuity2017_vote',
      fields: {
      ...
      options: {
        unique: true,
        background: false,
        name: 'continuity2017_vote.by_voteHash'
      }
    }], callback)
  ]
}, err => callback(err)));
...
/**
 * Identify votes that are not in storage.
 *
 * @param voteHash the hash or array of hashes to check.
 ...
 * @param electionTopic the topic of the election ('Events' or 'RollCall').
...
api.get = (ledgerNodeId, blockHeight, electionTopic, callback) => {
  ...
  const collection = database.collections.continuity2017_vote;
  collection.find(query, {vote: 1, meta: 1})
    .sort({'meta.voteHash': 1})
    .toArray((err, records) => {
      if(err) {
        return callback(err);
      }
...
/**
 * Get a vote by its hash.
 *
 * @param ledgerNodeId the ID of the node that is tracking the votes.
 * @param voteHash the hash of the vote to retrieve.
 * @param callback(err, result) called onced the operation completes.
 */
