    ...
    validate('continuity-server.gossip'), brRest.linkedDataHandler({
      ...
      get: callbackify(async (req, res) => {
        const localPeerId = config.server.baseUri +
          '/consensus/continuity2017/voters/' + req.params.voterId;
        // all announced information is what is *known* to the remote peer
        const {body: remoteInfo} = req;
        // `basisBlockHeight` is the last block that has reached consensus
        //   on the client
        // `blockHeight` is either the last full block that the client knows
        //   or, if an `blockEventCount` is given, it is the next partial block
        //   the client knows -- note that `blockEventCount=0` indicates
        //   the client knows the block exists but has no events for it yet
        // `blockEventCount` is optional, only present if the client doesn't
        //   know the full block, it can announce how many events into the
        //   block it does know; this is different from omitting
        //   `blockEventCount` which instead indicates the client knows the
        //   full block
        ...
        // FIXME: we do not presently authenticate the remote peer -- is it
        // worth doing so?

        // ensure that the remote peer did not send heads for itself, this is
        // a protocol violation; the local peer will use whatever heads it has
        // for the remote peer
        const {peerId: remotePeerId, peerHeads} = remoteInfo;
        for(const peerHead of peerHeads) {
          if(peerHead.creator === remotePeerId) {
            throw new BedrockError(
              'Remote peer must not send the head(s) it created itself.',
              'DataError', {
                httpStatusCode: 400,
                public: true,
                remotePeerId
