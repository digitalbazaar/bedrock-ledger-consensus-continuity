...
let _currentOperationValidationJobs = 0;
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  const MAX_EVENT_VALIDATION_JOBS = eventValidation.concurrency;
  const MAX_OPERATION_VALIDATION_JOBS = operationValidation.concurrency;
  ...
  // validate event
  app.post(
    routes.eventValidation, brRest.when.prefers.ld,
    asyncHandler(async (req, res) => {
      ...
      try {
        const {event, ledgerNodeId, url, sessionId} = req.body;
  ...
  // validate operation
  app.post(
    routes.operationValidation, brRest.when.prefers.ld,
    asyncHandler(async (req, res) => {
      if(_currentOperationValidationJobs >= MAX_OPERATION_VALIDATION_JOBS) {
        res.status(503).end();
        return;
      }

      _currentOperationValidationJobs++;

      try {
        const {operation, ledgerNodeId, basisBlockHeight, sessionId} = req.body;

        await _peerEvents.validateOperation({
          operation,
          ledgerNodeId,
          basisBlockHeight,
          sessionId
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      // FIXME: we need to authenticate `peerId`; http-signatures should be
      // added here
