...
bedrock.events.on(
  'bedrock-express.configure.bodyParser', (server, callback) => {
    server.use(bodyParser.json({limit: '1mb', type: ['json', '+json']}));
    callback(null, true);
  });
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  // Get events
  // TODO: use a more conservative limit default and make configurable
  app.post(routes.eventsQuery, brRest.when.prefers.ld, (req, res, next) => {
    const voterId = config.server.baseUri +
      '/consensus/continuity2017/voters/' + req.params.voterId;
    ...
    async.auto({
      ...
      // false indicates not to wrap records in an array
        ...
        .on('data', data => res.write(JSON.stringify(data)))
        .on('error', next)
        .once('end', () => res.end());
    });
  });
    ...
    get: (req, res, callback) => {
      ...
      async.auto({
        ...
        history: callback => {
          ...
          _events.partitionHistory(
            {creatorHeads, creatorId, peerId: callerId}, (err, result) => {
              if(err) {
                return callback(err);
              }
  ...
  app.post(routes.notify, brRest.when.prefers.ld, (req, res, next) => {
    ...
    // only need to send in the voterId to form the cache key
    const notificationKey = _cacheKey.gossipNotification(req.params.voterId);
