...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    (req, res, next) => {
      ...
      const voterId = config.server.baseUri +
        '/consensus/continuity2017/voters/' + req.params.voterId;
      if(!(req.headers['continuity2017-peer'] &&
        req.headers['continuity2017-merge-hash'])) {
        return next(new BedrockError(
          '`continuity2017-peer` and `continuity2017-merge-hash` headers are' +
          ' required.', 'DataError',
          {httpStatusCode: 400, public: true}));
      }
      const mergeHash = req.headers['continuity2017-merge-hash'];
      async.auto({
        cache: callback => {
          if(!cacheConfig.enabled) {
            return callback();
          }
          // header is the hash of the public key (43 chars)
          const cacheKey =
            `r-${req.params.voterId}-${req.headers['continuity2017-peer']}`;
        ...
        addEvent: ['exists', (results, callback) => {
          ...
          cache.client.incr(cacheKey, err => {
            ...
            cache.client.expire(cacheKey, 6000);
          });
