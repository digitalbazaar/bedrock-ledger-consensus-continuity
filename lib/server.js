...
bedrock.events.on('bedrock-express.configure.routes', app => {
  const routes = config['ledger-continuity'].routes;
  ...
  app.get(routes.root, brRest.when.prefers.ld, brRest.linkedDataHandler({
    get: function(req, res, callback) {
      ...
      storage.voters.get({voterId}, (err, voter) => {
        ...
        callback(null, {
          // TODO: use config.constants for this and everywhere else in
  ...
  // get block status
  // FIXME: use linkDataHandler
  app.get(routes.blockStatus, brRest.when.prefers.ld, (req, res, next) => {
    let blockHeight;
    try {
