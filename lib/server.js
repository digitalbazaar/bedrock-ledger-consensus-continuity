...
bedrock.events.on('bedrock-express.configure.routes', app => {
  const consensusAgentRoute = config['ledger-agent'].routes.agents;
  ...
  app.get(
    root + '/blocks/:index/status', brRest.when.prefers.ld,
    (req, res, next) => {
      ...
      async.auto({
        ...
        getBlockStatus: ['getVoter', (results, callback) => {
          ...
          _createBlockStatus(blockId, ledgerNode, voter, callback);
        }]
      }, (err, results) => {
        if(err) {
          return next(err);
