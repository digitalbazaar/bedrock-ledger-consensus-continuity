...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    (req, res, next) => {
      ...
      async.auto({
        ...
        checkMethod: ['ledgerNode', (results, callback) => {
          const {consensusMethod} = results.ledgerNode.consensus;
          if(consensusMethod !== 'Continuity2017') {
            return callback(new BedrockError(
              'Ledger node consensus method is not "Continuity2017".',
              'AbortError', {httpStatusCode: 400, public: true}));
        ...
        exists: ['checkMethod', (results, callback) => {
        ...
        addEvent: ['exists', (results, callback) => {
          results.ledgerNode.consensus._events.add(event, results.ledgerNode, {
  ...
  app.post(
    routes.eventsCompressed, upload.single('events.gz'), (req, res) => {
      res.status(204).end();
      const creatorId = config.server.baseUri +
        '/consensus/continuity2017/voters/' + req.params.voterId;
      async.auto({
        unzip: callback => zlib.gunzip(req.file.buffer, (err, result) => {
          if(err) {
            return callback(err);
          }
          let events;
        ...
        voter: ['unzip', (results, callback) =>
          storage.voters.get({voterId: creatorId}, callback)],
        ledgerNode: ['voter', (results, callback) =>
          brLedgerNode.get(null, results.voter.ledgerNodeId, callback)],
        checkMethod: ['ledgerNode', (results, callback) => {
          const {consensusMethod} = results.ledgerNode.consensus;
        ...
        process: ['checkMethod', (results, callback) => {
          ...
          async.eachSeries(results.unzip, (e, callback) => async.auto({
            ...
            addEvent: ['exists', (results, callback) => {
              ...
              ledgerNode.consensus._events.add(e.event, ledgerNode, {
                continuity2017: {peer: true}
              }, callback);
            }]
          }, callback), callback);
