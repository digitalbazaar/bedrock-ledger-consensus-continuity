  ...
  app.post(
    ...
    validate('continuity-server.gossip'), brRest.linkedDataHandler({
      ...
      get: callbackify(async (req, res) => {
        ...
        // `blockHeight` is last block that the client knows (partial or full)
        // `eventIndex` is optional, only present if the client doesn't know
        //   the full block, it can announce how many events into the block it
        //   does know
