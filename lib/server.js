    ...
    validate('continuity-server.gossip'), brRest.linkedDataHandler({
      ...
      get: callbackify(async (req, res) => {
        const localPeerId = config.server.baseUri +
          '/consensus/continuity2017/voters/' + req.params.voterId;
        // FIXME: remove `headsOnly` if not used
        // the schema validator ensures that `peerHeads` and `headsOnly` are
        // mutually exclusive
        const {
          body: {
            blockHeight, headsOnly, peerId: remotePeerId, peerHeads,
        ...
        if(headsOnly) {
          // FIXME: determine if this should be a different endpoint
          // ... and we can't fetch all creator heads of all time, we need
          // limitations
          return _getHeads({creatorId: localPeerId});
        }

        // FIXME: remove old call
        // const result = await _history.partition(
        //   {creatorHeads, creatorId: localPeerId, peerId: remotePeerId});
        //console.log('result', result);

        // FIXME: just return result, rename function once working
        const result = await _history.partition2({
          ledgerNode, remoteInfo: req.body
        });
...
async function _getHeads({creatorId}) {
  const ledgerNodeId = await _peers.getLedgerNodeId(creatorId);
  const ledgerNode = await brLedgerNode.get(null, ledgerNodeId);
  const {heads} = await _history.getCreatorHeads({ledgerNode});
  return heads;
}
