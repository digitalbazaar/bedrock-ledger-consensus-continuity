...
bedrock.events.on('bedrock-express.configure.routes', app => {
  //const consensusAgentRoute = config['ledger-continuity'].routes.root;
  const root = '/consensus/continuity2017/:voterId'
  const routes = {
    root,
    status: root + '/status'
  };
  ...
  app.get(
    ...
    (req, res, next) => {
      ...
      let voterId;
      try {
        voterId = multihash.decode(
          multihash.fromB58String(req.params.voterId)).digest.toString();
      } catch(err) {
        return next(new BedrockError(
          'Voter ID could not be decoded.',
      ...
      async.auto({
        ...
        console.log('1111111111', err, results.getBlockStatus);
        if(err) {
          return next(err);
        }
...
function _createBlockStatus(blockHeight, ledgerNode, voter, callback) {
  ...
  async.auto({
    // TODO: get latest config block, get config event, get ledger ID
    // and concatenate `/blocks/blockHeight` to get block ID... or support
    // querying via `blockHeight` in ledger storage
    blockId: callback => callback(null, 'urn:uuid:1234/blocks/' + blockHeight),
