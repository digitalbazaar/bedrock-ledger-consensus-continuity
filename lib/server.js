...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(routes.gossip, brRest.when.prefers.ld, brRest.linkedDataHandler({
    get: (req, res, callback) => {
      const voterId = config.server.baseUri +
        '/consensus/continuity2017/voters/' + req.params.voterId;
      const treeHash = req.body.treeHash;
      async.auto({
        ...
        firstDescendant: ['ledgerNode', (results, callback) => {
          ...
          const query = {
            ...
            'meta.continuity2017.creator': voterId
          };
        ...
        aggregate: ['firstDescendant', (results, callback) => {
          ...
          collection.aggregate([{
            // FIXME: is there a way to do this without $match stage?
          ...
          {
            $project: {
              mergeEventHashes: '$_parents.eventHash'
            }
          }
          ]).toArray(callback);
        }]
        ...
        // console.log('FIRSTDESCENDANT', JSON.stringify(results.firstDescendant, null, 2));
        if(!results.firstDescendant) {
        ...
        // FIXME: don't have to add this depending on aggrgate query
...
function _createBlockStatus(blockHeight, ledgerNode, voter, callback) {
  ...
  async.auto({
    ...
    block: ['ledgerId', (results, callback) => {
      ...
      ledgerNode.storage.blocks.getSummary(blockId, (err, record) => {
