...
bedrock.events.on('bedrock.exit', async () => {
  // flush all operation writers before exiting
  const promises = [];
  OPERATION_WRITER_MAP.forEach(operationWriter => {
    promises.push(operationWriter.flush().catch(e => {
      logger.error(`Error flushing operation writer.`, {
        error: e,
        ledgerNodeId: operationWriter.ledgerNodeId
      });
    }));
  });
