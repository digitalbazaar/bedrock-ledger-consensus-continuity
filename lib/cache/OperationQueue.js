...
module.exports = class OperationQueue {
  ...
  async hasNextChunk() {
    ...
    (this.window.tail + 1) < this.queue.length)) {
      this.window.head = 0;
      this.window.tail = 0;
      this.queue = null;
    ...
    if(head === null) {
      logger.verbose('Unable to find valid head of operation queue');
      return false;
    }
  ...
  async _getChunkOperations() {
    ...
    const end = Math.min((this.window.head + this.opCount), this.queue.length);
  ...
  async _isDatabaseEmpty() {
    ...
    try {
      ...
      logger.error('Error checking if operation queue is empty', {
        error: e
      });
      return true;
    }
