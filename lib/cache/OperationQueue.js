...
module.exports = class OperationQueue {
  ...
  async _getChunkOperations() {
    ...
    for(let i = this.window.head; i < end; i++) {
      ...
      promises.push(limit(async () => {
        try {
          ...
          logger.debug('Non-critical operation validation error', {
            error: e,
            operation
          });
          this.removedOperations.push(operation);
    ...
    if(this.removedOperations.length > 0) {
      logger.debug('Removed operations from chunk', {
        n: this.removedOperations.length
      });
    }
  ...
  async _removeOperationsFromQueue() {
    if(this.removedOperations.length === 0) {
      return;
    }
    ...
    try {
      await this.collection.updateOne({
        ...
        $pullAll: {
          'queue.records': this.removedOperations
        }
      });
    } catch(e) {
