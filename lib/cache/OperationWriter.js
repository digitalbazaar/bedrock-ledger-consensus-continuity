...
class OperationWriter {
  ...
  async init() {
    ...
    try {
      ...
      // ignore duplicate error, op queue exists for the given ledger node
    }
  }
  ...
  async flush() {
    ...
    const records = queue.map(({meta, operation}) => ({
      meta: {...meta, created: now, updated: now}, operation
    }));
    // retry indefinitely on duplicate errors (duplicates will be removed
    // from the `queue` array so the rest of the operations can be inserted)
    const collection = database.collections['bedrock-ledger-operation-queue'];
    try {
      await collection.updateOne({'queue.ledgerNodeId': this.ledgerNodeId}, {
        $push: {
          'queue.records': {$each: records},
        },
        $set: {
