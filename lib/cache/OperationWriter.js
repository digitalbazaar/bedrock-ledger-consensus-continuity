...
class OperationWriter {
  ...
  async add({operation, meta, forceFlush = false} = {}) {
    ...
    if(this.databaseOperationQueueLength >= MAX_OPERATION_QUEUE_LENGTH) {
      this._updateCachedDatabaseQueueLength().catch(console.error);
  ...
  async flush() {
    ...
    try {
      ...
      await this._updateCachedDatabaseQueueLength();
    } catch(e) {
  ...
  async _updateCachedDatabaseQueueLength() {
    const fn = async () => {
      try {
        ...
        logger.error('Failed to update length of operation queue on disk', {
          error: e
        });
      }
    };

    return PROMISE_QUEUE.memoize({
      key: 'updateCount',
      fn
    });
  }
