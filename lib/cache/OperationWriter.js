...
const PROMISE_QUEUE = new LruCache({max: 10, disposeOnGet: true});

// Error Messages
const DATABASE_QUEUE_LENGTH_ERROR = 'Non-critical error: failed to cache ' +
  'length of operation queue on disk';
...
class OperationWriter {
  constructor({ledgerNodeId, highWaterMark = 100, timeout = 1000} = {}) {
    ...
    this.recentOperationHashes = new LRU({max: 1000, maxAge: 5 * 60 * 1000});
  ...
  async add({operation, meta, forceFlush = false} = {}) {
    ...
    if(this.databaseOperationQueueLength >= MAX_OPERATION_QUEUE_LENGTH) {
      ...
      this._cacheDatabaseQueueLength().catch(e => {
  ...
  async flush() {
    ...
    try {
      ...
      await this._cacheDatabaseQueueLength().catch(e => {
        logger.debug(DATABASE_QUEUE_LENGTH_ERROR, {error: e});
      });
    } catch(e) {
  ...
  /**
   * Caches the length of the database in queue in memory.
   *
   */
  async _cacheDatabaseQueueLength() {
    const fn = async () => {
      const {ledgerNodeId} = this;
      const projection = {_id: 1};
      const count = await this.collection.find({
        ledgerNodeId
      }, {projection}).count();
    ...
    try {
      await PROMISE_QUEUE.memoize({key: 'updateCount', fn});
    } catch(e) {
      logger.debug(DATABASE_QUEUE_LENGTH_ERROR, {error: e});
    }
  }
   ...
   *
   * @param {object} operation - A webledger operation record.
   * @param {object} operation.meta - The metadata of an operation.
   */
