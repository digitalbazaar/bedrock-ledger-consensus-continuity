...
/* Note: Max size is 25. This promise queue is only used for the update
count function. We expect, in the worst case, no more than 25 ledger nodes
serviced by a single process updating their queue count. */
const lruCacheOptions = {max: 25, disposeOnSettle: true};
...
class OperationWriter {
   ...
   * writer will flush 100 operations/second to the persisted queue in the
   * database. In the extraordinary case, where the process dies between
   * flushes, a maximum of 100 lost operations will occur. A timeout of
   ...
   *   records should be retained in memory before forcing a flush to the
   *   database.
   */
  constructor({ledgerNodeId, highWaterMark = 100, timeout = 1000} = {}) {
    ...
    /* Tracks operation hashes for soft duplicate prevention within the same
  ...
  async add({operation, meta, forceFlush = false} = {}) {
    ...
    check fails, there exist slower and robust checks when the operation
