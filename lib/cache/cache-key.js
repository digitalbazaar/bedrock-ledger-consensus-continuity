...
api.consensusBehind = ledgerNodeId => `cb|${_lni(ledgerNodeId)}`;
...
// priority peers for progressing consensus
api.priorityPeers = ledgerNodeId => `pp|${_lni(ledgerNodeId)}`;
...
// key for tracking how many merges are permitted
api.mergePermits = ledgerNodeId => `mp|${_lni(ledgerNodeId)}`;
