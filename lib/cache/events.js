...
exports.addPeerEvent = async ({event, meta, ledgerNodeId}) => {
  ...
  // FIXME: `eventQueueSetKey` was previously used to do diffs to determine
  // whether an event was in the redis cache or not... whether it can be
  // safely removes needs to be investigated; see gossip.js and difference()
...
exports.addLocalMergeEvent = async ({event, meta, ledgerNodeId}) => {
  ...
  // FIXME: this is no longer needed; recent history that is used by
  // the continuity algorithm is stored in memory
 ...
 * @returns {Promise} resolves once the operation completes.
 */
exports.addLocalRegularEvent = async ({ledgerNodeId, isConfig = false}) => {
