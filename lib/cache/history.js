...
exports.setHead = async (
  {creatorId, ledgerNodeId, eventHash, generation, basisBlockHeight}) => {
  const key = _cacheKey.head({creatorId, ledgerNodeId});
  await _setHead({key, eventHash, generation, basisBlockHeight});
};
...
exports.getGenesisHead = async ({ledgerNodeId}) => {
  ...
  if(eventHash) {
    return {eventHash, generation: 0, basisBlockHeight: 0};
  }
  return null;
};
...
async function _setHead({key, eventHash, generation, basisBlockHeight}) {
...
async function _getHead({key}) {
  const [eventHash, generation, basisBlockHeight] =
    await cache.client.hmget(key, 'h', 'g', 'b');
