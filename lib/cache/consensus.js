...
/**
 * Set or clear the consensus behind flag.
 *
 * @param ledgerNodeId {string} - The ID of the ledger node.
 * @param [behind] {Boolean} - `true` to indicate consensus is behind, `false`
 *        to clear the flag.
 *
 * @returns {Promise} resolves once the operation completes.
 */
exports.setConsensusBehind = async ({ledgerNodeId, behind}) => {
  const key = _cacheKey.consensusBehind(ledgerNodeId);
  if(behind) {
...
/**
 * Returns whether the consensus behind flag is set.
 *
 * @param ledgerNodeId {string} - The ID of the ledger node.
 *
 * @returns {Promise} resolves once the operation completes.
 */
exports.isConsensusBehind = async ({ledgerNodeId}) => {
  const key = _cacheKey.consensusBehind(ledgerNodeId);
  const behind = (await cache.client.get(key) !== null);
  return behind;
};
