 ...
 */
exports.extend = async ({worker, savedState}) => {
  ...
  while(!worker.halt()) {
    ...
    try {
      ...
      result = await _findConsensus({worker, state});
    } catch(error) {
 ...
 * @returns {Promise}
 */
async function _findConsensus({worker, state}) {
  const {ledgerNode} = worker;
  ...
  worker._removeConsensusEvents({eventHashes: consensusResult.mergeEventHash});
