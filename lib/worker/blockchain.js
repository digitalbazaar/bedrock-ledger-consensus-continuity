 ...
 */
exports.extend = async ({worker, ledgerNode, savedState, halt}) => {
  // FIXME: move savedState into `worker`
  const state = {};
  let blocks = 0;
  let result = {
    consensus: false,
  ...
  while(!halt()) {
    ...
    try {
      ...
      result = await _findConsensus({worker, ledgerNode, state});
    } catch(error) {
...
async function _findConsensus({worker, ledgerNode, state}) {
  ...
  worker.removeConsensusEvents({eventHashes: consensusResult.mergeEventHash});
