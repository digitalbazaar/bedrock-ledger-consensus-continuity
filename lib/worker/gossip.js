...
exports.runGossipCycle = async ({
  worker, ledgerNode, priorityPeers, creatorId, peerSelector, needsGossip, halt
}) => {
  ...
  do {
    ...
    for(const peer of peers) {
      ...
      const {mergePermitsReceived: received} = await _gw(
        {worker, ledgerNode, creatorId, peer});
...
async function _gw({worker, ledgerNode, creatorId, peer}) {
  let result;
  let err;
