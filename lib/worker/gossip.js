...
async function _gw({worker, peer}) {
  ...
  if(events.length > 0) {
    ...
    // FIXME: if the `batchResult` includes a "withheld"/"deferred" event, then
    // we to store it in worker memory as a potential commitment target, so call
    // `worker._addDeferredEvent()`; an event will only be withheld if it was
    // created by the remote peer and there is no event that has committed to
    // it that has reached consensus; the "withheld" event will include a merge
    // event and any regular events it merges; there can be at most one of
    // "withheld" events these per peer at a time and to allow for us losing
    // track of it since it's only held in memory, we do not update the cursor
    // for the peer -- and we do not gossip with the peer again until the
    // event that committed to the withheld event has reached consensus
  }
