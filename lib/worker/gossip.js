...
exports.runGossipCycle = async ({worker, needsGossip}) => {
  ...
  const {priorityPeers, witnesses} = worker.consensusState;
...
async function _gw({worker, peer}) {
  ...
  try {
    ...
    result = {events: [], neededHashes: [], truncated: false, next: null};
  }
...
async function _getNeeded({worker, peer}) {
  ...
  try {
    ...
    if(result.totalEvents === 0) {
      ...
      return {events: [], neededHashes: [], truncated: false, next: null};
    }
    ...
    if(neededHashes.length === 0) {
      ...
      return {events: [], neededHashes, truncated, next};
    }
