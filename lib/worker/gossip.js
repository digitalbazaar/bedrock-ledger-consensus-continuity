...
async function _getNeeded({worker, remotePeerId}) {
  ...
  try {
    ...
    // FIXME: note that if `result.consensusEvents.truncated` is set, then
    // there is more to download from the peer and we MUST keep track of this
    // so that subsequent calls use it; if the client doesn't use this
    // information, it could get stuck receiving the same result every time
    // if it takes more than the number of events returned to make the next
    // block
    const truncated =
      result.consensusEvents.truncated || result.nonConsensusEvents.truncated;
