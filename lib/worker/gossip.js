...
exports.runGossipCycle = async ({worker, needsGossip}) => {
  ...
  do {
    ...
    for(const peer of peers) {
      // FIXME: if we know that the `next` event to be received from the
      // peer requires a higher `basisBlockHeight` than we have, then it
      // is pointless to contact the peer before trying to compute consensus
      // even if `needsGossip` is true, we need to account for that below
...
exports.sendNotification = async ({creatorId, priorityPeers, peerSelector}) => {
  ...
  while(peers.length > 0 && sent < 2 && attempts < maxRetries) {
    ...
    try {
      await _client.notifyPeer({localPeerId: creatorId, remotePeerId});
...
async function _gw({worker, peer}) {
  ...
  // store `next` along with status for what to request in subsequent call
  const {next} = result;
  await peer.success({backoff, next});
  return {mergeEventsReceived, next};
}
...
async function _getNeeded({worker, peer}) {
  ...
  try {
    ...
    const [peerHeadsMap, peerStatus] = await Promise.all([
      // FIXME: need to be sure to only send a limited number of heads (1000?)
      // ... what happens when we can't send enough heads, are we guaranteed
      // to make progress?
      // FIXME: make `countPerPeer` configurable
      worker._getNonConsensusPeerHeads({countPerPeer: 2, peerLimit: 1000}),
      peer.getStatus()
    ]);
    ...
    // if peer status indicates that the next request should be for a
    // `blockHeight` greater than `blockHeight`, use it and the
    // `blockEventCount` instead to continue receiving a partial block
    // Note: `blockHeight` could be `undefined` if the `next` request is
    // for non-consensus data (then next only indicates the required
    // `basisBlockHeight`), but when it is `undefined` this conditional
    // is always false, which is what is desired.
    if(peerStatus.next && peerStatus.next.blockHeight > blockHeight) {
      ({blockHeight, blockEventCount} = peerStatus.next);
    }

    const result = await _client.getHistory({
      basisBlockHeight, blockHeight, localPeerId,
      peerHeadsMap, remotePeerId, blockEventCount
    });

    if(result.totalEvents === 0) {
      // peer has nothing to share
      return {events: [], neededHashes: [], next: null};
    }
    ...
    if(neededHashes.length === 0) {
      ...
      // was truncated; we must also ensure we return `next` which indicates
      // that the server has more events to send... we MUST keep track of this
      // so subsequent calls can use it to prevent the client from getting
      // stuck if more events are required from the server to make the next
      // block
      return {events: [], neededHashes, next};
    }
