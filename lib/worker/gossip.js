...
async function _gw({worker, peer}) {
  ...
  if(events.length > 0) {
    ...
    // FIXME: if the `batchResult` indicates that events were "withheld",
    // then do not store the cursor, keep the current one; then, in gossip
    // peer selection, ensure that the worker is consulted to determine if
    // it is holding any withheld events for a particular peer -- and if so,
    // do not gossip with that peer
  }
