...
exports.runGossipCycle = async ({
  ...
  while(peers.length > 0 && remainingPermits > 0 && !halt()) {
    ...
    // gossip with `peer`
    ({mergePermitsConsumed, behind} = await _gw(
      {ledgerNode, mergePermits, creatorId, peer}));
    remainingPermits -= mergePermitsConsumed;
  }

  // track whether gossip is behind or not
  const ledgerNodeId = ledgerNode.id;
  if(behind) {
    await _cache.gossip.gossipBehind({ledgerNodeId});
  } else {
    await _cache.gossip.gossipBehind({ledgerNodeId, remove: true});
  }
...
async function _gw({ledgerNode, mergePermits, creatorId, peer}) {
  ...
  return {mergePermitsConsumed, behind: deferredEvents.length > 0};
}
