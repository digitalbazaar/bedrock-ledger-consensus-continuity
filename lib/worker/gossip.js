...
exports.runGossipCycle = async ({worker, needsGossip}) => {
  ...
  do {
    ...
    for(const peer of peers) {
      ...
      // priority peer
      const mustContact = needsGossip && priorityPeerSet.has(peer.id);
...
async function _gw({worker, peer}) {
  ...
  try {
    ...
    result = {events: [], needed: [], truncated: false, next: null};
  }
...
async function _getNeeded({worker, peer}) {
  ...
  try {
    ...
    const result = await _client.getHistory({
      blockHeight, localPeerId, peerHeads, remotePeerId, blockEventCount
    });

    if(result.totalEvents === 0) {
      // peer has nothing to share
      return {events: [], needed: [], truncated: false, next: null};
    }
    ...
    const needed = await _diff({worker, eventHashes});
    const {next} = result.consensusEvents;
    const truncated = !!next || result.nonConsensusEvents.truncated;
    if(needed.length === 0) {
      ...
      // that a partial block was sent by the server... we MUST keep track of
      // this so subsequent calls can use it to prevent the client from getting
      // stuck if the number of events required to make the next block is less
      // than however many events could be received from the server
      return {events: [], needed, truncated, next};
    }
