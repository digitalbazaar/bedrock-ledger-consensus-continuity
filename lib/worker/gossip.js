...
exports.runGossipCycle = async ({worker, needsGossip}) => {
  /* How the gossip cycle is intended to work:

  1. We open a loop to try and pull events from other peers.
  2. We select some peers to pull from.
  3. If there are no peers left to pull from, e.g., they are all backed off,
    then we break out of the loop.
  4. We try to pull from each peer in the selection in serial. If the pull
    is successful, we flip a flag indicating that we contacted at least one
    peer, regardless of whether we got any events. We also track how many
    events, in total, have been received from peers.
  5. Once we've tried pulling from the selection of peers, we want to break
    out of the loop immediately if we're a priority peer (others are depending
    on us to merge) and we've contacted any of them.
  6. If we haven't successfully contacted any peers yet, we will loop and
    try again. If we have contacted at least one peer, but we didn't receive
    any events and we need events to get to consensus, we will loop and try
    talking with other peers. We will loop up to `retries` times.
  7. After we exit the loop, if we didn't contact any peers and we're not
    a priority peer ourselves, then we consider the network `busy`. This will
    cause us make our pipeline enter into cooldown for a short period of time
    while we wait for other peers to hopefully become less busy. */
...
exports.sendNotification = async ({ledgerNodeId, peerSelector} = {}) => {
  // notify all selected peers in parallel
  const peers = await peerSelector.selectNotifyPeers();
  const promises = [];
  for(const peer of peers) {
    promises.push(_notifyPeer({ledgerNodeId, peer}));
  }
  try {
...
async function _gw({worker, peer}) {
  ...
  if(events.length > 0) {
    ...
    if(!valid) {
      ...
      // non-critical to our work session, but could be a fatal error for the
      // remote peer
      logger.debug(
        'A non-critical error occurred during gossip batch processing.',
        {peer: peer.id, error});

      // if the error was not fatal and was a local event validation service
      // timeout error, return early without writing success or failure to
      // the gossip peer
      if(!fatal && error.name === 'TimeoutError') {
        const {cursor} = peer.getStatus();
        return {mergeEventsReceived: 0, cursor, success: false};
      }
  ...
  let {samplePeers} = result;
  // do not add self, if present in sample peers
  const {localPeerId} = worker;
  samplePeers = samplePeers.filter(p => p.id !== localPeerId);
  await Promise.all(samplePeers.map(remotePeer =>
    _peers.optionallyAdd({ledgerNode, remotePeer, localPeerId})));
...
async function _notifyPeer({ledgerNodeId, peer}) {
  // FIXME: fix access of private `_peer`
  const remotePeer = {id: peer._peer.id, url: peer._peer.url};
  try {
