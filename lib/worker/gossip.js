...
async function _gw({worker, peer}) {
  let mergeEventsReceived = 0;
  // FIXME: remove me
  /*console.log('*********GW: ', worker.localPeerId.substr(-4), '=>',
    peer.id.substr(-4));*/
  ...
  if(events.length > 0) {
    ...
    if(withheld) {
      ...
      const {cursor} = await peer.getStatus();
      // FIXME: remove me
      /*console.log('keep old cursor',
        cursor ? cursor.localEventNumber : null,
        result.cursor.localEventNumber);*/
...
async function _getNeeded({worker, peer}) {
  ...
  try {
    ...
    // FIXME: remove me
    //console.log('eventHashes', eventHashes);
    //console.log('needed', neededHashes);
