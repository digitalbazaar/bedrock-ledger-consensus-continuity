...
api.merge = async ({
  ...
  events may be pending and have event numbers assigned to them that MUST be
  followed by the merge event that merges them, otherwise the ordering used for
  gossiping non-consensus events can become corrupt. */
  ...
  const meta = {
    ...
    continuity2017: {
      ...
      type: 'm',
      // FIXME: add `meta.continuity2017.eventNumber'
      // eventNumber: worker.nextEventNumber++
    },
    eventHash
  };
...
async function _prepareNextMerge({
  ...
  // use at least `threshold` parent hashes and at least 1, if available
