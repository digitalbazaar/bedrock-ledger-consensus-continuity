...
async function _prepareNextMerge({
  ...
  /* Note: We could find a peer head with a `mergeHeight` that is greater than
  our head's `mergeHeight` yet it does not descend from our head. This would be
  possible if enough merges happened in another part of the DAG that we have
  yet to merge. We need to be able to merge that peer head or consensus could
  fail. But this means we have to check its ancestry to ensure it doesn't
  descend from our head. We could walk back from each of the peer head's
  parents until we find our head or until hit our `mergeHeight` (at which
  point we know the peer head does not descend from our head), but this would
  be very slow if the peer had a much higher `mergeHeight` than our head.
