...
async function _prepareNextMerge({
  ...
  // get `peerHeadCommitment` to ensure previously committed event can still
  // be legally merged
  const {head, peerHeadCommitment} = worker;
  ...
  const threshold = head.generation === 0 ?
    1 : (localOutstanding ? nonEmptyThreshold : emptyThreshold);
  ...
  // if `peerHeadCommitment` is set, it refers to the non-witness peer head
  // committed to in `head`, so use its hash value for the non-witness merge
  // event parent; otherwise do not set a non-witness merge parent
  if(peerHeadCommitment && !peerCreatorSet.has(peerHeadCommitment.creator)) {
    // it is important to add the non-witness event as a parent first, even
    // for an "empty merge event", to ensure that operations that are only
    // flowing through non-witnesses get merged
    parentHashes.add(peerHeadCommitment.eventHash);
    peerCreatorSet.add(peerHeadCommitment.creator);
    mergeHeight = Math.max(mergeHeight, peerHeadCommitment.mergeHeight);
    remainingSlots--;
  }

  /* Find a non-witness merge event to commit to. Look first in the deferred
  events from the worker, and second using the remaining childless peer
  heads. */

  // FIXME: check `worker.deferredEventMap` first for a non-witness event
  // to commit to
  ...
  /* If no peer head to commit to could be found in deferred events, then
  ...
  of a parent we've already added will not have a peer head in the list. It is
  conceivable that the same creator we committed to last time and that we
  already have a parent for could technically have a childless peer that we
  filtered out that we could have committed to here without violating protocol.
  This could occur if another witness had already committed to the same parent
  event we just committed to *and* it had reached consensus and its creator
  has already gossiped the next event).

  This seems unlikely and we would rather not have to change the code to allow
  for that corner case when there are no other non-witnesses to choose from.
  Instead we use a simpler implementation that will result in our commitment
  slot not being used for the same creator twice in a row. */
