...
async function _prepareNextMerge({
  ...
  // FIXME: if `head.parentHashCommitment` is set and `mergeCommitment` is
  // `true`, use its value is for the non-witness merge event parent; otherwise
  // do not set a non-witness merge parent
  // FIXME: pick a non-witness merge event first from the withheld events
  // in the worker state, and then if there aren't any, from the remaining
  // childless peer heads
