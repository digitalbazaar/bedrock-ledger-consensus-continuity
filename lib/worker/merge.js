...
api.merge = async ({
  ...
  witnessPeerThreshold = 1,
  witnessEmptyEventPeerThreshold
}) => {
  if(witnessEmptyEventPeerThreshold === undefined) {
    if(witnesses % 3 === 1) {
      // `2f` is the default witness empty event peer threshold
      const f = (witnesses - 1) / 3;
      witnessEmptyEventPeerThreshold = 2 * f;
    } else {
      // cannot create an empty merge event with a single witness
      witnessEmptyEventPeerThreshold = 1;
    }
  }

  const status = await _prepareNextMerge({
    ledgerNode, creatorId, priorityPeers, witnesses, basisBlockHeight, halt,
    witnessPeerThreshold, witnessEmptyEventPeerThreshold
  });
  if(!status.mergeable) {
    // nothing to merge
...
async function _prepareNextMerge({
  ...
  halt, witnessPeerThreshold, witnessEmptyEventPeerThreshold
}) {
  const ledgerNodeId = ledgerNode.id;
  ...
  // FIXME: improve `needsGossip` reporting based on other conditions
  // if there are no outstanding operations, then report that gossip is needed
  status.needsGossip = !status.hasOutstandingOperations;
  ...
  */
  ...
  //console.log('basisBlockHeight', basisBlockHeight);
  const witness = true;
