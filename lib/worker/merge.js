...
async function _prepareNextMerge({
  ...
  // FIXME: determine if this is the first generation event for the local peer
  // and, if so, if the local peer has never been a witness and therefore
  // requires a peer commitment before it can create its first event -- fetch
  // a peer commitment to use as a parent
