...
api.merge = async ({
  ...
  basisBlockHeight, emptyThreshold
}) => {
...
async function _prepareNextMerge({
  ...
  if(emptyThreshold >= maxParents) {
    throw new Error(
      `Empty merge event threshold (${emptyThreshold}) ` +
  ...
  if(!canLocallyContribute) {
    ...
    const threshold = head.generation === 0 ?
      ...
      Math.min(1, emptyThreshold) : emptyThreshold;
    ...
    for(const peerHead of witnessHeads.values()) {
      ...
      if(peerHead !== head && peerHead.mergeHeight >= head.mergeHeight &&
        ...
        if(thresholdWitnessHeads.size === threshold) {
          break;
        }
      }
    }
