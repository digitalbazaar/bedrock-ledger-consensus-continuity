...
api.merge = async ({
  worker, ledgerNode, creatorId, priorityPeers = [],
  ...
  const status = await _prepareNextMerge({
    worker, ledgerNode, creatorId, priorityPeers, witnesses,
    basisBlockHeight, halt, nonEmptyThreshold, emptyThreshold
  });
  if(!status.mergeable) {
    // nothing to merge
  ...
  const record = await worker.addLocalMergeEvent({event: signed, meta});
...
async function _prepareNextMerge({
  ...
  const {peerChildlessMap} = worker.peerEventWriter;
  const {pendingLocalRegularEventHashes} = worker;
  // FIXME: get map from peer event writer, not just hashes
  const peerChildlessHashes2 =
    [...worker.peerEventWriter.peerChildlessMap.keys()];
  const localChildlessHashes2 = [...worker.pendingLocalRegularEventHashes];
  if(peerChildlessHashes.length !== peerChildlessHashes2.length) {
    console.log('peerChildlessHashes', peerChildlessHashes);
    console.log('peerChildlessHashes2', peerChildlessHashes2);
    process.exit(1);
  }
  if(localChildlessHashes.length !== localChildlessHashes2.length) {
    console.log('localChildlessHashes', localChildlessHashes);
    console.log('localChildlessHashes2', localChildlessHashes2);
    process.exit(1);
  }
  ...
  // this flag is only set to true if we've determined we need to merge but we
  // need more gossip to be able to do so
