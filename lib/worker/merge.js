...
async function _prepareNextMerge({
  ...
  const isSoloWitness = witnessSet.size === 1 && isWitness;
  // FIXME: do we need an extra check here in the solo witness case to ensure
  // that there is either a non-witness or a local regular event to merge?
  ...
  // if this is the first generation event for the local peer and the local
  // peer has never been a witness, fetch a peer commitment to use as a parent
  let peerCommitmentParent;
  if(head.generation === 0 && !isWitness &&
  ...
  for(const peerHead of thresholdWitnessHeads.values()) {
    // FIXME: only add threshold witness heads for empty merge events, so this
    // check should be unnecessary; but for now, skip a matching parent with
    // a peer commitment that was already added
    if(peerCommitmentParent &&
      peerCommitmentParent.creator === peerHead.creator) {
