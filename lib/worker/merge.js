...
exports.merge = async ({ledgerNode, creatorId, priorityPeers, halt}) => {
  const cooldown = await brCooldown.isActive({
    key: 'ledger-consensus-continuity:catchup'
  });
  if(cooldown) {
    logger.verbose('Cooldown Activated: Merging skipped...');
    return {merged: false, hasOutstandingOperations: true};
  }
