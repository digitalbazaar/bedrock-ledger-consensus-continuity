...
api.merge = async ({
  ...
  // FIXME: status needs to say whether or not operations were included
  // in the merge event
...
async function _prepareNextMerge({
  ...
  const {localPeerId, ledgerNode, lastLocalOpsConsensus} = worker;
  const ledgerNodeId = ledgerNode.id;
  ...
  // ops have not yet reached consensus
  if(!status.hasOutstandingOperations ||
    (!isWitness && !lastLocalOpsConsensus)) {
  ...
  if(!(localOutstanding && lastLocalOpsConsensus) &&
  ...
  // been created if `lastLocalOpsConsensus` is true
  ...
  operation queue (provided that `lastLocalOpsConsensus` is `true`). */
  ...
  // if the last local operations have achieved consensus, then we can add
  // more local operations
  if(lastLocalOpsConsensus) {
