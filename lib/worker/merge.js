...
async function _prepareNextMerge({
  ...
  // 1. There's something local to merge or the `basisBlockHeight` is
  //   not beyond our current one
  // 2. The merge event is not our own merge head
  ...
  for(const [creator, head] of witnessHeads.entries()) {
    if((localOutstanding || head.basisBlockHeight <= basisBlockHeight) &&
      creator !== creatorId) {
