...
async function _prepareNextMerge({
  ...
  2. No merge event may descent from any parent with a `basisBlockHeight` that
  is greater than its `basisBlockHeight`.

  3. If there are local operations that need to be merged (which will result
  ...
  4. If there are no local operations, then only a witness may make a merge
  ...
  less than or equal to `basisBlockHeight`. Note that the witnesses used
  when creating the next block are those for `basisBlockHeight+1` (the
  next block). So a merge event with a `basisBlockHeight` of `N` used witnesses
  for `N+1`.

  // FIXME: update rule #4 so that selected 2f parent merge events must also
  // have a `basisBlockHeight-1` or greater (so parent `basisBlockHeight` must
  // be either `basisBlockHeight-1` or `basisBlockHeight` ... nothing else is
  // permitted). This should be ok because in order to make the previous
  // block, at least `2f` merge events had to be created that had
  // `basisBlockHeight-1` (prove this true) -- we won't get stuck
  // FIXME: additionally, require the `mergeHeight` of those parents be
  // equal to or greater than the `mergeHeight` of the head (tree parent)
  // FIXME: prove that we still won't get stuck with this `mergeHeight` rule...
  // we need the following:
  // `2f+1` peers must be able to make a empty events, which means, for every
  // one of these peers, there must be `2f` merge events with a
  // `basisBlockHeight` >= `basisBlockHeight-1` and a `mergeHeight` >= the
  // head's `mergeHeight`. Prove this is ok and implement. If it is ok,
  // remove parent reuse rule, no longer needed.

  5. Finally, every merge event must have a `mergeHeight` that is 1 greater
  ...
  1. The peer event's `basisBlockHeight` <= our `basisBlockHeight` (to be
    attached to the merge event we create).
  ...
  1. `peerHead.basisBlockHeight <= basisBlockHeight`
  AND
  ...
  for(const peerHead of witnessHeads.values()) {
    // our own head does not count toward the threshold and we cannot use
    // a peer event with a `basisBlockHeight` beyond our own
    if(peerHead === head || peerHead.basisBlockHeight > basisBlockHeight) {
  ...
  if(mergeableWitnessHeads.size < threshold &&
    ...
    const {diff} = await _diffNonParentHeads(
  ...
  // add mergeable witness heads to parent hashes and update `mergeHeight`;
  // use at least `threshold` parent hashes from mergeable witness heads and
  // at least 1, if available
...
async function _diffNonParentHeads({ledgerNode, creatorId, eventHashes}) {
