...
async function _prepareNextMerge({
  ...
  const {hasBeenWitness} = ledgerNode.storage.blocks
  ...
  if(!canLocallyContribute) {
    ...
    !await hasBeenWitness({peerId: localPeerId})) {
    ...
    const {getCommitmentToPeer} = ledgerNode.storage.events
      .plugins['continuity-storage'];
    const [commitment] = await getCommitmentToPeer({peerId: localPeerId});
    if(!commitment) {
