...
api.run = async ({worker, targetCycles = -1} = {}) => {
  ...
  let halt = worker.halt.bind(worker);
  ...
  try {
    ...
    // First, always try to extend the blockchain in case we halted the
    // previous work session with events we haven't run through consensus
    // yet.
