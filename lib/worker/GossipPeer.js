...
module.exports = class GossipPeer {
  ...
  async isRecommended() {
    const {backoff, lastContactDate, next} = await this.getStatus();
    ...
    if(next && next.basisBlockHeight > blockHeight) {
      ...
      // need to reach `next.basisBlockHeight` to get any more data from it
      return false;
    }
    return true;
  }
  ...
  async success({backoff = 0, next = null} = {}) {
    const {id: peerId, ledgerNodeId} = this;
    const status = await this.getStatus();
