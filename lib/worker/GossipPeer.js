...
module.exports = class GossipPeer {
  ...
  async isRecommended() {
    ...
    if(cursor && cursor.requiredBlockHeight > blockHeight) {
      // the last time we communicated with the peer, it indicated that we need
      // to reach `cursor.requiredBlockHeight` to get any more data from it
      return false;
    }
  ...
  async fail({error, cursor, fatal = false} = {}) {
    ...
    const backoff = Math.min(
      backoffConfig.maxFailure,
      status.consecutiveFailures * backoffConfig.minFailure);
  ...
  async success({mergeEventsReceived, cursor = null} = {}) {
    ...
    if(mergeEventsReceived) {
      ...
      status.backoffUntil += Math.min(
        backoffConfig.minIdle * Math.max(1, points), backoffConfig.maxIdle);
      if(_peer.reputation < 0) {
        if(!isWitness) {
          await this.delete();
          return;
