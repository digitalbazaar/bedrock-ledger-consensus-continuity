...
module.exports = class GossipPeerSelector {
  ...
  async refreshCandidates() {
    ...
    const [recommended, pushers] = await Promise.all([
    ...
    // FIXME: create separate lists/maps for recommended vs. non-recommended
    // peers?
  }
  ...
  async selectNotifyPeers({peerIds} = {}) {
    // randomly select one non-recommended LRU peer and all recommended peers
    const recommended = [];
    const lru = [];
