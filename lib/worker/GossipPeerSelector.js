...
module.exports = class GossipPeerSelector {
  ...
  async refreshCandidates() {
    ...
    // gather peers to gossip with; can only use peer for gossip if:
    ...
    for(const record of records) {
      ...
      if(peer.status.requiredBlockHeight <= maxRequiredBlockHeight &&
        ...
        gossipCandidates.push(record);
      }
    }

    // sort acceptable records by reputation
    const byReputation = gossipCandidates.slice().sort(
      _sortByHighestReputation);
