...
api.find = async ({worker}) => {
  ...
  for(const eventHash of result.eventHashes.mergeEventHashes) {
    ...
    const {meta: {continuity2017: {
      creator, localForkNumber, forkDetectedBlockHeight
    }}} = eventSummary;
    ...
    // record fork number (even if `0`) for creator if not already recorded;
    ...
    // `localForkNumber` that is non-zero vs. one that is `0` indicates a
    // fork even though `0` indicates no fork yet
    const existing = forkNumberMap.get(creator);
    if(existing === undefined) {
      forkNumberMap.set(creator, localForkNumber);
    } else if(existing !== localForkNumber) {
