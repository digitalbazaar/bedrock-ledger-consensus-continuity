...
api.find = async ({worker}) => {
  ...
  // FIXME: need to include `meta.continuity2017.forkNumber` and
  // meta.continuity2017.forkDetectedBlockHeight` in event summaries
  ...
  for(const [creator, forkNumber] of forkNumberMap) {
    ...
    // FIXME: do a query for any creator+forkNumber combination in a consensus
    // event that does NOT match... if found, add `creator` to `forkerSet`
    $or.push({
  ...
  // FIXME: return `forkerSet` -- *all* merge events for *all* of the entries
  // in the `forkerSet` must be updated to have
  // `meta.continuity2017.forkDetectedBlockHeight` of the block being created
