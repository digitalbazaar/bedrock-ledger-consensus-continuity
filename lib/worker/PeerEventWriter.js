...
module.exports = class PeerEventWriter {
  constructor({
    worker,
    capacity = config['ledger-consensus-continuity'].writer.maxEvents
  }) {
  ...
  async add({event, meta} = {}) {
    const {capacity, operationMap, eventMap} = this;
    ...
    // auto-flush when queue fills up
    if(eventMap.size >= capacity) {
      await this.flush();
    }
  }
