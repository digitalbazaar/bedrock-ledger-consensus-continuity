...
module.exports = class PeerEventWriter {
  ...
  async flush() {
    ...
    try {
      ...
      // not async
      const processedData = this._processEvents({operations, events});
      ({creators} = await this._storeEvents({processedData, storedEvents}));
    } catch(e) {
  ...
  _processEvents({operations, events}) {
  ...
  async _storeEvents({processedData, storedEvents}) {
    const {creatorHeads, headHashes, parentHashes} = processedData;
