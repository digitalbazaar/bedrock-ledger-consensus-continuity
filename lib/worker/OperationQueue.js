...
module.exports = class OperationQueue {
  ...
  async hasNextChunk() {
    ...
    if(this.removing) {
      ...
      // reset `removing` to null after success or failure
      this.removing = null;
    }
    ...
    // are retrieved
    const end = this.queue ? this.queue.length - 1 : -1;
    if(!(this.queue && this.head < end)) {
      this.head = 0;
      this.queue = null;
      ...
      try {
        ...
        // reset `fetchRecords` to null after failure
        this.fetchRecords = null;
    ...
    // no valid operation was found to serve as the head
    if(head === null) {
      // reset `fetchRecords` to null after failing to find a valid head
      this.fetchRecords = null;
