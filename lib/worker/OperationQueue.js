...
module.exports = class OperationQueue {
  ...
  async hasNextChunk() {
    ...
    if(!(Array.isArray(this.queue) && (this.head + 1) < this.queue.length)) {
      ...
      // note: in the case where this function is called concurrently, we will
      // hit the database multiple times to fetch the latest records. We opt for
      // the last write wins
      this.queue = records;
    }

    // `OperationQueue._getHead` is not safe to concurrent requests and must be
    // wrapped with a Promise Queue. Not doing so, will result in inconsistency
    // bugs due to the window being modified concurrently and then subsequently
    // used in array traversal of the cached queue
    const {head, opCount} = await PROMISE_QUEUE.memoize({
      key: 'head',
      fn: async () => this._getHead()
    });
