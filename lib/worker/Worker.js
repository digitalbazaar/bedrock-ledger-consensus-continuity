...
module.exports = class Worker {
  ...
  async init() {
    ...
    // FIXME:
    if(!this.lastLocalContributorConsensus) {
      console.log('lastLocalContributorConsensus should be true');
      process.exit(1);
    }
  ...
  async _addLocalMergeEvent({event, meta} = {}) {
    ...
    // FIXME: get `lastLocalContributor`
  ...
  async _getHead({peerId} = {}) {
    ...
    if(records.length > 0) {
      ...
      head = {
        ...
        basisBlockHeight,
        mergeHeight,
        // FIXME: add `lastLocalContributor`
        localAncestorGeneration
      };
    ...
    for(const {event, meta} of records) {
      const {
        ...
        continuity2017: {
          ...
          // `isLocalContributor` and `lastLocalContributor`
