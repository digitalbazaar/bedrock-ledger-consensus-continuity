...
module.exports = class Worker {
  ...
  async _addLocalMergeEvent({event, meta, peerHeadCommitment = null} = {}) {
    ...
    this.head = {
      ...
      mergeHeight,
      parentHashCommitment: parentHashCommitment || null
    };
    this.peerHeadCommitment = peerHeadCommitment;
  ...
  async _initRecentHistory() {
    ...
    // any commitment was dropped when the work session cycled; only set
    // this if the event is found in non-consensus events
    this.peerHeadCommitment = null;
    let parentHashCommitment;
    if(this.head.parentHashCommitment) {
      parentHashCommitment = this.head.parentHashCommitment[0];
    }
    ...
    for(const {event, meta} of records) {
      ...
      if(meta.continuity2017.type === 'm') {
        ...
        // information so it can be checked and cleared if necessary (if the
        // creator is detected as a replayer)
        if(eventHash === parentHashCommitment) {
          this.peerHeadCommitment = {
            creator,
            eventHash,
            generation,
            basisBlockHeight,
  ...
  async _updateConsensusState({blockRecord, consensusResult} = {}) {
    ...
    if(consensusResult) {
      ...
      if(replayerSet.size > 0) {
        ...
        // clear peer head commitment (do not merge it) if the commitment was
        // for an event created by a now-detected replayer
        if(this.peerHeadCommitment &&
          replayerSet.has(this.peerHeadCommitment.creator)) {
          this.peerHeadCommitment = null;
        }
      }
