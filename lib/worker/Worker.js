...
module.exports = class Worker {
  constructor({session, halt = this._halt.bind(this)} = {}) {
    ...
    this.lastLocalOpsConsensus = false;
  ...
  async init() {
    ...
    // FIXME:
    if(!this.lastLocalOpsConsensus) {
      console.log('lastLocalOpsConsensus should be true');
      process.exit(1);
    }
  ...
  async _addLocalMergeEvent({event, meta} = {}) {
    ...
    // FIXME: get `lastLocalOpsTreeAncestor`
  ...
  async _getHead({peerId} = {}) {
    ...
    if(records.length > 0) {
      ...
      head = {
        ...
        basisBlockHeight,
        mergeHeight,
        // FIXME: add `lastLocalOpsTreeAncestor`
        localAncestorGeneration
      };
  ...
  async _initRecentHistory() {
    ...
    merge local operations, check to see if head's `lastLocalOpsTreeAncestor`
    is a non-consensus event or not. */
    const lastLocalOpsTreeAncestor = head.mergedLocalOps ?
      head.eventHash : head.lastLocalOpsTreeAncestor;
    // start with `true` and set to false if non-consensus match is found
    this.lastLocalOpsConsensus = true;
    ...
    for(const {event, meta} of records) {
      const {
        eventHash,
        continuity2017: {
          // FIXME: everywhere `localAncestorGeneration` is, add
          // `mergedLocalOps` and `lastLocalOpsTreeAncestor`
          creator, type, generation, localAncestorGeneration,
          localReplayNumber, replayDetectedBlockHeight,
          localEventNumber
        }
      } = meta;
      // if event is `lastLocalOpsTreeAncestor` then the last local ops
      // that were merged have not yet reached consensus
      if(lastLocalOpsTreeAncestor === eventHash) {
        this.lastLocalOpsConsensus = false;
      }
  ...
  async _updateConsensusState({blockRecord, consensusResult} = {}) {
    const {consensusState, head, ledgerNode} = this;

    // clear `needsGossipToMerge` flag as consensus state has changed
    this.needsGossipToMerge = false;

    if(consensusResult) {
      // if the last merge event to merge local ops has reached consensus,
      // set `lastLocalOpsConsensus` flag to `true`
      const lastLocalOpsTreeAncestor = head.mergedLocalOps ?
        head.eventHash : head.lastLocalOpsTreeAncestor;

      // remove consensus merge event hashes from recent history
      const {mergeEventHash: eventHashes, replayerSet} = consensusResult;
      const {historyMap} = this;
      for(const eventHash of eventHashes) {
        if(eventHash === lastLocalOpsTreeAncestor) {
          this.lastLocalOpsConsensus = true;
        }
        historyMap.delete(eventHash);
      }
      if(replayerSet.size > 0) {
        // update `replayDetectedBlockHeight` in event summaries for all
        // replays
