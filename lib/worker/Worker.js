...
module.exports = class Worker {
    ...
    try {
      ...
      while(!halt()) {
        ...
        const {blocks, priorityPeers, witnesses, blockHeight} =
          await extendBlockchain({worker: this});
  ...
  async _updateConsensusState() {
    const {consensusState, ledgerNode} = this;
    // FIXME: this is only needed on initialization, otherwise it should
    // be automatically updated when blocks are written
    ...
    // FIXME: use `nextBlockHeight` here instead to avoid confusion
    consensusState.blockHeight = nextBlockHeight;
    consensusState.previousBlockHash = previousBlockHash;
    consensusState.previousBlockId = previousBlockId;
    if(!consensusState.state ||
      consensusState.state.blockHeight !== nextBlockHeight) {
