...
module.exports = class Worker {
  ...
  async _updateHeads({events, dupHashes}) {
    ...
    // walk `creatorChildren` for each head in the cache to update any heads
    // that need updating in LRU cache
    ...
    // FIXME: old code below mixed with `peerChildlessMap` new code
    const parentHashes = [...new Set(_parentHashes)];
    // FIXME: move code to update `peerChildlessMap` up here, separating
    // it from older redis cache updating code
