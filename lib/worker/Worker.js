...
module.exports = class Worker {
    ...
    try {
      ...
      while(!halt()) {
        ...
        const {blocks, priorityPeers, witnesses, blockHeight} =
          await extendBlockchain({worker: this, ledgerNode, savedState, halt});
  ...
  async addPeerEvents({events, dupHashes} = {}) {
  ...
  removeConsensusEvents({eventHashes} = {}) {
    // update recent history
    const {historyMap} = this;
...
async function _validateCache({ledgerNode, creatorId}) {
  ...
  // FIXME: remove once recent history is stored in worker state
