...
module.exports = class Worker {
  ...
  async _initRecentHistory() {
    ...
    // if there are no non-consensus events, then assume that there is a
    // single witness and the head is the latest event on the ledger
    if(records.length === 0) {
      const {eventHash} = head;
      const eventRecord = await ledgerNode.storage.events.get(eventHash);
      const {meta: {continuity2017: {localEventNumber}}} = eventRecord;
