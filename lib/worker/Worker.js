...
module.exports = class Worker {
  ...
  // worker resets due to a work session cycle
  // FIXME: make sync
  async isPeerWithheld({peerId} = {}) {
    const {consensusState: {witnesses}, withheld, withheldCache} = this;
    // if the peer is currently a witness, do not consider it withheld, even
    // if it has some previous events that were withheld
    // FIXME: what do we know about this transition? can we remove withheld
    // events for peers that are now witnesses?
    ...
    /*if(foo) {
      if(withheld && withheld.mergeEvent.meta.continuity2017.creator === peerId) {
        const {witnesses} = await _witnesses.getBlockWitnesses(
          {blockHeight: withheld.mergeEvent.event.basisBlockHeight + 1,
            ledgerNode: this.ledgerNode});
        console.log('witnesses @ withheld creation', witnesses);
        if(witnesses.includes(peerId)) {
          console.log('SHOULD NEVER HAPPEN WITNESS WITHHELD!');
          console.log('withheld', withheld);
          process.exit(1);
        }
      }
      const wh = withheldCache.get(peerId);
      if(wh) {
        console.log('wh', wh, wh.mergeEvent.meta.continuity2017.creator);
        const {witnesses} = await _witnesses.getBlockWitnesses(
          {blockHeight: wh.mergeEvent.event.basisBlockHeight + 1,
            ledgerNode: this.ledgerNode});
        if(witnesses.includes(peerId)) {
          console.log('789 WITNESS WITHHELD!');
          console.log('withheld', wh);
          process.exit(1);
        }
      }
      if(witnesses.includes(peerId)) {
        console.log('XXX WITNESS WITHHELD!', {
          localPeerId: this.localPeerId.substr(-4),
          peerId: peerId.substr(-4),
          nextBlockHeight: this.consensusState.nextBlockHeight,
          witnesses
        });
        if(withheld) {
          console.log('withheld', withheld,
            withheld.mergeEvent.meta.continuity2017.creator);
        }
        if(wh) {
          console.log('wh', wh,
            wh.mergeEvent.meta.continuity2017.creator);
        }
        process.exit(1);
      }
    }*/
  ...
  async _addWithheld({withheld} = {}) {
    ...
    const {witnesses} = await _witnesses.getBlockWitnesses(
      {blockHeight: withheld.mergeEvent.event.basisBlockHeight + 1,
        ledgerNode: this.ledgerNode});
    if(witnesses.includes(withheld.mergeEvent.meta.continuity2017.creator)) {
      console.log('123 WITNESS WITHHELD!');
      console.log('withheld', withheld);
      process.exit(1);
    }
    if((withheld.mergeEvent.event.basisBlockHeight + 2) <=
      this.consensusState.nextBlockHeight) {
      const {witnesses} = await _witnesses.getBlockWitnesses(
        {blockHeight: withheld.mergeEvent.event.basisBlockHeight + 2,
          ledgerNode: this.ledgerNode});
      if(witnesses.includes(withheld.mergeEvent.meta.continuity2017.creator)) {
        console.log('NEXT witnesses',
          withheld.mergeEvent.event.basisBlockHeight + 2,
          witnesses);
        console.log('AAA WITNESS WITHHELD!',
          withheld.mergeEvent.meta.continuity2017.creator.substr(-4));
        console.log('withheld', withheld);
        process.exit(1);
      }
    }
    if(this.consensusState.witnesses.includes(creator)) {
      console.log('CCC NEXT witnesses', {
        nextBlockHeight: this.consensusState.nextBlockHeight,
        witnesses: this.consensusState.witnesses
      });
      console.log('CCC WITNESS WITHHELD!',
        withheld.mergeEvent.meta.continuity2017.creator.substr(-4));
      console.log('withheld', withheld);
      process.exit(1);
    }

    /*console.log('ADDING TO WITHHELD CACHE', {
      localPeerId: this.localPeerId.substr(-4),
      peerId: creator.substr(-4),
      nextBlockHeight: this.consensusState.nextBlockHeight,
      witnesses: this.consensusState.witnesses
    });*/
  ...
  _selectWithheld({witnesses = []} = {}) {
    ...
    if(withheldCache.itemCount === 0 || witnesses.length === 0) {
      return null;
    }
  ...
  async _initRecentHistory() {
    ...
    if(mergeCommitment && !this.mergeCommitment) {
      ...
      if(withheld &&
        ...
        // FIXME: remove me
        //console.log('***restored withheld', parentHashCommitment[0]);
        //console.log('***head', this.head);
        //console.log('***committedBy', mergeCommitment.committedBy);
        this.withheld = withheld;
        mergeCommitment.committedTo = {
    ...
    if(consensusResult) {
      ...
      for(const eventHash of eventHashes) {
        ...
        if(this.mergeCommitment &&
          ...
          // FIXME: remove me
          /*
          const [fooRecord] = await _events.getEvents(
            {worker: this, eventHash});
          console.log('COMMITMENT RECORD', fooRecord);
          if(!fooRecord.meta.consensus ||
            !fooRecord.event.parentHashCommitment ||
            fooRecord.event.parentHashCommitment[0] !==
              this.mergeCommitment.committedTo.eventHash) {
            console.log('commitment is broken!', fooRecord);
            process.exit(1);
          }*/
      ...
      for(const witness of witnesses) {
        // FIXME: remove me
        /*console.log(this.localPeerId.substr(-4),
          'CLEARING WITHHELD CACHE FOR', witness.substr(-4));*/
  ...
  async _writeWithheld({blockHeight} = {}) {
    ...
    // FIXME: remove me
    //console.log('***WRITING WITHHELD', mergeEvent.meta.eventHash);
