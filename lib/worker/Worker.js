...
module.exports = class Worker {
  ...
  async init() {
    ...
    // initialize the next local event number
    await this._initNextLocalEventNumber();
  ...
  async _addLocalMergeEvent({event, meta, peerHeadCommitment = null} = {}) {
    ...
    const {
      creator, isLocalContributor, generation, lastLocalContributor
    } = meta.continuity2017;
  ...
  async _initRecentHistory() {
    ...
    const {getNonConsensusEvents} = ledgerNode.storage.events
      .plugins['continuity-storage'];
    const records = await getNonConsensusEvents(
      {basisBlockHeight: this.head.basisBlockHeight});
    ...
    if(!mergeCommitment) {
      ...
      const {getLatestParentHashCommitment} = ledgerNode.storage.events
        .plugins['continuity-storage'];
      const committedBy = await getLatestParentHashCommitment(
        {creator: localPeerId, minGeneration});
      if(committedBy) {
  ...
  // called in init()
  async _initNextLocalEventNumber() {
    const {getMostRecentLocalEventNumber} = this.ledgerNode.storage.events
      .plugins['continuity-storage'];
    const {localEventNumber} = await getMostRecentLocalEventNumber();
