...
async function _sync(session) {
  ...
  try {
    ...
    while(!halt()) {
      ...
      // if no merge permits were consumed or the only permit used was for
      // a local merge, reset merge permits to cause consensus to run again to
      // handle the case where priority peers have changed since last run
      if(mergePermitsConsumed === 0 || (mergePermitsConsumed === 1 && merged)) {
        mergePermits = 0;
      } else {
