...
async function _sync({session, halt, targetCycles = -1}) {
  ...
  try {
    ...
    while(!halt()) {
      ...
      // FIXME: remove `mergePermits`
      const {priorityPeers, mergePermits, halted, witnesses, blockHeight} =
      ...
      let {mergePermitsConsumed} = await runGossipCycle({
        ...
        mergePermits: mergePermits - 1, needsGossip,
        witnesses, blockHeight, halt
      });
      ...
      // 4. merge if possible
      const {merged, halted: mergeHalted, status: mergeStatus} = await merge({
        ledgerNode, creatorId, priorityPeers, witnesses,
        basisBlockHeight: blockHeight - 1, halt
      });
      needsGossip = !mergeHalted && mergeStatus.needsGossip;
