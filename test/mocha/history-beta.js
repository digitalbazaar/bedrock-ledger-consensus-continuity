...
module.exports = async function historyBeta(
  ...
  [results.cpa, results.cbp] = await Promise.all([
    api.copyAndMerge({from: 'beta', nodes, to: 'alpha'}),
    api.copyAndMerge({from: 'gamma', nodes, to: 'delta'})
  ]);
  [results.cp1, results.cp2] = await Promise.all([
    api.copyAndMerge({from: 'alpha', nodes, to: 'beta'}),
    api.copyAndMerge({from: 'delta', nodes, to: 'gamma'})
  ]);
  ...
  [results.cp3, results.cp4] = await Promise.all([
    api.copyAndMerge({from: ['beta', 'delta'], nodes, to: 'gamma'}),
    api.copyAndMerge({
      from: ['alpha', 'gamma'], nodes, to: 'beta',
      useSnapshot: true
    })
  ]);
  ...
  [results.cp5, results.cp6] = await Promise.all([
    api.copyAndMerge({from: 'beta', nodes, to: 'gamma'}),
    api.copyAndMerge({from: 'gamma', nodes, to: 'beta', useSnapshot: true})
  ]);
  ...
  results.cp15 = await api.copyAndMerge({from: 'beta', nodes, to: 'delta'});
  results.cp16 = await api.copyAndMerge(
    {from: ['beta', 'delta'], nodes, to: 'alpha'});
  results.cp17 = await api.copyAndMerge(
    {from: ['alpha', 'delta'], nodes, to: 'beta'});

  // snapshot beta before copy
  results.ss17 = await api.snapshotEvents({ledgerNode: nodes.beta});
  results.cp18 = await api.copyAndMerge({from: 'beta', nodes, to: 'delta'});
  ...
  results.cp20 = await api.copyAndMerge({
    from: ['alpha', 'delta'], nodes, to: 'gamma',
    useSnapshot: true
  });
  ...
  [results.cp22, results.cp23] = await Promise.all([
    ...
    api.copyAndMerge({
      from: ['beta', 'gamma'], nodes, to: 'delta',
      useSnapshot: true
    })
  ]);
  results.cp24 = await api.copyAndMerge({from: 'delta', nodes, to: 'gamma'});
