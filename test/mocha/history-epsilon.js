...
module.exports = async function historyEpsilon(
  ...
  [results.cp15, results.cp16] = await Promise.all([
    api.copyAndMerge({from: ['alpha', 'gamma'], nodes, to: 'beta'}),
    api.copyAndMerge({from: ['beta', 'delta'], nodes, to: 'gamma'})
  ]);
  [results.cp17, results.cp18] = await Promise.all([
    api.copyAndMerge({from: 'gamma', nodes, to: 'beta'}),
    api.copyAndMerge({from: 'beta', nodes, to: 'gamma'})
  ]);
  ...
  [results.cp19, results.cp20] = await Promise.all([
    api.copyAndMerge({from: ['beta', 'gamma'], nodes, to: 'alpha'}),
    api.copyAndMerge({from: ['beta', 'gamma'], nodes, to: 'delta'})
  ]);
